...
test("Japan environment preserves honest MHLW classification scopes", () => {
  assert.match(migration, /'JP:environmental-scientist'[\s\S]*null,'JPY',false/)
  assert.match(migration, /'JP:environmental-scientist','004-01'/)
  assert.match(migration, /'JP:environmental-scientist','011-99'/)
  assert.match(migration, /'JP:agronomist'[\s\S]*'005-01','JPY',false/)
  assert.match(migration, /'JP:farm-manager'[\s\S]*null,'JPY',false/)
  for (const code of ["064-01", "064-02", "064-03"]) assert.match(migration, new RegExp(`'JP:farm-manager','${code}'`))
  assert.match(migration, /'JP:forestry-technician'[\s\S]*'005-01','JPY',false/)
  assert.match(migration, /'JP:food-technologist'[\s\S]*null,'JPY',false/)
  assert.match(migration, /'JP:food-technologist','006-01'/)
  assert.match(migration, /'JP:food-technologist','007-01'/)
  assert.match(migration, /'JP:sustainability-specialist'[\s\S]*null,'JPY',false/)
  assert.match(migration, /'JP:horticulturist'[\s\S]*null,'JPY',false/)
  assert.match(migration, /'JP:horticulturist','064-02'/)
  assert.match(migration, /'JP:horticulturist','064-05'/)
  assert.match(migration, /'JP:animal-science-technician'[\s\S]*'005-01','JPY',false/)
})

test("Japan environment does not invent universal licences", () => {
  for (const id of careers) assert.match(migration, new RegExp(`'JP:${id}'[\\s\\S]*'JPY',false`), id)
