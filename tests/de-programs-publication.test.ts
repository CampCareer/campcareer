...
test("Germany remains published when Singapore is added next",()=>{assert.match(header,/"DE", "SG"/);assert.match(page,/searchDePrograms/);assert.match(page,/DeProgramsExplorer/);assert.match(page,/searchSgPrograms/);assert.match(page,/SgProgramsExplorer/)})
