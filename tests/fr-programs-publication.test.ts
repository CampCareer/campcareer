...
test("France remains published in the shared Programs route",()=>{assert.match(header,/PUBLISHED_PROGRAM_COUNTRIES/);assert.match(header,/"FR"/);assert.match(page,/searchFrPrograms/);assert.match(page,/FrProgramsExplorer/);assert.match(page,/filters\.country === "FR"/)})
