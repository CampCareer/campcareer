...
test("FIFO guide sales page renders the completed product, actual PDF previews and separate purchase checkout", async ({ page }) => {
  ...
  await expect(capture.getByRole("link", { name: "Terms of Service" })).toHaveAttribute("href", "/terms")
  await expect(capture.getByRole("link", { name: "Privacy Policy" })).toHaveAttribute("href", "/privacy")
  ...
  await expect(restoredCapture.getByRole("checkbox", { name: /I request immediate delivery of the digital guide/i })).not.toBeChecked()
  await expect(restoredCapture.getByRole("checkbox", { name: /After purchase, also send me occasional CampCareer FIFO updates/i })).not.toBeChecked()
