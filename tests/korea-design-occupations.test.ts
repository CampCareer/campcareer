...
test("Korea design preserves shared-code scope boundaries", () => {
  ...
  assert.match(migration, /'KR:graphic-designer'[\s\S]*'4154'/)
  assert.match(migration, /'KR:ux-designer'[\s\S]*'4154'/)
  assert.match(migration, /'KR:multimedia-designer'[\s\S]*'4155'/)
  assert.match(migration, /'KR:web-designer'[\s\S]*'4155'/)
})

test("Korea architect keeps the regulated professional scope explicit", () => {
  assert.match(migration, /'KR:architect'[\s\S]*'1401'[\s\S]*true/)
