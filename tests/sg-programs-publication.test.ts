...
test("Singapore is published after Germany without changing later-country scope",()=>{assert.match(header,/"DE", "SG"/);assert.match(page,/searchSgPrograms/);assert.match(page,/SgProgramsExplorer/);assert.match(page,/filters\.country===?\s*"SG"|filters\.country==="SG"/);assert.match(sitemap,/INDEXABLE_SG_PROGRAM_PATHS/)})
