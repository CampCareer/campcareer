...
test("Korea environment keeps broad KECO mappings explicit", () => {
  assert.match(migration, /environmental-scientist'.*'1221'/s)
  assert.match(migration, /agronomist'.*'1221'/s)
  assert.match(migration, /farm-manager'.*'KECO','2025',null/s)
  assert.match(migration, /forestry-technician'.*'1223'/s)
  assert.match(migration, /food-technologist'.*'1571'/s)
  assert.match(migration, /sustainability-specialist'.*'1555'/s)
  assert.match(migration, /horticulturist'.*'9014'/s)
  assert.match(migration, /animal-science-technician'.*'1223'/s)
