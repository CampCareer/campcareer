...
test("Phase 8 preserves programme/HBO pending disclosures and source methodology caveats", () => {
  ...
  assert.ok(compareMatrix.includes("Programme coverage remains verification pending"))
  assert.ok(compareMatrix.includes("HBO provider coverage remains incomplete"))
  assert.ok(compareMatrix.includes("CBS municipality"))
  assert.ok(compareMatrix.includes("Source-native transport products are preserved"))
