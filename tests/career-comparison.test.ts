...
test("career IDs trim, allowlist, dedupe, preserve order, and cap at three", () => {
  assert.deepEqual(
    normalizeCareerIds(" registered-nurse , electrician, electrician,not-a-career, carpenter,plumber"),
    ["registered-nurse", "electrician", "carpenter"],
  )
...
test("career URL is canonical and city falls back to National view", () => {
  assert.equal(
    buildCareerCompareHref("sydney", ["registered-nurse", "electrician"]),
    "/compare?type=career&country=AU&city=sydney&profile=starting-from-scratch&careers=registered-nurse,electrician",
  )
  assert.equal(
  ...
  const city = parseCareerComparisonState(new URLSearchParams("country=AU&city=melbourne&profile=starting-from-scratch&careers=registered-nurse,electrician"))
  ...
  const national = parseCareerComparisonState(new URLSearchParams("country=AU&city=unknown&profile=starting-from-scratch&careers=registered-nurse,electrician"))
...
test("career selection helpers cover 0, 1, 2, and 3 selections", () => {
  ...
  const two = appendCareer(one, "electrician")
  const three = appendCareer(two, "carpenter")
  assert.deepEqual(one, ["registered-nurse"])
  assert.deepEqual(two, ["registered-nurse", "electrician"])
  assert.deepEqual(three, ["registered-nurse", "electrician", "carpenter"])
  assert.deepEqual(replaceCareerAtIndex(two, 1, "plumber"), ["registered-nurse", "plumber"])
  assert.deepEqual(replaceCareerAtIndex(two, 1, "registered-nurse"), two)
  assert.deepEqual(removeCareerAtIndex(three, 1), ["registered-nurse", "carpenter"])
})

test("career chooser exposes the four occupation-backed AU careers and disables duplicates", () => {
  ...
  assert.equal(getCareerCompareOptions(["registered-nurse"], "electrician").find((option) => option.id === "registered-nurse")?.disabled, true)
  assert.equal(AU_CAREER_COMPARISON_CATALOG.some((career) => (career.id as string) === "software-engineer"), false)
})

test("AU career catalog is null-safe before server hydration", () => {
  const rows = getCareerComparisonRows(AU_CAREER_COMPARISON_CATALOG)
  const score = rows.find((row) => row.key === "opportunityScore")
  const salary = rows.find((row) => row.key === "annualisedMedianSalary")
  assert.ok(score)
  assert.ok(salary)
  assert.ok(score?.values.every((value) => value.primary === "Not available"))
  assert.ok(salary?.values.every((value) => value.primary === "Not available"))
...
test("hydrated career metrics format for comparison without fabricating missing values", () => {
  const base = AU_CAREER_COMPARISON_CATALOG[0]
  const hydrated = {
    ...base,
    snapshot: {
      asOfDate: "2026-05-01",
      employmentTotal: 366200,
      medianWeeklyEarnings: 2192,
      annualisedMedianSalary: 113984,
      vacanciesThreeMonthAvg: 7287.66667,
      vacancyYoyPct: 21.99,
      employmentGrowth5yPct: 13.67,
      employmentGrowth10yPct: 26.08,
      opportunityScore: 93,
      scoreStatus: "provisional",
      publicationStatus: "decision_ready",
    },
  }
  const rows = getCareerComparisonRows([hydrated])
  assert.equal(rows.find((row) => row.key === "opportunityScore")?.values[0].primary, "93/100")
  assert.equal(rows.find((row) => row.key === "annualisedMedianSalary")?.values[0].primary, "AUD 113,984")
  assert.equal(rows.find((row) => row.key === "employmentTotal")?.values[0].primary, "366,200")
  assert.equal(rows.find((row) => row.key === "employmentGrowth5yPct")?.values[0].primary, "+13.7%")
  assert.equal(rows.find((row) => row.key === "publicationStatus")?.values[0].primary, "Decision Ready")
})
