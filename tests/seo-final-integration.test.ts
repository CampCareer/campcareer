...
test("institutions and city fallback pages declare canonical behavior", () => {
  ...
  const caDetail = readFileSync("src/app/(workspace)/institutions/ca-institution-detail.tsx", "utf8")
  ...
  assert.ok(detail.includes("CaInstitutionDetailView"))
  assert.ok(caDetail.includes("caProgramDetailPath"))
  assert.ok(caDetail.includes("Published target programs"))
