...
-- Greater Sydney membership is a conservative locality whitelist over the current official CRICOS snapshot.
...
select
  s.institution_id,
  s.location_name,
  case when s.is_greater_sydney then 'Sydney' else initcap(coalesce(nullif(btrim(s.city),''),'Unknown')) end,
  s.state,
  'AU',
  'active',
  case when s.is_greater_sydney then g.id else null end,
  case when s.is_greater_sydney then initcap(nullif(btrim(s.city),'')) else null end,
...
left join core.geographies g
  on g.country_code='AU' and g.geography_type='city' and g.slug='sydney' and g.canonical_geography_id is null and g.status='active'
...
join catalog.campuses c
 ...
 and c.city=case when s.is_greater_sydney then 'Sydney' else initcap(coalesce(nullif(btrim(s.city),''),'Unknown')) end
