create temporary table _field_outcomes_uk on commit drop as
...
cross join lateral (
  values ('median_earnings',f.median_earnings::numeric,'GBP'::text),
...
select s.evidence_id,s.metric_key,'field','UK:'||s.field_name,
       to_jsonb(s.metric_value),s.unit,ss.id,'observed','medium','Legacy field outcome import',
       jsonb_build_object('legacy_table','field_earnings_uk','legacy_id',s.legacy_id,'legacy_source',s.source),
       'review_required',s.updated_at
from _field_outcomes_uk s
...
select 'UK',field_name,'all',metric_key,metric_value,unit,evidence_id,'review_required'
from _field_outcomes_uk;