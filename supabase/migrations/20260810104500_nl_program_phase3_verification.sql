...
-- Replace Twente overview URLs with exact current programme pages and capture
-- the official CROHO/RIO programme codes displayed on those pages.
...
-- Twente exact programme pages display current full-time English bachelor
-- provision and NVAO accreditation. The university's foreign-diploma bachelor
-- admission page is the programme-family international admission source.
...
-- WUR's current international-prior-education bachelor admission page names
-- all eight staged English-taught bachelor programmes and their subject rules.
UPDATE public.program_international_nl_staging x
...
-- UvA Business Analytics has a dedicated international-prior-education path,
-- while the September 2026 application window is currently closed. Eligibility
-- and current application state are deliberately represented separately.
UPDATE public.program_international_nl_staging x
SET full_time_evidence = TRUE,
    international_students_eligible = TRUE,
...
-- The three HBO providers remain provider-identity candidates, but their current
-- official programme pages do establish programme-level international study
-- evidence. Capturing that evidence does not promote the providers canonically.
UPDATE public.program_international_nl_staging x
SET full_time_evidence = TRUE,
    international_students_eligible = TRUE,
    international_admission_status = 'programme_international_evidence_present',
...
-- Publication-facing admission state. Do not infer an open application window
-- merely from programme existence or general international eligibility.
...
-- Assign verification tiers after the evidence refresh.
UPDATE public.program_catalog_nl_staging p
