do $$
begin
  if exists(select 1 from public.program_catalog_es_staging where institution_id is null) then raise exception 'ES program canonicalization blocked: staged program missing institution_id'; end if;
  if (select count(*) from public.program_catalog_es_staging) <> (select count(*) from public.program_international_es_staging) then raise exception 'ES program canonicalization blocked: international evidence not 1:1'; end if;
end $$;
...
select md5('ES|PROGRAM|'||p.source_program_key)::uuid,p.institution_id,p.title,p.degree_level,p.field_category,case when p.duration_years is null then null else greatest(1,round(p.duration_years*12)::int) end,'active',now()
from public.program_catalog_es_staging p where p.verification_tier in ('A','B')
...
select md5('ES|PROGRAM|'||p.source_program_key)::uuid,'ES_RUCT_PROGRAM_KEY',p.source_name||':'||p.source_program_key,p.official_program_url,p.source_as_of,null
from public.program_catalog_es_staging p where p.verification_tier in ('A','B')
...
select md5('ES|OFFERING|'||p.source_program_key)::uuid,md5('ES|PROGRAM|'||p.source_program_key)::uuid,
  ...
  coalesce(x.admission_source_url,p.international_source_url,p.official_program_url),coalesce(x.source_as_of,p.source_as_of),null,now(),now(),'ES_RUCT_OFFICIAL',p.source_name||':'||p.source_program_key,
...
from public.program_catalog_es_staging p join public.program_international_es_staging x on x.program_catalog_id=p.id
...
create or replace view public.program_explorer_es_v1 with (security_invoker=true) as
...
from public.program_catalog_es_staging p join catalog.programmes pr on pr.id=md5('ES|PROGRAM|'||p.source_program_key)::uuid join catalog.institutions i on i.id=pr.institution_id join public.program_international_es_staging x on x.program_catalog_id=p.id left join public.program_occupation_es_staging o on o.program_catalog_id=p.id and o.review_status='approved'
...
create or replace view public.program_detail_es_v1 with (security_invoker=true) as
select e.*,'RUCT official-degree registration plus current Spanish university programme evidence; programme identity and current admission are tracked separately, and RUCT or quality-system status is not converted into a CampCareer programme-accreditation claim without decision-level matching.'::text as source_verification_label,false as has_programme_accreditation_claim
from public.program_explorer_es_v1 e;
