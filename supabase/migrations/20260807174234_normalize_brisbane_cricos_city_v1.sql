-- Normalize verified CRICOS Greater Brisbane locations onto the canonical Brisbane city ID.
-- Uses QLD + conservative locality membership; postcode is not used for metro classification.

create temporary table _brisbane_cricos_locations on commit drop as
select
  l.provider_code,
  l.location_name,
...
where upper(btrim(coalesce(l.state,'')))='QLD'
...
set city='Brisbane',
    ...
    metadata=(c.metadata - 'greater_brisbane_v1') || jsonb_build_object('greater_brisbane_v1',true),
    updated_at=now()
from _brisbane_cricos_locations s
join catalog.campus_identifiers ci
  on ci.identifier_system='AU_CRICOS_LOCATION_KEY'
 and ci.identifier_value=s.location_key
cross join brisbane b
where c.id=ci.campus_id;

-- Rebuild course-level verified city arrays from the official CRICOS campuses.
...
-- Republish official campus/provider snapshots for all normalized launch cities.
with target_cities as (
  select id
  from core.geographies
  where country_code='AU'
    and geography_type='city'
    and slug in ('sydney','melbourne','brisbane')
    and canonical_geography_id is null
    and status='active'
)
...
where c.country_code='AU'
  ...
  and g.slug in ('sydney','melbourne','brisbane')
