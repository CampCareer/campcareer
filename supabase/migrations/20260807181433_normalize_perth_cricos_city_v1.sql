-- Normalize verified CRICOS Greater Perth locations onto the canonical Perth city ID.
-- Uses WA + conservative locality membership; postcode is not used for metro classification.

create temporary table _perth_cricos_locations on commit drop as
select
  l.provider_code,
  l.location_name,
...
where upper(btrim(coalesce(l.state,'')))='WA'
...
set city='Perth',
    ...
    metadata=(c.metadata - 'greater_perth_v1') || jsonb_build_object('greater_perth_v1',true),
    updated_at=now()
from _perth_cricos_locations s
...
with target_cities as (
  ...
  where country_code='AU'
    and geography_type='city'
    and slug in ('sydney','melbourne','brisbane','perth')
    and canonical_geography_id is null
    and status='active'
)
...
where c.country_code='AU'
  ...
  and g.slug in ('sydney','melbourne','brisbane','perth')
