...
select 'IE', 'Atlantic Technological University', 'education_provider',
       'https://www.atu.ie/', 'active', 'atlantic-technological-university',
       'university', 'public'
where not exists (
  select 1 from catalog.institutions
  where country_code='IE' and slug='atlantic-technological-university'
);
...
select 'IE', 'Technological University of the Shannon: Midlands Midwest', 'education_provider',
       'https://tus.ie/', 'active', 'technological-university-of-the-shannon',
       'university', 'public'
where not exists (
  select 1 from catalog.institutions
  where country_code='IE' and slug='technological-university-of-the-shannon'
);

create temp table ie_institution_merge_map(
...
-- Move canonical programme ownership to the current parent institution.
...
-- Campus IDs remain stable so all programme_offerings keep their existing FKs.
...
join catalog.institutions alias
  on alias.country_code='IE' and alias.slug=m.alias_slug
where c.institution_id=alias.id;

-- Preserve every legacy provider/name identity, but attach it to the current parent.
...
-- Preserve legacy institution entity-map keys so historical imports still resolve.
update catalog.legacy_entity_map lem
set entity_id = parent.id,
    metadata = coalesce(lem.metadata,'{}'::jsonb) || jsonb_build_object(
      'institution_normalization','ie_current_parent_v1',
...
join catalog.institutions alias
  on alias.country_code='IE' and alias.slug=m.alias_slug
where lem.entity_type='institution'
  and lem.entity_id=alias.id;

update catalog.institutions alias
set status='inactive',
    updated_at=now()
from ie_institution_merge_map m
where alias.country_code='IE'
  and alias.slug=m.alias_slug;

-- User-facing current names for two imported records whose legacy labels were noisy.
update catalog.institutions
set canonical_name='Institute of Art, Design and Technology',
    website_url='https://iadt.ie/',
...
update catalog.institutions
set canonical_name='Dundalk Institute of Technology',
...
update catalog.institutions
set canonical_name='National College of Art and Design',
    updated_at=now()
where country_code='IE' and slug='national-college-of-art-and-design';

create temp table ie_hea_profiles(
  slug text primary key,
  hea_listed_name text not null,
...
set website_url=p.website_url,
    ...
    status='active',
    updated_at=now()
from ie_hea_profiles p
where i.country_code='IE' and i.slug=p.slug;

-- This is explicitly HEA-list membership, not a fabricated numeric provider code.
...
create or replace view public.institution_identity_ie_v1
...
select
  ...
  i.ownership_type,
  i.website_url,
  hea.identifier_value as hea_listed_name,
  hea.source_url as hea_source_url,
  coalesce(
...
join catalog.institution_identifiers hea
  on hea.institution_id=i.id
 and hea.identifier_system='IE_HEA_LISTED_HEI_NAME'
...
group by i.id,i.country_code,i.slug,i.canonical_name,i.institution_kind,
         i.ownership_type,i.website_url,hea.identifier_value,hea.source_url;

comment on view public.institution_identity_ie_v1 is
  'Service-role Ireland HEA-listed Institution identity read model. HEA membership is source-backed; legacy provider IDs remain historical import keys and are not presented as official regulatory codes.';
...
declare
  ...
  hea_identity_count integer;
  hea_website_count integer;
  active_alias_count integer;
  programme_parent_failures integer;
...
begin
  ...
  if active_programs <> 2876 then
    raise exception 'Expected 2876 active Irish programmes after institution normalization, found %', active_programs;
  end if;
  ...
  if active_institutions <> 185 then
    raise exception 'Expected 185 active Irish institutions after current-parent consolidation, found %', active_institutions;
  end if;

  select count(*) into hea_identity_count
  from public.institution_identity_ie_v1;
  if hea_identity_count <> 17 then
    raise exception 'Expected 17 HEA-listed programme-relevant institutions in the IE identity view, found %', hea_identity_count;
  end if;

  select count(*) into hea_website_count
  from public.institution_identity_ie_v1
  where website_url like 'https://%';
  if hea_website_count <> 17 then
    raise exception 'Expected official websites for all 17 HEA-listed programme-relevant institutions, found %', hea_website_count;
  end if;
  ...
  if active_alias_count > 0 then
    raise exception 'Found % technological-university/legacy aliases still active', active_alias_count;
  end if;

  select count(*) into programme_parent_failures
  from catalog.programmes p
  join catalog.institutions i on i.id=p.institution_id
  where i.country_code='IE' and p.status='active' and i.status='inactive';
  if programme_parent_failures > 0 then
    raise exception 'Found % active Irish programmes still attached to inactive institution aliases', programme_parent_failures;
  end if;
  ...
  if offering_count <> 2876 then
    raise exception 'Expected 2876 Irish programme offerings to remain connected, found %', offering_count;
  end if;

  select jsonb_object_agg(slug,program_count) into parent_counts
  from (
    select i.slug,count(*)::integer as program_count
    from catalog.institutions i
  ...
  if parent_counts is distinct from jsonb_build_object(
    'atlantic-technological-university',336,
    'munster-technological-university',125,
    'south-east-technological-university',234,
    'technological-university-of-the-shannon',193
  ) then
    raise exception 'Unexpected TU programme totals after consolidation: %', parent_counts;
  end if;
end $$;
