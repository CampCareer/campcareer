do $$
begin
  if exists(select 1 from public.program_catalog_sg_staging where institution_id is null) then raise exception 'SG program canonicalization blocked: staged program missing institution_id'; end if;
  if (select count(*) from public.program_catalog_sg_staging where verification_tier in ('A','B') and study_mode='full_time') <> (select count(*) from public.program_international_sg_staging) then raise exception 'SG program canonicalization blocked: international evidence not 1:1'; end if;
end $$;
...
select md5('SG|PROGRAM|'||p.source_name||'|'||p.source_program_key)::uuid,p.institution_id,p.title,p.degree_level,p.field_category,case when p.duration_years is null then null else greatest(1,round(p.duration_years*12)::int) end,'active',now()
from public.program_catalog_sg_staging p where p.verification_tier in ('A','B') and p.study_mode='full_time'
...
select md5('SG|PROGRAM|'||p.source_name||'|'||p.source_program_key)::uuid,'SG_OFFICIAL_PROGRAM_KEY',p.source_name||':'||p.source_program_key,p.official_program_url,p.source_as_of,null
from public.program_catalog_sg_staging p where p.verification_tier in ('A','B') and p.study_mode='full_time'
...
select md5('SG|OFFERING|'||p.source_name||'|'||p.source_program_key)::uuid,md5('SG|PROGRAM|'||p.source_name||'|'||p.source_program_key)::uuid,
  (select c.id from catalog.campuses c where c.institution_id=p.institution_id and c.status='active' order by case when lower(coalesce(c.city,''))='singapore' then 0 else 1 end,c.created_at,c.id limit 1),
  ...
  coalesce(x.admission_source_url,p.international_source_url,p.official_program_url),coalesce(x.source_as_of,p.source_as_of),null,now(),now(),'SG_OFFICIAL_UNIVERSITIES',p.source_name||':'||p.source_program_key,
...
from public.program_catalog_sg_staging p join public.program_international_sg_staging x on x.program_catalog_id=p.id
where p.verification_tier in ('A','B') and p.study_mode='full_time'
...
create or replace view public.program_explorer_sg_v1 with (security_invoker=true) as
...
from public.program_catalog_sg_staging p join catalog.programmes pr on pr.id=md5('SG|PROGRAM|'||p.source_name||'|'||p.source_program_key)::uuid join catalog.institutions i on i.id=pr.institution_id join public.program_international_sg_staging x on x.program_catalog_id=p.id left join public.program_occupation_sg_staging o on o.program_catalog_id=p.id and o.review_status='approved'
where pr.status='active' and p.verification_tier in ('A','B') and p.study_mode='full_time'
...
create or replace view public.program_detail_sg_v1 with (security_invoker=true) as
select e.*,'Current Singapore local-university programme evidence from NUS, NTU, SMU, SIT, SUSS and SUTD official sources. Programme existence, full-time study mode, international applicant timing and ICA Student''s Pass context are tracked separately. Institution status or programme listing is not converted into a CampCareer programme-accreditation or professional-registration claim without programme-specific evidence.'::text as source_verification_label,false as has_programme_accreditation_claim
from public.program_explorer_sg_v1 e;

revoke all on public.program_explorer_sg_v1 from public,anon,authenticated;
revoke all on public.program_detail_sg_v1 from public,anon,authenticated;
grant select on public.program_explorer_sg_v1 to service_role;
grant select on public.program_detail_sg_v1 to service_role;
