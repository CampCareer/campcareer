-- Normalize verified CRICOS Greater Adelaide locations onto the canonical Adelaide city ID.
-- Uses SA + reviewed locality membership. A CRICOS anomaly with city='AUSTRALIA' is included only
-- for the exact North Terrace location name; postcode is not used as the general metro rule.

create temporary table _adelaide_cricos_locations on commit drop as
select
  l.provider_code,
  l.location_name,
...
where upper(btrim(coalesce(l.state,'')))='SA'
  and (
    upper(btrim(coalesce(l.city,''))) in (
      'ADELAIDE','NORTH ADELAIDE','REGENCY PARK','WAYVILLE','BEDFORD PARK','CLOVELLY PARK','NETHERBY',
      'URRBRAE','MAGILL','GILLES PLAINS','MAWSON LAKES','PARAFIELD AIRPORT'
    )
    or (
      upper(btrim(coalesce(l.city,'')))='AUSTRALIA'
      and upper(btrim(l.location_name))='THE UNIVERSITY OF ADELAIDE, NORTH TERRACE'
    )
  );
...
set city='Adelaide',
    geography_id=a.id,
    locality=case
      when upper(btrim(coalesce(s.official_city,'')))='AUSTRALIA' then 'Adelaide'
      else initcap(nullif(btrim(s.official_city),''))
    end,
    metadata=(c.metadata - 'greater_adelaide_v1') || jsonb_build_object('greater_adelaide_v1',true),
    updated_at=now()
from _adelaide_cricos_locations s
join catalog.campus_identifiers ci
  on ci.identifier_system='AU_CRICOS_LOCATION_KEY'
 and ci.identifier_value=s.location_key
cross join adelaide a
...
with location_rows as (
  select
    ...
    cl.course_code,
    cl.location_name,
    case when upper(btrim(coalesce(l.city,'')))='AUSTRALIA' and cp.geography_id is not null then cp.locality else l.city end as official_city,
    l.state as official_state,
    l.postcode,
    cp.id as campus_id,
...
with target_cities as (
  ...
  where country_code='AU'
    and geography_type='city'
    and slug in ('sydney','melbourne','brisbane','perth','adelaide')
    and canonical_geography_id is null
    and status='active'
)
...
where c.country_code='AU'
  ...
  and g.slug in ('sydney','melbourne','brisbane','perth','adelaide')
