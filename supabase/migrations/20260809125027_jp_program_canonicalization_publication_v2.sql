do $$
begin
  if exists(select 1 from public.program_catalog_jp_staging where institution_id is null) then raise exception 'JP program canonicalization blocked: staged program missing institution_id'; end if;
  if (select count(*) from public.program_catalog_jp_staging) <> (select count(*) from public.program_international_jp_staging) then raise exception 'JP program canonicalization blocked: international evidence not 1:1'; end if;
end $$;

insert into catalog.campuses(id,institution_id,name,city,country_code,status,source_url,source_checked_at,metadata)
select md5('JP|PROGRAM_PROVIDER_CAMPUS|'||i.id::text)::uuid,i.id,'Program publication location',min(p.city),'JP','active',min(coalesce(p.international_source_url,p.official_program_url)),now(),jsonb_build_object('scope','program_provenance_only','source','JP programme rollout')
from public.program_catalog_jp_staging p join catalog.institutions i on i.id=p.institution_id
where not exists (select 1 from catalog.campuses c where c.institution_id=i.id and c.status='active')
group by i.id
on conflict(id) do update set city=excluded.city,source_url=excluded.source_url,source_checked_at=excluded.source_checked_at,updated_at=now();
...
select md5('JP|PROGRAM|'||p.source_program_key)::uuid,p.institution_id,p.title,p.degree_level,p.field_category,case when p.duration_years is null then null else greatest(1,round(p.duration_years*12)::int) end,'active',now()
from public.program_catalog_jp_staging p where p.verification_tier in ('A','B')
...
select md5('JP|PROGRAM|'||p.source_program_key)::uuid,'JP_PROGRAM_SOURCE_KEY',p.source_name||':'||p.source_program_key,coalesce(p.official_program_url,p.international_source_url),p.source_as_of,null
from public.program_catalog_jp_staging p where p.verification_tier in ('A','B')
...
select md5('JP|OFFERING|'||p.source_program_key)::uuid,md5('JP|PROGRAM|'||p.source_program_key)::uuid,
  ...
  coalesce(x.admission_source_url,p.international_source_url,p.official_program_url),coalesce(x.source_as_of,p.source_as_of),null,now(),now(),'JP_PROGRAM_SOURCE',p.source_name||':'||p.source_program_key,
...
from public.program_catalog_jp_staging p join public.program_international_jp_staging x on x.program_catalog_id=p.id
...
create or replace view public.program_detail_jp_v1 with (security_invoker=true) as
select e.*,'Official university/provider + JASSO Study in Japan international-student source'::text as source_verification_label,false as has_programme_accreditation_claim
from public.program_explorer_jp_v1 e;

revoke all on public.program_explorer_jp_v1 from public,anon,authenticated;
revoke all on public.program_detail_jp_v1 from public,anon,authenticated;
grant select on public.program_explorer_jp_v1 to service_role;
grant select on public.program_detail_jp_v1 to service_role;
