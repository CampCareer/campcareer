...
insert into catalog.programme_identifiers(programme_id,identifier_system,identifier_value,source_url)
select s.programme_id,'CRICOS',c.cricos_code,s.source_url
from _programme_stage s
join public.courses_au c on s.legacy_table='courses_au' and s.legacy_key=c.id::text
where c.cricos_code is not null and c.cricos_code<>''
on conflict (identifier_system,identifier_value)
do update set programme_id=excluded.programme_id,source_url=excluded.source_url;

insert into catalog.programme_identifiers(programme_id,identifier_system,identifier_value,source_url)
select s.programme_id,'PROVIDER_COURSE_CODE','AU:'||c.institution_id||':'||c.course_code,s.source_url
from _programme_stage s join public.courses_au c on s.legacy_table='courses_au' and s.legacy_key=c.id::text where c.course_code is not null
union all
select s.programme_id,'PROVIDER_COURSE_CODE','CA:'||c.institution_id||':'||c.course_code,s.source_url
from _programme_stage s join public.courses_ca c on s.legacy_table='courses_ca' and s.legacy_key=c.id::text where c.course_code is not null
union all
select s.programme_id,'PROVIDER_COURSE_CODE','IE:'||c.institution_id||':'||coalesce(c.course_code,c.cao_code),s.source_url
from _programme_stage s join public.courses_ie c on s.legacy_table='courses_ie' and s.legacy_key=c.id::text where coalesce(c.course_code,c.cao_code) is not null
on conflict (identifier_system,identifier_value)
do update set programme_id=excluded.programme_id,source_url=excluded.source_url;
...
from _programme_stage where fee_amount is not null and fee_amount>0;
