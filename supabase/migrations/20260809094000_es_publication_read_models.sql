-- Spain Tier A Explorer / Detail publication read models.
-- Institution identity and verified primary administrative locations publish
-- independently of the future Spain programme pipeline.

create or replace view public.institution_explorer_es_v1
...
left join lateral (
  ...
  from public.institution_location_es_v1 l where l.institution_id=i.id
) locations on true
where i.country_code='ES' and i.status<>'inactive' and i.slug is not null;

revoke all on public.institution_explorer_es_v1 from public,anon,authenticated;
grant select on public.institution_explorer_es_v1 to service_role;

create or replace view public.institution_detail_es_v1
with (security_invoker = true) as
select
...
revoke all on public.institution_detail_es_v1 from public,anon,authenticated;
grant select on public.institution_detail_es_v1 to service_role;

do $$ declare c int; begin
  select count(*) into c from public.institution_explorer_es_v1;
  if c<>10 then raise exception 'Expected 10 ES explorer rows, found %',c; end if;
  select count(*) into c from public.institution_detail_es_v1
  where campus_count<1 or jsonb_array_length(campus_locations)<1;
  if c>0 then raise exception 'ES details missing location %',c; end if;
  select count(*) into c from public.institution_detail_es_v1
  where program_count<>0 or jsonb_array_length(programme_preview)<>0;
  if c>0 then raise exception 'ES programme catalogue expected pending %',c; end if;
end $$;
