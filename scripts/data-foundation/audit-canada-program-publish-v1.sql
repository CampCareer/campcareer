...
with joined as (
  ...
  select *,
    case
      ...
      when lower(coalesce(international_program_admission_status,'')) ~ '(suspended|cancelled|not_accepting|not currently|not_current|unavailable|closed|not_yet_open|not yet open|restricted_not_open|temporarily_paused|not_eligible_for_study_permit|legacy_program)'
