...
-- Baseline captured 2026-08-08 against public staging tables.
...
with joined as (
  select
    ...
    p.pgwp_rule_category,
    p.pgwp_program_status,
    p.cip_code,
    p.verified_at
  from public.program_catalog_ca_staging c
  left join public.program_pgwp_ca_staging p
    on p.program_catalog_id = c.id
), decisions as (
  select
    *,
    case
    ...
    case
      when ircc_program_eligible is true then 'eligible'
      when ircc_program_eligible is false then 'ineligible'
      else 'unknown'
    end as pgwp_state
  from joined
), classified as (
...
-- Hold-reason breakdown for Tier C investigation.
with joined as (
  select c.*, p.matched_dli_number, p.international_students_eligible,
         p.international_program_admission_status
  from public.program_catalog_ca_staging c
