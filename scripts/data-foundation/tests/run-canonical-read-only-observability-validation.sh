...
#
# The tooling report is written to $VALIDATION_OUTPUT_DIR/validation_report.tooling.json
# (default: the 2026-08-02 canonical observability artifact directory).
# The actual observation report (validation_report.json, task 10.7A-2) is
# auto-generated by the runner itself on observed runs.
#
# Usage:
...
echo "  tooling report: $VALIDATION_OUTPUT_DIR/validation_report.tooling.json"
...
jq_eq "$SUC_OUT/validation_report.json" '.task' "10.7A-2 Credentialed Canonical Read-Only Observation" || { echo "  success: validation_report task must be 10.7A-2" >&2; SUC_OK=1; }
jq_eq "$SUC_OUT/validation_report.json" '.report_kind' "actual_database_observation" || { echo "  success: validation_report kind must be actual_database_observation" >&2; SUC_OK=1; }
jq_true "$SUC_OUT/validation_report.json" '.transaction_read_only_verified == true' || { echo "  success: validation_report txn_read_only must be true" >&2; SUC_OK=1; }
...
# ---------------------------------------------------------------------------
# Write validation_report.tooling.json
# ---------------------------------------------------------------------------
...
report = {
    ...
    "report_kind": "tooling_harness",
    "report_scope": "4-mode mock harness validation of the runner/builder tooling. Actual observation results are recorded separately by the runner in validation_report.json (task 10.7A-2).",
...
with open(os.path.join(outdir, "validation_report.tooling.json"), "w") as fh:
    json.dump(report, fh, indent=2)
    fh.write("\n")
print("validation_report.tooling.json written to", os.path.join(outdir, "validation_report.tooling.json"))
PYEOF
