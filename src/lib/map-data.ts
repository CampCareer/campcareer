...
import { NZ_OCCUPATIONS, NZ_SHORTAGE_BY_REGION, NZ_HIGH_PAY_BY_REGION, NZ_UNIVERSITIES, NZ_CITIES, NZ_REGIONS, type NZOccupation, type NZRegion, type NZUniversity, type NZCity as NZCityData } from "@/data/nz-map-data"
import { NO_OCCUPATIONS, NO_REGIONS, NO_SHORTAGE_BY_REGION, NO_HIGH_PAY_BY_REGION, NO_UNIVERSITIES, NO_CITIES, type NOOccupation, type NORegion, type NOUniversity, type NOCity as NOCityData } from "@/data/no-map-data"
import { SE_OCCUPATIONS, SE_REGIONS, SE_SHORTAGE_BY_REGION, SE_HIGH_PAY_BY_REGION, SE_UNIVERSITIES, SE_CITIES, type SEOccupation, type SERegion, type SEUniversity, type SECity as SECityData } from "@/data/se-map-data"
import { DK_OCCUPATIONS, DK_REGIONS, DK_SHORTAGE_BY_REGION, DK_HIGH_PAY_BY_REGION, DK_UNIVERSITIES, DK_CITIES, type DKOccupation, type DKRegion, type DKUniversity, type DKCity as DKCityData } from "@/data/dk-map-data"
import { FI_OCCUPATIONS, FI_REGIONS, FI_SHORTAGE_BY_REGION, FI_HIGH_PAY_BY_REGION, FI_UNIVERSITIES, FI_CITIES, type FIOccupation, type FIRegion, type FIUniversity, type FICity as FICityData } from "@/data/fi-map-data"
import { CH_OCCUPATION_GROUPS, CH_REGIONS, CH_SHORTAGE_BY_CANTON, CH_HIGH_PAY_NATIONAL, CH_UNIVERSITIES, CH_CITIES, type CHOccupationGroup, type CHRegion, type CHUniversity, type CHCity } from "@/data/ch-map-data"
import { AE_EMIRATES, AE_SHORTAGE_OCCUPATIONS, AE_HIGH_INCOME_OCCUPATIONS, type UAEEmirate } from "@/data/ae-map-data"
...
export interface MapData {
  ...
  nzRegions: NZRegion[]
  nzOccupations: NZOccupation[]
  nzHighPayByRegion: Record<string, NZOccupation[]>
  nzShortageByRegion: Record<string, NZOccupation[]>
  nzUniversities: NZUniversity[]
  nzCities: NZCityData[]
  noOccupations: NOOccupation[]
  noRegions: NORegion[]
  noHighPayByRegion: Record<string, NOOccupation[]>
  noShortageByRegion: Record<string, NOOccupation[]>
  noUniversities: NOUniversity[]
  noCities: NOCityData[]
  seRegions: SERegion[]
  seOccupations: SEOccupation[]
  seHighPayByRegion: Record<string, SEOccupation[]>
  seShortageByRegion: Record<string, SEOccupation[]>
  seUniversities: SEUniversity[]
  seCities: SECityData[]
  dkRegions: DKRegion[]
  dkOccupations: DKOccupation[]
  dkHighPayByRegion: Record<string, DKOccupation[]>
  dkShortageByRegion: Record<string, DKOccupation[]>
  dkUniversities: DKUniversity[]
  dkCities: DKCityData[]
  fiRegions: FIRegion[]
  fiOccupations: FIOccupation[]
  fiHighPayByRegion: Record<string, FIOccupation[]>
  fiShortageByRegion: Record<string, FIOccupation[]>
  fiUniversities: FIUniversity[]
  fiCities: FICityData[]
  chRegions: CHRegion[]
  chOccupationGroups: CHOccupationGroup[]
  chShortageByCanton: Record<string, CHOccupationGroup[]>
  chHighPayNational: CHOccupationGroup[]
  chUniversities: CHUniversity[]
  chCities: CHCity[]
  aeEmirates: UAEEmirate[]
  aeShortageOccupations: typeof AE_SHORTAGE_OCCUPATIONS
  aeHighIncomeOccupations: typeof AE_HIGH_INCOME_OCCUPATIONS
}
