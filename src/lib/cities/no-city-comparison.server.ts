...
import { SUPPORTED_NO_CITY_SLUGS } from "@/lib/cities/city-routes"
import { getNoCityProfile, type NoCityProfile } from "@/lib/cities/no-city-profile.server"

export type NoCityCompareOption = { slug: string; name: string; regionName: string }
export type NoCityComparison = { left: NoCityProfile; right: NoCityProfile; options: NoCityCompareOption[] }
...
function readyProfile(profile: NoCityProfile) {
  return Boolean(
    profile.population &&
    profile.livingCost &&
    profile.transport &&
...
async function loadCompareReadyNoCities(): Promise<NoCityProfile[]> {
  const [{ data: cityData, error: cityError }, { data: metricData, error: metricError }] = await Promise.all([
    supabaseAdmin
      ...
      .in("slug", [...SUPPORTED_NO_CITY_SLUGS])
  ...
  if (cityError) throw new Error(`Unable to load Norway compare cities: ${cityError.message}`)
  if (metricError) throw new Error(`Unable to load Norway compare metrics: ${metricError.message}`)
  ...
  return (
    await Promise.all(
      SUPPORTED_NO_CITY_SLUGS.filter((slug) => ready.has(slug)).map((slug) => getNoCityProfile(slug)),
    )
  ).filter((profile): profile is NoCityProfile => Boolean(profile && readyProfile(profile)))
}

export const getCompareReadyNoCities = cache(loadCompareReadyNoCities)

function choosePair(
  profiles: readonly NoCityProfile[],
  ...
  const left = bySlug.get(normalizeSlug(requestedLeft)) ?? bySlug.get("oslo") ?? profiles[0]
  if (!left) return null
  ...
  const defaultRight = left.slug === "oslo" ? bySlug.get("trondheim") : bySlug.get("oslo")
...
async function loadNoCityComparison(
  left?: string | null,
  right?: string | null,
): Promise<NoCityComparison | null> {
  const profiles = await getCompareReadyNoCities()
...
export async function getNoCityComparison(left?: string | null, right?: string | null) {
  return loadNoCityComparison(left, right)
}
