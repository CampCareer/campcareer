...
import { isSupportedFiCitySlug, normalizeCitySlug } from "@/lib/cities/city-routes"
...
export type FiCityProfile = {
  ...
  livingCost: { low: number; high: number; currency: string; period: string; citySpecific: boolean; note: string | null } | null
...
async function loadFiCityProfile(input: string): Promise<FiCityProfile | null> {
  ...
  const { data: cityData, error: cityError } = await supabaseAdmin
    ...
    .eq("slug", slug)
    .maybeSingle()
  if (cityError) throw new Error(`Unable to load Finland city: ${cityError.message}`)
  if (!cityData) return null
  const city = cityData as CityRow

  const [institutionResult, programmeResult, metricResult] = await Promise.all([
    supabaseAdmin.from("city_institution_directory_fi_v1")
    ...
    supabaseAdmin.from("city_programme_directory_fi_v1")
  ...
  if (institutionResult.error) throw new Error(`Unable to load Finland city institutions: ${institutionResult.error.message}`)
  if (programmeResult.error) throw new Error(`Unable to load Finland city programmes: ${programmeResult.error.message}`)
  if (metricResult.error) throw new Error(`Unable to load Finland city metrics: ${metricResult.error.message}`)

  const institutionMap = new Map<string, FiCityProfile["institutions"][number]>()
  for (const row of (institutionResult.data ?? []) as InstitutionRow[]) {
    ...
    if (existing) existing.locations.push(location)
      ...
      authorityIdentifierSystem: row.authority_identifier_system, authorityIdentifier: row.authority_identifier,
      authoritySourceUrl: row.authority_source_url, identifierMaturity: row.identifier_maturity, locations: [location],
    })
  }
  ...
  return {
    id: city.city_id, slug: city.slug, name: city.name, countryCode: "FI", countryName: "Finland",
    ...
    institutionCoverageStatus: city.institution_coverage_status, institutionIdentifierMaturity: city.institution_identifier_maturity,
    programmeCoverage: verifiedPartial ? {
      status: "verified_partial",
      label: `${city.linked_program_count} programmes with verified city evidence`,
      detail: "Each published programme requires an exact FI_OFFICIAL source key, the same canonical university, and a source city matching this verified municipality study location. The current catalogue covers a selected university core rather than every recognised Finnish HEI or UAS programme in the municipality.",
    } : {
    ...
    livingCost: livingRow && low !== null && high !== null ? { low, high, currency: stringValue(living.currency) ?? "EUR", period: stringValue(living.period) ?? "month", citySpecific: living.city_specific === true, note: stringValue(living.note) } : null,
    transport: transportRow && fare !== null ? { amount: fare, currency: stringValue(transport.currency) ?? "EUR", period: stringValue(transport.period) ?? "source_native", referenceKind: stringValue(transport.reference_kind), note: stringValue(transport.note) } : null,
    workRights: workRow && hours !== null ? { hoursNormalPeriod: hours, period: stringValue(work.period) ?? "week_average", note: stringValue(work.note) } : null,
    ...
    programmeSample: ((programmeResult.data ?? []) as ProgrammeRow[]).map((row) => ({ id: row.programme_id, title: row.programme_title, type: row.programme_type, fieldName: row.field_name, sourceDegreeLevel: row.source_degree_level, institutionName: row.institution_name, institutionSlug: row.institution_slug, locationName: row.campus_name, enrolmentStatus: row.enrolment_status, officialUrl: row.official_program_url, verificationTier: row.verification_tier })),
...
export const getFiCityProfile = cache(loadFiCityProfile)
