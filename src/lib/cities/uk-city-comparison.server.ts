...
import { PUBLISHED_UK_CITY_SLUGS } from "@/lib/cities/city-routes"
import { getUkCityProfile, type UkCityProfile } from "@/lib/cities/uk-city-profile.server"

export type UkCityCompareOption = {
  slug: string
  name: string
  regionName: string
}

export type UkCityComparison = {
  left: UkCityProfile
  right: UkCityProfile
  options: UkCityCompareOption[]
}
...
function isCompareReadyProfile(profile: UkCityProfile) {
  return Boolean(
    profile.population &&
      profile.livingCost &&
      profile.transport &&
...
async function loadCompareReadyUkCities(): Promise<UkCityProfile[]> {
  const [{ data: cityData, error: cityError }, { data: metricData, error: metricError }] = await Promise.all([
    supabaseAdmin
      ...
      .in("slug", [...PUBLISHED_UK_CITY_SLUGS])
  ...
  if (cityError) throw new Error(`Unable to load UK compare cities: ${cityError.message}`)
  if (metricError) throw new Error(`Unable to load UK compare city metrics: ${metricError.message}`)
  ...
  const profiles = (await Promise.all(
    PUBLISHED_UK_CITY_SLUGS.filter((slug) => ready.has(slug)).map((slug) => getUkCityProfile(slug)),
  )).filter((profile): profile is UkCityProfile => Boolean(profile && isCompareReadyProfile(profile)))

  return profiles
}

export const getCompareReadyUkCities = cache(loadCompareReadyUkCities)

function chooseComparisonPair(
  profiles: readonly UkCityProfile[],
  ...
  const left = bySlug.get(leftSlug) ?? bySlug.get("london") ?? profiles[0]
  if (!left) return null

  const requestedRightProfile = rightSlug && rightSlug !== left.slug ? bySlug.get(rightSlug) : undefined
  const defaultRight = left.slug === "london" ? bySlug.get("manchester") : bySlug.get("london")
  const right =
...
async function loadUkCityComparison(
  requestedLeft?: string | null,
  requestedRight?: string | null,
): Promise<UkCityComparison | null> {
  const profiles = await getCompareReadyUkCities()
  ...
  return {
    ...
    options: profiles.map((profile) => ({
      slug: profile.slug,
      name: profile.name,
      regionName: `${profile.educationNation} · ${profile.scopeLabel}`,
    })),
  }
}

export async function getUkCityComparison(
  leftSlug?: string | null,
  rightSlug?: string | null,
): Promise<UkCityComparison | null> {
  return loadUkCityComparison(leftSlug, rightSlug)
}
