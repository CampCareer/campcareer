...
import { isSupportedNoCitySlug, normalizeCitySlug } from "@/lib/cities/city-routes"
...
export type NoCityProfile = {
  ...
  transport: { amount: number; currency: string; period: string; referenceKind: string | null; studentSpecific: boolean; note: string | null } | null
  workRights: { hoursNormalPeriod: number; period: string; fullTimeHolidays: boolean; note: string | null } | null
...
async function loadNoCityProfile(input: string): Promise<NoCityProfile | null> {
  ...
  if (cityError) throw new Error(`Unable to load Norway city: ${cityError.message}`)
  if (!cityData) return null
  const city = cityData as CityRow

  const [institutionResult, programmeResult, metricResult] = await Promise.all([
    supabaseAdmin.from("city_institution_directory_no_v1")
    ...
    supabaseAdmin.from("city_programme_directory_no_v1")
  ...
  if (institutionResult.error) throw new Error(`Unable to load Norway city institutions: ${institutionResult.error.message}`)
  if (programmeResult.error) throw new Error(`Unable to load Norway city programmes: ${programmeResult.error.message}`)
  if (metricResult.error) throw new Error(`Unable to load Norway city metrics: ${metricResult.error.message}`)

  const institutionMap = new Map<string, NoCityProfile["institutions"][number]>()
  ...
  const low = numberValue(living.low)
  const high = numberValue(living.high)

  const transportRow = metrics.get("student_transport_reference")
  const transport = record(transportRow?.value)
  const fare = numberValue(transport.amount)
  ...
  return {
    ...
    programmeCoverage: verifiedPartial ? {
      ...
      detail: "Each published programme requires an exact NO_STUDYINNORWAY source key, the same canonical university, and a source city matching this verified municipality study location. The current institution foundation covers NOKUT's university category rather than Norway's complete approved HEI universe.",
    } : {
    ...
    population: popRow && popAmount !== null ? {
      amount: popAmount,
      geography: stringValue(pop.geography) ?? `${city.name} municipality`,
      dataAsOf: popRow.data_as_of,
    } : null,
    livingCost: livingRow && low !== null && high !== null ? {
      low,
      high,
      currency: stringValue(living.currency) ?? "NOK",
      period: stringValue(living.period) ?? "month",
      citySpecific: living.city_specific === true,
      note: stringValue(living.note),
    } : null,
    transport: transportRow && fare !== null ? {
      amount: fare,
      currency: stringValue(transport.currency) ?? "NOK",
      period: stringValue(transport.period) ?? "source_native",
...
export const getNoCityProfile = cache(loadNoCityProfile)
