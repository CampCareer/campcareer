...
import { PUBLISHED_NZ_CITY_SLUGS } from "@/lib/cities/city-routes"
import { getNzCityProfile, type NzCityProfile } from "@/lib/cities/nz-city-profile.server"

export type NzCityCompareOption = {
  slug: string
  name: string
  regionName: string
}

export type NzCityComparison = {
  left: NzCityProfile
  right: NzCityProfile
  options: NzCityCompareOption[]
}
...
function isCompareReadyProfile(profile: NzCityProfile) {
  return Boolean(
    profile.population &&
      profile.livingCost &&
      profile.transport &&
...
async function loadCompareReadyNzCities(): Promise<NzCityProfile[]> {
  const [{ data: cityData, error: cityError }, { data: metricData, error: metricError }] = await Promise.all([
    supabaseAdmin
      ...
      .in("slug", [...PUBLISHED_NZ_CITY_SLUGS])
  ...
  if (cityError) throw new Error(`Unable to load New Zealand compare cities: ${cityError.message}`)
  if (metricError) throw new Error(`Unable to load New Zealand compare city metrics: ${metricError.message}`)
  ...
  const profiles = (await Promise.all(
    PUBLISHED_NZ_CITY_SLUGS.filter((slug) => ready.has(slug)).map((slug) => getNzCityProfile(slug)),
  )).filter((profile): profile is NzCityProfile => Boolean(profile && isCompareReadyProfile(profile)))

  return profiles
}

export const getCompareReadyNzCities = cache(loadCompareReadyNzCities)

function chooseComparisonPair(
  profiles: readonly NzCityProfile[],
  ...
  const left = bySlug.get(leftSlug) ?? bySlug.get("auckland") ?? profiles[0]
  if (!left) return null

  const requestedRightProfile = rightSlug && rightSlug !== left.slug ? bySlug.get(rightSlug) : undefined
  const defaultRight = left.slug === "auckland" ? bySlug.get("christchurch") : bySlug.get("auckland")
  const right =
...
async function loadNzCityComparison(
  requestedLeft?: string | null,
  requestedRight?: string | null,
): Promise<NzCityComparison | null> {
  const profiles = await getCompareReadyNzCities()
...
export async function getNzCityComparison(
  leftSlug?: string | null,
  rightSlug?: string | null,
): Promise<NzCityComparison | null> {
  return loadNzCityComparison(leftSlug, rightSlug)
}
