...
import { isSupportedJpCitySlug, normalizeCitySlug } from "@/lib/cities/city-routes"
...
export type JpCityProfile = {
  ...
  population: { amount: number; geography: string; dataAsOf: string; preliminary: boolean } | null
  livingCost: {
    low: number
    high: number
    currency: string
  ...
  workRights: { hoursNormalPeriod: number; period: string; permissionRequired: boolean; note: string | null } | null
...
async function loadJpCityProfile(input: string): Promise<JpCityProfile | null> {
  ...
  const { data: cityData, error: cityError } = await supabaseAdmin
    .from("city_directory_jp_v1")
    .select("city_id,slug,name,region_code,region_name,admin_code,study_destination_scope,linked_campus_count,linked_institution_count,linked_program_count,institution_coverage_status,institution_identifier_maturity,programme_coverage_status")
    .eq("slug", slug)
    .maybeSingle()

  if (cityError) throw new Error(`Unable to load Japan city: ${cityError.message}`)
  if (!cityData) return null
  const city = cityData as CityRow

  const [institutionResult, programmeResult, metricResult] = await Promise.all([
    supabaseAdmin
      .from("city_institution_directory_jp_v1")
    ...
    supabaseAdmin
      .from("city_programme_directory_jp_v1")
      .select("programme_id,programme_title,programme_type,field_name,source_degree_level,institution_name,institution_slug,campus_name,enrolment_status,official_program_url,offering_source_url,verification_tier")
      .eq("city_id", city.city_id)
      .order("programme_title", { ascending: true })
      .limit(8),
    supabaseAdmin
      .from("city_metric_directory_jp_v1")
      .select("metric_key,value,source_name,source_url,data_as_of,confidence,evidence_kind")
      .eq("city_id", city.city_id)
      .order("metric_key", { ascending: true }),
  ])

  if (institutionResult.error) throw new Error(`Unable to load Japan city institutions: ${institutionResult.error.message}`)
  if (programmeResult.error) throw new Error(`Unable to load Japan city programmes: ${programmeResult.error.message}`)
  if (metricResult.error) throw new Error(`Unable to load Japan city metrics: ${metricResult.error.message}`)

  const institutionMap = new Map<string, JpCityProfile["institutions"][number]>()
  ...
  return {
    ...
    adminCode: city.admin_code,
    scopeLabel: city.study_destination_scope === "tokyo_23_special_wards_aggregate" ? "Tokyo 23 special wards" : "Official municipality",
    ...
    programmeCoverage: verifiedPartial
      ? {
          ...
          label: `${city.linked_program_count} programmes with conservative city linkage`,
          detail: "Only programme groups whose source record and Phase 3 teaching-location evidence support the same city are counted. The result is deliberately partial and is not a complete citywide programme catalogue.",
        }
      : {
          status: "verification_pending",
          label: "Programme delivery verification pending",
          detail: "Verified institution and teaching-location evidence exists, but programme delivery is not inferred from a university brand, a raw prefecture/city label or an inherited publication campus. Zero here means verification is pending, not that the city has no programmes.",
        },
    population: populationRow && populationAmount !== null ? {
      amount: populationAmount,
      geography: stringValue(populationValue.geography) ?? city.name,
      dataAsOf: populationRow.data_as_of,
      preliminary: populationValue.preliminary === true,
    } : null,
    livingCost: livingRow && livingLow !== null && livingHigh !== null ? {
      low: livingLow,
      high: livingHigh,
      currency: stringValue(livingValue.currency) ?? "JPY",
      period: stringValue(livingValue.period) ?? "month",
      citySpecific: livingValue.city_specific === true,
      rankingSafe: livingValue.ranking_safe === true,
      note: stringValue(livingValue.note),
    } : null,
    transport: transportRow && transportAmount !== null ? {
      amount: transportAmount,
      currency: stringValue(transportValue.currency) ?? "JPY",
      period: stringValue(transportValue.period) ?? "source_native",
      referenceKind: stringValue(transportValue.reference_kind),
      mode: stringValue(transportValue.mode),
    ...
    workRights: workRow && workHours !== null ? {
      ...
      permissionRequired: workValue.permission_required === true,
    ...
    programmeSample: ((programmeResult.data ?? []) as ProgrammeRow[]).map((row) => ({
      ...
      officialUrl: row.official_program_url ?? row.offering_source_url,
...
export const getJpCityProfile = cache(loadJpCityProfile)
