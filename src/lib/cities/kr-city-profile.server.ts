...
import { isSupportedKrCitySlug, normalizeCitySlug } from "@/lib/cities/city-routes"
...
async function loadKrCityProfile(input: string): Promise<KrCityProfile | null> {
  ...
  if (cityError) throw new Error(`Unable to load South Korea city: ${cityError.message}`)
  if (!cityData) return null
  const city = cityData as CityRow

  const [institutionResult, programmeResult, metricResult] = await Promise.all([
    supabaseAdmin
      .from("city_institution_directory_kr_v1")
    ...
    supabaseAdmin
      .from("city_programme_directory_kr_v1")
  ...
  if (institutionResult.error) throw new Error(`Unable to load South Korea city institutions: ${institutionResult.error.message}`)
  if (programmeResult.error) throw new Error(`Unable to load South Korea city programmes: ${programmeResult.error.message}`)
  if (metricResult.error) throw new Error(`Unable to load South Korea city metrics: ${metricResult.error.message}`)

  const institutionMap = new Map<string, KrCityProfile["institutions"][number]>()
  ...
  return {
    ...
    scopeLabel: "MOIS administrative city",
    ...
    programmeCoverage: verifiedPartial
      ? {
          ...
          label: `${city.linked_program_count} programmes with verified city linkage`,
          detail: "Programme rows are shown only when the Study in Korea source city exactly matches this Phase 2 administrative city and the offering is linked to a verified Phase 3 teaching-location representative. The count is deliberately partial, not a complete citywide catalogue.",
        }
      : {
          status: "verification_pending",
          label: "Programme delivery verification pending",
          detail: "Verified institution and teaching-location evidence exists, but programme delivery is not inferred from institution brand or inherited campus links.",
        },
    ...
    livingCost: livingRow && livingLow !== null && livingHigh !== null
      ? {
          low: livingLow,
          high: livingHigh,
          currency: stringValue(livingValue.currency) ?? "KRW",
    ...
    transport: transportRow && transportAmount !== null
      ? {
          ...
          currency: stringValue(transportValue.currency) ?? "KRW",
...
export const getKrCityProfile = cache(loadKrCityProfile)
