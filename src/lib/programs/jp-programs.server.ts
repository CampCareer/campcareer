...
export const JP_PROGRAM_PAGE_SIZE = 20
...
export type JpProgram = {
  ...
  internationalAdmissionStatus: JpProgramRow["international_admission_status"]
  ...
  admissionVerificationStatus: JpProgramRow["admission_verification_status"]
  occupationIds: string[]
}

export type JpProgramDetail = JpProgram & {
  sourceVerificationLabel: string | null
  hasProgrammeAccreditationClaim: boolean
}

export type JpProgramSearchResult = { programs: JpProgram[]; total: number; page: number; pageSize: number; pageCount: number }
...
function mapRow(row: JpProgramRow): JpProgram {
  return {
    id: row.programme_id,
...
const EXPLORER_SELECT = [
  "programme_id","program_slug","title","source_program_name","degree_level","school_faculty","field_category","language_context","verification_tier","collection_status",
  "institution_id","institution_name","institution_slug","city","default_duration_months","official_program_url","international_source_url",
  ...
  "admission_source_url","admission_verification_status","occupation_ids",
].join(",")

export async function searchJpPrograms(filters: ProgramSearchFilters): Promise<JpProgramSearchResult> {
  let query = supabaseAdmin.from("program_explorer_jp_v1").select(EXPLORER_SELECT, { count: "exact" })
  ...
  const from = (filters.page - 1) * JP_PROGRAM_PAGE_SIZE
  const { data, error, count } = await query.range(from, from + JP_PROGRAM_PAGE_SIZE - 1)
  if (error) throw new Error(`Unable to load Japan programs: ${error.message}`)
  const programs = ((data ?? []) as unknown as JpProgramRow[]).map(mapRow)
  const total = count ?? programs.length
  return { programs, total, page: filters.page, pageSize: JP_PROGRAM_PAGE_SIZE, pageCount: total === 0 ? 0 : Math.ceil(total / JP_PROGRAM_PAGE_SIZE) }
}

async function loadJpProgramBySlug(slug: string): Promise<JpProgramDetail | null> {
  const { data, error } = await supabaseAdmin.from("program_detail_jp_v1")
    .select(`${EXPLORER_SELECT},source_verification_label,has_programme_accreditation_claim`)
    .eq("program_slug", slug).maybeSingle()
  if (error) throw new Error(`Unable to load Japan program detail: ${error.message}`)
  if (!data) return null
  const row = data as unknown as JpProgramRow
...
export const getJpProgramBySlug = cache(loadJpProgramBySlug)
