...
function mapRow(row: BeProgramRow): BeProgram { return { id: row.programme_id, slug: row.program_slug, title: row.title, sourceProgramName: row.source_program_name, degreeLevel: row.degree_level, schoolFaculty: row.school_faculty, fieldCategory: row.field_category, languageContext: row.language_context, verificationTier: row.verification_tier, collectionStatus: row.collection_status, institutionId: row.institution_id, institutionName: row.institution_name, institutionSlug: row.institution_slug, city: row.city, durationMonths: numberOrNull(row.default_duration_months), officialProgramUrl: row.official_program_url, internationalSourceUrl: row.international_source_url, internationalStudentsEligible: row.international_students_eligible, internationalAdmissionStatus: row.international_admission_status, languageRequirementContext: row.language_requirement_context, visaContext: row.visa_context, intakeLabel: row.intake_label, intakeStartDate: row.intake_start_date, applicationDeadline: row.application_deadline, admissionSourceUrl: row.admission_source_url, admissionVerificationStatus: row.admission_verification_status, occupationIds: row.occupation_ids ?? [] } }
const EXPLORER_SELECT = ["programme_id","program_slug","title","source_program_name","degree_level","school_faculty","field_category","language_context","verification_tier","collection_status","institution_id","institution_name","institution_slug","city","default_duration_months","official_program_url","international_source_url","international_students_eligible","international_admission_status","language_requirement_context","visa_context","intake_label","intake_start_date","application_deadline","admission_source_url","admission_verification_status","occupation_ids"].join(",")

export async function searchBePrograms(filters: ProgramSearchFilters): Promise<BeProgramSearchResult> {
  let query = supabaseAdmin.from("program_explorer_be_v1").select(EXPLORER_SELECT, { count: "exact" })
  ...
  const from = (filters.page - 1) * BE_PROGRAM_PAGE_SIZE
  const { data, error, count } = await query.range(from, from + BE_PROGRAM_PAGE_SIZE - 1)
  if (error) throw new Error(`Unable to load Belgium programs: ${error.message}`)
  const programs = ((data ?? []) as unknown as BeProgramRow[]).map(mapRow); const total = count ?? programs.length
  return { programs, total, page: filters.page, pageSize: BE_PROGRAM_PAGE_SIZE, pageCount: total === 0 ? 0 : Math.ceil(total / BE_PROGRAM_PAGE_SIZE) }
}
async function loadBeProgramBySlug(slug: string): Promise<BeProgramDetail | null> { const { data, error } = await supabaseAdmin.from("program_detail_be_v1").select(`${EXPLORER_SELECT},source_verification_label,has_programme_accreditation_claim`).eq("program_slug", slug).maybeSingle(); if (error) throw new Error(`Unable to load Belgium program detail: ${error.message}`); if (!data) return null; const row = data as unknown as BeProgramRow; return { ...mapRow(row), sourceVerificationLabel: row.source_verification_label ?? null, hasProgrammeAccreditationClaim: row.has_programme_accreditation_claim === true } }
export const getBeProgramBySlug = cache(loadBeProgramBySlug)
