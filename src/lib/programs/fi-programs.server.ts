...
export const FI_PROGRAM_PAGE_SIZE = 20
...
export type FiProgram = {
  ...
  internationalAdmissionStatus: FiProgramRow["international_admission_status"]
  ...
  admissionVerificationStatus: FiProgramRow["admission_verification_status"]
  occupationIds: string[]
}

export type FiProgramDetail = FiProgram & { sourceVerificationLabel: string | null; hasProgrammeAccreditationClaim: boolean }
export type FiProgramSearchResult = { programs: FiProgram[]; total: number; page: number; pageSize: number; pageCount: number }
...
function mapRow(row: FiProgramRow): FiProgram {
...
export async function searchFiPrograms(filters: ProgramSearchFilters): Promise<FiProgramSearchResult> {
  let query = supabaseAdmin.from("program_explorer_fi_v1").select(EXPLORER_SELECT, { count: "exact" })
  ...
  const from = (filters.page - 1) * FI_PROGRAM_PAGE_SIZE
  const { data, error, count } = await query.range(from, from + FI_PROGRAM_PAGE_SIZE - 1)
  if (error) throw new Error(`Unable to load Finland programs: ${error.message}`)
  const programs = ((data ?? []) as unknown as FiProgramRow[]).map(mapRow)
  const total = count ?? programs.length
  return { programs, total, page: filters.page, pageSize: FI_PROGRAM_PAGE_SIZE, pageCount: total === 0 ? 0 : Math.ceil(total / FI_PROGRAM_PAGE_SIZE) }
}

async function loadFiProgramBySlug(slug: string): Promise<FiProgramDetail | null> {
  const { data, error } = await supabaseAdmin.from("program_detail_fi_v1").select(`${EXPLORER_SELECT},source_verification_label,has_programme_accreditation_claim`).eq("program_slug", slug).maybeSingle()
  if (error) throw new Error(`Unable to load Finland program detail: ${error.message}`)
  if (!data) return null
  const row = data as unknown as FiProgramRow
...
export const getFiProgramBySlug = cache(loadFiProgramBySlug)
