...
export const SE_PROGRAM_PAGE_SIZE = 20
...
export type SeProgram = {
  id: string; slug: string; title: string; sourceProgramName: string | null; degreeLevel: string; schoolFaculty: string | null; fieldCategory: string | null; languageContext: string | null; verificationTier: "A" | "B" | "C"; collectionStatus: string | null; institutionId: string; institutionName: string; institutionSlug: string | null; city: string | null; durationMonths: number | null; officialProgramUrl: string | null; internationalSourceUrl: string | null; internationalStudentsEligible: boolean | null; internationalAdmissionStatus: SeProgramRow["international_admission_status"]; languageRequirementContext: string | null; visaContext: string | null; intakeLabel: string | null; intakeStartDate: string | null; applicationDeadline: string | null; admissionSourceUrl: string | null; admissionVerificationStatus: SeProgramRow["admission_verification_status"]; occupationIds: string[]
}
export type SeProgramDetail = SeProgram & { sourceVerificationLabel: string | null; hasProgrammeAccreditationClaim: boolean }
export type SeProgramSearchResult = { programs: SeProgram[]; total: number; page: number; pageSize: number; pageCount: number }
...
export async function searchSePrograms(filters: ProgramSearchFilters): Promise<SeProgramSearchResult> {
  let query = supabaseAdmin.from("program_explorer_se_v1").select(EXPLORER_SELECT, { count: "exact" })
  ...
  const from = (filters.page - 1) * SE_PROGRAM_PAGE_SIZE
  const { data, error, count } = await query.range(from, from + SE_PROGRAM_PAGE_SIZE - 1)
  if (error) throw new Error(`Unable to load Sweden programs: ${error.message}`)
  const programs = ((data ?? []) as unknown as SeProgramRow[]).map(mapRow); const total = count ?? programs.length
  return { programs, total, page: filters.page, pageSize: SE_PROGRAM_PAGE_SIZE, pageCount: total === 0 ? 0 : Math.ceil(total / SE_PROGRAM_PAGE_SIZE) }
}
async function loadSeProgramBySlug(slug: string): Promise<SeProgramDetail | null> { const { data, error } = await supabaseAdmin.from("program_detail_se_v1").select(`${EXPLORER_SELECT},source_verification_label,has_programme_accreditation_claim`).eq("program_slug", slug).maybeSingle(); if (error) throw new Error(`Unable to load Sweden program detail: ${error.message}`); if (!data) return null; const row = data as unknown as SeProgramRow; return { ...mapRow(row), sourceVerificationLabel: row.source_verification_label ?? null, hasProgrammeAccreditationClaim: row.has_programme_accreditation_claim === true } }
export const getSeProgramBySlug = cache(loadSeProgramBySlug)
