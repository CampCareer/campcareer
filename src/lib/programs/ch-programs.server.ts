...
export const CH_PROGRAM_PAGE_SIZE = 20
...
export type ChProgram = {
  id: string; slug: string; title: string; sourceProgramName: string | null; degreeLevel: string; schoolFaculty: string | null; fieldCategory: string | null; languageContext: string | null; verificationTier: "A" | "B" | "C"; collectionStatus: string | null; institutionId: string; institutionName: string; institutionSlug: string | null; city: string | null; durationMonths: number | null; officialProgramUrl: string | null; internationalSourceUrl: string | null; internationalStudentsEligible: boolean | null; internationalAdmissionStatus: ChProgramRow["international_admission_status"]; languageRequirementContext: string | null; visaContext: string | null; intakeLabel: string | null; intakeStartDate: string | null; applicationDeadline: string | null; admissionSourceUrl: string | null; admissionVerificationStatus: ChProgramRow["admission_verification_status"]; occupationIds: string[]
}
export type ChProgramDetail = ChProgram & { sourceVerificationLabel: string | null; hasProgrammeAccreditationClaim: boolean }
export type ChProgramSearchResult = { programs: ChProgram[]; total: number; page: number; pageSize: number; pageCount: number }
...
export async function searchChPrograms(filters: ProgramSearchFilters): Promise<ChProgramSearchResult> {
  let query = supabaseAdmin.from("program_explorer_ch_v1").select(EXPLORER_SELECT, { count: "exact" })
  ...
  const from = (filters.page - 1) * CH_PROGRAM_PAGE_SIZE
  const { data, error, count } = await query.range(from, from + CH_PROGRAM_PAGE_SIZE - 1)
  if (error) throw new Error(`Unable to load Switzerland programs: ${error.message}`)
  const programs = ((data ?? []) as unknown as ChProgramRow[]).map(mapRow); const total = count ?? programs.length
  return { programs, total, page: filters.page, pageSize: CH_PROGRAM_PAGE_SIZE, pageCount: total === 0 ? 0 : Math.ceil(total / CH_PROGRAM_PAGE_SIZE) }
}
async function loadChProgramBySlug(slug: string): Promise<ChProgramDetail | null> { const { data, error } = await supabaseAdmin.from("program_detail_ch_v1").select(`${EXPLORER_SELECT},source_verification_label,has_programme_accreditation_claim`).eq("program_slug", slug).maybeSingle(); if (error) throw new Error(`Unable to load Switzerland program detail: ${error.message}`); if (!data) return null; const row = data as unknown as ChProgramRow; return { ...mapRow(row), sourceVerificationLabel: row.source_verification_label ?? null, hasProgrammeAccreditationClaim: row.has_programme_accreditation_claim === true } }
export const getChProgramBySlug = cache(loadChProgramBySlug)
