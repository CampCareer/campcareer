...
export const ES_PROGRAM_PAGE_SIZE = 20
...
export async function searchEsPrograms(filters: ProgramSearchFilters): Promise<EsProgramSearchResult> {
  let query = supabaseAdmin.from("program_explorer_es_v1").select(EXPLORER_SELECT, { count: "exact" })
  ...
  const from = (filters.page - 1) * ES_PROGRAM_PAGE_SIZE
  const { data, error, count } = await query.range(from, from + ES_PROGRAM_PAGE_SIZE - 1)
  if (error) throw new Error(`Unable to load Spain programs: ${error.message}`)
  const programs = ((data ?? []) as unknown as EsProgramRow[]).map(mapRow); const total = count ?? programs.length
  return { programs, total, page: filters.page, pageSize: ES_PROGRAM_PAGE_SIZE, pageCount: total === 0 ? 0 : Math.ceil(total / ES_PROGRAM_PAGE_SIZE) }
}
async function loadEsProgramBySlug(slug: string): Promise<EsProgramDetail | null> { const { data, error } = await supabaseAdmin.from("program_detail_es_v1").select(`${EXPLORER_SELECT},source_verification_label,has_programme_accreditation_claim`).eq("program_slug", slug).maybeSingle(); if (error) throw new Error(`Unable to load Spain program detail: ${error.message}`); if (!data) return null; const row = data as unknown as EsProgramRow; return { ...mapRow(row), sourceVerificationLabel: row.source_verification_label ?? null, hasProgrammeAccreditationClaim: row.has_programme_accreditation_claim === true } }
export const getEsProgramBySlug = cache(loadEsProgramBySlug)
