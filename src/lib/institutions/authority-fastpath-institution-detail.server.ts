...
export type AuthorityFastpathCountryCode = "FI" | "NO" | "JP" | "KR"
...
export type AuthorityFastpathInstitutionIdentity = {
  officialName: string
  authoritySourceUrl: string
  identifierSystem: string
}

export type AuthorityFastpathInstitutionDetailResult = {
  institution: InstitutionDetail
  identity: AuthorityFastpathInstitutionIdentity
}
...
export const getAuthorityFastpathInstitutionDetail = cache(async (
  countryCode: AuthorityFastpathCountryCode,
  slug: string,
): Promise<AuthorityFastpathInstitutionDetailResult | null> => {
  const { data, error } = await supabaseAdmin
    .from("institution_detail_authority_fastpath_v1")
    .select([
  ...
  const [identityResult, logoResult] = await Promise.all([
    supabaseAdmin
      .from("institution_identity_authority_fastpath_v1")
