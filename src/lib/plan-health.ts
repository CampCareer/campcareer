...
export type PlanHealthSignal = {
  ...
  title: string
  description: string
  href: "/myplan/applications" | "/myplan/money" | "/myplan/english" | "/myplan/pathway"
  cta: string
}
...
export function buildPlanHealth(input: PlanHealthInput): PlanHealth {
  ...
  if (!signals.length) {
    signals.push({
      id: "on-track",
      severity: "positive",
      title: isKo ? "현재 기록된 계획에는 막힌 부분이 없습니다" : "There are no blockers in your recorded plan",
      description: isKo
        ? "새로운 마감일, 자금 변화 또는 영어 계획이 생기면 여기서 다음 우선순위를 다시 알려드립니다."
        : "When a deadline, funding assumption, or English plan changes, this space will surface the next priority again.",
      href: "/myplan/pathway",
