        ...
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div>
            ...
            <ul className="mt-3 grid grid-cols-2 gap-x-4 gap-y-2.5">
              <FooterLink href="/sources" locale={pathLocale}>{isKo ? "전체 출처" : "All sources"}</FooterLink>
              {sourceLinks.map(([href, ko, en]) => (
                <FooterLink key={href} href={href} locale={pathLocale}>{isKo ? ko : en}</FooterLink>
              ))}
            </ul>
          </div>
