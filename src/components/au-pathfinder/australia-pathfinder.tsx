...
function CategorySelect({ label, value, choices, onChange }: { label: string; value: AuPathfinderCategory | "any"; choices: Choice<AuPathfinderCategory | "any">[]; onChange: (value: AuPathfinderCategory | "any") => void }) {
  return <fieldset><legend className="text-sm font-semibold text-slate-900">{label}</legend><div className="mt-3 grid gap-2 sm:grid-cols-3">{choices.map((choice) => {
    const visual = choice.value === "any" ? null : getStudyCategoryVisual(choice.value)
    const Icon = visual?.Icon
    return <button key={choice.value} type="button" aria-pressed={value === choice.value} onClick={() => onChange(choice.value)} className={`flex items-center gap-2.5 rounded-xl border p-3 text-left transition focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:ring-offset-2 ${value === choice.value ? "border-blue-500 bg-blue-50 shadow-sm" : "border-slate-200 bg-white hover:border-slate-300 hover:bg-slate-50"}`}>
      {Icon && <span className={`grid size-9 shrink-0 place-items-center rounded-lg ${visual!.tone}`}><Icon className="size-4" strokeWidth={2.2} /></span>}
