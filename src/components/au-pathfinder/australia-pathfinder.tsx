...
function PathwayCard({ pathway, rank, locale, isKo, featured }: { pathway: RankedAuPathway; rank: number; locale: "en" | "ko"; isKo: boolean; featured: boolean }) {
  ...
  return <article className={`group relative rounded-2xl border bg-white p-5 shadow-sm transition duration-200 hover:-translate-y-0.5 hover:border-blue-300 hover:shadow-[0_14px_32px_rgba(37,99,235,.12)] ${featured ? "border-blue-300 ring-1 ring-blue-100" : "border-slate-200"}`}>
    <Link href={link} aria-label={`${isKo ? `${label} 경로 자세히 보기` : `Explore the ${label} pathway`}`} className="absolute inset-0 z-0 rounded-2xl focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:ring-offset-2"><span className="sr-only">{isKo ? `${label} 경로 보기` : `View ${label} pathway`}</span></Link>
    <div className="pointer-events-none relative z-10">
      <div className="flex items-start gap-4"><span className={`grid size-11 shrink-0 place-items-center rounded-xl ${tone}`}><Icon className="size-5" strokeWidth={2.2} /></span><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[.14em] text-blue-700">{featured ? (isKo ? "가장 잘 맞는 경로" : "Best current fit") : `${isKo ? "추천" : "Rank"} ${rank}`}</p><h3 className="mt-1 text-lg font-semibold text-slate-950">{label}</h3></div><div className="flex shrink-0 items-start gap-2"><span className="pointer-events-auto"><SavedStudyConceptButton compact concept={{ slug: pathway.concept.slug, label: pathway.concept.label, labelKo: pathway.concept.labelKo, category: pathway.concept.category }} /></span><div className="rounded-xl bg-slate-950 px-2.5 py-1.5 text-right text-white"><p className="text-base font-semibold leading-none">{pathway.score}</p><p className="mt-1 text-[10px] font-medium uppercase tracking-wide text-slate-300">{isKo ? "적합도" : "fit"}</p></div></div></div><p className="mt-2 text-sm leading-6 text-slate-600">{pathway.concept.description}</p></div></div>
