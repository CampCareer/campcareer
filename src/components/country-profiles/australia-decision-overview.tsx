import { COUNTRY_ROI_DATA_META, COUNTRY_ROI_INSIGHTS, type DataConfidence, type DataSource } from "@/data/country-roi-mvp"

const australia = COUNTRY_ROI_INSIGHTS.find((country) => country.code === "AU")
  ...
  return (
    <aside className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        <MiniMetric label="First salary" value={australia.salaries.first} />
        <MiniMetric label="5-year salary" value={australia.salaries.year5} />
        <MiniMetric label="Rent" value={australia.rent} />
        <MiniMetric label="Budget" value={australia.initialBudget} />
      </div>
      <div className="mt-4 rounded-lg bg-slate-50 p-3">
        <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Strong majors</p>
        <div className="mt-2 flex flex-wrap gap-2">
          {australia.bestMajors.map((major) => (
...
export function AustraliaDecisionOverview() {
  ...
  const salaryRows = [
    { label: "After graduation", value: australia.salaries.first },
    { label: "After 3 years", value: australia.salaries.year3 },
    { label: "After 5 years", value: australia.salaries.year5 },
    { label: "After 10 years", value: australia.salaries.year10 },
  ]
  const metricRows = [
    { label: "First salary", value: australia.salaries.first, note: "Market entry estimate" },
    { label: "5-year salary", value: australia.salaries.year5, note: "Mid-career preview" },
    { label: "Monthly rent", value: australia.rent, note: australia.cities },
    { label: "Initial budget", value: australia.initialBudget, note: "Minimum to comfortable runway" },
    { label: "Tax preview", value: australia.tax, note: "Simplified effective rate" },
    { label: "Policy route", value: australia.policy, note: "Verify before applying" },
  ]
    ...
    <section className="mb-12 space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
        <div className="space-y-6">
          <Panel title="Who Australia is best for"><BulletList items={australia.detail.bestFor} /></Panel>
          ...
          <Panel title="Budget and take-home preview">
            <div className="grid gap-3 sm:grid-cols-3">
              {australia.detail.budgetBreakdown.map((item) => <div key={item.label} className="rounded-lg border border-slate-200 bg-slate-50 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">{item.label}</p><p className="mt-2 text-xl font-semibold text-slate-950">{item.value}</p><p className="mt-2 text-sm leading-6 text-slate-600">{item.note}</p></div>)}
            </div>
          </Panel>

          <Panel title="Visa and policy signals">
            <BulletList items={australia.detail.policyHighlights} />
            <a href={sourceHref(australia.sources.policy)} target="_blank" rel="noopener noreferrer" className="mt-5 inline-flex rounded-lg border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-900 transition-colors hover:border-slate-300 hover:bg-slate-50">Official source: {australia.sources.policy.sourceName}</a>
          </Panel>
              ...
              <table className="min-w-[720px] w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-200">
                  {sourceRows.map(({ key, label }) => {
                    const source = australia.sources[key]
        ...
        <aside className="h-fit lg:sticky lg:top-28">
          <Panel title="Risks to check first"><BulletList items={australia.detail.watchouts} /></Panel>
        </aside>
      </div>
    </section>
  )
