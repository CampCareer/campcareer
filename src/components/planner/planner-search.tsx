...
export function PlannerSearch({ isKo, onNavigate, onOpenPath }: PlannerSearchProps) {
  ...
  return <>
    <button type="button" onClick={() => setOpen(true)} className="grid size-8 place-items-center rounded-lg text-slate-400 transition hover:bg-white/10 hover:text-slate-200" title={isKo ? "페이지 검색" : "Search pages"} aria-label={isKo ? "페이지 검색" : "Search pages"}><Search className="size-4" /></button>
    {open && typeof document !== "undefined" && createPortal(<div className="fixed inset-0 z-[100] bg-black/60 p-4 backdrop-blur-[2px] sm:p-6" role="presentation" onMouseDown={() => setOpen(false)}>
      <section role="dialog" aria-modal="true" aria-labelledby="planner-search-title" className="mx-auto mt-[12vh] w-full max-w-2xl overflow-hidden rounded-[1.75rem] border border-white/10 bg-[#1a1a20] shadow-[0_24px_80px_rgba(0,0,0,.5)]" onMouseDown={(event) => event.stopPropagation()}>
        <form onSubmit={submitSearch} className="flex items-center gap-3 border-b border-white/10 px-5 py-4"><Search className="size-5 shrink-0 text-blue-400" /><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder={isKo ? "My Plan에서 페이지 검색" : "Search My Plan pages"} aria-label={isKo ? "My Plan 페이지 검색" : "Search My Plan pages"} className="min-w-0 flex-1 bg-transparent text-base text-slate-200 outline-none placeholder:text-slate-500" /><kbd className="hidden rounded-md border border-white/10 bg-white/5 px-2 py-1 text-[10px] font-semibold text-slate-500 sm:inline">ESC</kbd><button type="button" onClick={() => setOpen(false)} className="grid size-8 shrink-0 place-items-center rounded-lg text-slate-500 transition hover:bg-white/10 hover:text-slate-300" aria-label={isKo ? "검색 닫기" : "Close search"}><X className="size-4" /></button></form>
        ...
        <p id="planner-search-title" className="border-t border-white/10 px-5 py-3 text-[11px] text-slate-500">{isKo ? "페이지를 선택하면 최근 방문에 저장됩니다." : "Choose a page to save it to Recents."}</p>
      </section>
    </div>, document.body)}
  </>
}
...
function SearchResult({ item, onClick }: { item: SearchItem; onClick: () => void }) {
  ...
  return <button type="button" onClick={onClick} className="group flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left transition hover:bg-white/5"><span className="grid size-9 shrink-0 place-items-center rounded-xl bg-white/10 text-slate-400 transition group-hover:bg-blue-600/20 group-hover:text-blue-400"><Icon className="size-4" /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold text-slate-200">{item.label}</span><span className="mt-0.5 block truncate text-xs text-slate-500">{item.description}</span></span><ArrowRight className="size-4 shrink-0 text-slate-600 transition group-hover:translate-x-0.5 group-hover:text-blue-400" /></button>
}
