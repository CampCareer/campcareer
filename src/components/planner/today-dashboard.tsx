  ...
  return <section id="today" aria-labelledby="today-dashboard-title" className="scroll-mt-6 space-y-10 pb-12">
    <header className="px-1 py-3">
      <div className="flex flex-col gap-7 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="min-w-[12rem] border-l border-slate-200 pl-5">
          ...
          <p className="mt-2 text-3xl font-semibold tracking-tight text-slate-950">{readyCount}<span className="ml-1 text-base font-medium text-slate-400">/ 9</span></p>
          <div className="mt-3 flex gap-1" aria-label={isKo ? `핵심 준비 9개 중 ${readyCount}개 완료` : `${readyCount} of 9 key preparations complete`}>{readiness.map((complete, index) => <span key={index} className={cn("h-1.5 flex-1 rounded-full", complete ? "bg-blue-500" : "bg-slate-100")} />)}</div>
          <p className="mt-3 text-xs leading-5 text-slate-400">{isKo ? "9개 핵심 준비를 채우며 경로의 불확실성을 줄여보세요." : "Complete the nine essentials to reduce uncertainty in your path."}</p>
        </div>
      </div>
      <div className="mt-7 flex flex-wrap gap-x-5 gap-y-2 border-t border-slate-200 pt-5 text-sm text-slate-500"><span className="inline-flex items-center gap-2"><BriefcaseBusiness className="size-4 text-blue-600" />{goalProfile.target_occupation_title || (isKo ? "직업 목표를 정해보세요" : "Choose a career direction")}</span><span className="inline-flex items-center gap-2"><GraduationCap className="size-4 text-blue-600" />{goalProfile.target_study_concept_label || (isKo ? "전공 방향을 정해보세요" : "Choose a study direction")}</span><span className="inline-flex items-center gap-2"><CalendarClock className="size-4 text-blue-600" />{goalProfile.target_intake_month ? `${isKo ? "목표 입학" : "Target intake"} · ${formatMonth(goalProfile.target_intake_month, locale)}` : (isKo ? "입학 시기 미정" : "Intake to confirm")}</span></div>
    </header>

    <section className="border-t border-slate-200 pt-8" aria-labelledby="plan-health-title">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div><p className="text-xs font-semibold uppercase tracking-[.14em] text-blue-600">{isKo ? "계획 건강도" : "PLAN HEALTH"}</p><h2 id="plan-health-title" className="mt-1 text-xl font-semibold tracking-tight text-slate-950">{healthStatusCopy(planHealth.status, isKo)}</h2><p className="mt-1 text-sm leading-6 text-slate-500">{isKo ? "입력한 일정·자금·영어·후보 데이터를 바탕으로 다음 우선순위를 먼저 보여드립니다." : "Your saved timeline, funding, English and pathway details shape the priority shown here."}</p></div>
        <div className={cn("flex shrink-0 items-center gap-3", planHealth.status === "on-track" ? "text-emerald-600" : planHealth.status === "attention" ? "text-amber-600" : "text-rose-600")}><span className="text-3xl font-semibold tracking-tight">{planHealth.score}</span><span className="border-l border-current/20 pl-3 text-xs font-semibold uppercase tracking-[.12em]">{healthStatusCopy(planHealth.status, isKo)}</span></div>
      </div>
    ...
    <div className="grid gap-8 border-t border-slate-200 pt-8 xl:grid-cols-[minmax(0,1.35fr)_minmax(18rem,.65fr)] xl:divide-x xl:divide-slate-200">
      <section className="xl:pr-8" aria-labelledby="next-best-move-title">
        <div className="flex items-start gap-4"><span className="grid size-11 shrink-0 place-items-center rounded-2xl bg-blue-600 text-white shadow-[0_10px_20px_rgba(59,130,246,.3)]"><Compass className="size-5" /></span><div className="min-w-0"><p className="text-xs font-semibold uppercase tracking-[.14em] text-blue-600">Next best move · 10 min</p><h2 id="next-best-move-title" className="mt-1 text-xl font-semibold tracking-tight text-slate-950">{nextMove.title}</h2><p className="mt-2 max-w-xl text-sm leading-6 text-slate-500">{nextMove.description}</p></div></div>
      ...
      <section id="my-pathway" className="scroll-mt-6 border-t border-slate-200 pt-8 xl:border-t-0 xl:pl-8 xl:pt-0" aria-labelledby="route-title">
        ...
        <p className="mt-2 text-sm leading-6 text-slate-500">{goalProfile.target_occupation_title || goalProfile.target_study_concept_label || (isKo ? "목표를 설정하면 경로가 여기에 표시됩니다." : "Set a goal to see your route here.")}</p>
        <div className="mt-4 space-y-1">{goalOptions.length ? goalOptions.slice(0, 3).map((option) => <div key={option.id} className="flex min-w-0 items-center gap-3 border-b border-slate-100 py-2.5 last:border-b-0"><span className="grid size-6 shrink-0 place-items-center rounded-lg bg-blue-50 text-xs font-bold text-blue-700">{option.position}</span><div className="min-w-0"><p className="truncate text-sm font-semibold text-slate-700">{option.title}</p><p className="truncate text-xs text-slate-400">{option.field_name || option.provider_name || (isKo ? "저장한 후보" : "Saved option")}</p></div></div>) : <Link href="/au/study" className="flex items-center justify-between border-b border-dashed border-slate-200 py-3 text-sm font-medium text-slate-500 transition hover:border-blue-500/30 hover:text-blue-600"><span>{isKo ? "첫 후보를 저장해 보세요" : "Save your first option"}</span><ArrowRight className="size-4" /></Link>}</div>
      </section>
    </div>
...
function HealthSignalCard({ signal }: { signal: PlanHealthSignal }) {
  ...
  return <Link href={signal.href} className={cn("group border-l-2 py-1 pl-4 transition", isPositive ? "border-emerald-500/60" : isCritical ? "border-rose-500/60" : "border-amber-500/60")}><div className="flex items-start gap-3"><span className={cn("mt-0.5 grid size-8 shrink-0 place-items-center rounded-xl", isPositive ? "bg-emerald-50 text-emerald-600" : isCritical ? "bg-rose-50 text-rose-600" : "bg-amber-50 text-amber-600")}>{isPositive ? <CircleCheck className="size-4" /> : <Circle className="size-4 fill-current" />}</span><div className="min-w-0 flex-1"><p className="text-sm font-semibold text-slate-700">{signal.title}</p><p className="mt-1 text-xs leading-5 text-slate-500">{signal.description}</p><span className="mt-3 inline-flex items-center gap-1 text-xs font-semibold text-slate-600 transition group-hover:gap-1.5">{signal.cta}<ArrowRight className="size-3.5" /></span></div></div></Link>
}
...
function getNextMove({ isKo, hasShortlist, hasEnglishBaseline, hasFundTarget, hasApplicationSchedule, nextDeadline }: { isKo: boolean; hasShortlist: boolean; hasEnglishBaseline: boolean; hasFundTarget: boolean; hasApplicationSchedule: boolean; nextDeadline: { title: string; due_date: string } | null }) {
  ...
  if (!hasEnglishBaseline) return { href: "/planner/english", title: isKo ? "현재 영어 점수를 기록하세요" : "Record your current English score", description: isKo ? "정확한 점수가 아니어도 괜찮습니다. 현재 위치를 기록하면 필요한 준비 기간을 가늠할 수 있어요." : "An estimate is enough. Once your starting point is visible, you can judge the preparation time you need.", cta: isKo ? "영어 계획 열기" : "Open English plan" }
  if (!hasFundTarget) return { href: "/planner/money", title: isKo ? "초기 자금 목표를 입력하세요" : "Set your first funding target", description: isKo ? "완벽한 예산이 아니어도 됩니다. 먼저 목표 금액을 잡으면 부족액과 월별 계획이 보입니다." : "It does not need to be a perfect budget. A first target makes the gap and monthly plan visible.", cta: isKo ? "자금 계획 열기" : "Open money plan" }
  if (!hasApplicationSchedule) return { href: "/planner/applications", title: isKo ? "지원 관련 일정 하나를 추가하세요" : "Add one application date", description: isKo ? "마감일 하나만 잡아도 막연한 계획이 실제 일정으로 바뀝니다." : "One date is enough to turn an abstract plan into a real timeline.", cta: isKo ? "일정 추가하기" : "Add a date" }
  if (nextDeadline) return { href: "/planner/applications", title: isKo ? "다음 지원 일정을 10분 안에 확인하세요" : "Review your next date in ten minutes", description: isKo ? `다음 일정은 “${nextDeadline.title}”입니다. 필요한 문서와 다음 행동을 한 줄로 적어보세요.` : `Your next date is “${nextDeadline.title}”. Write down the document you need and the immediate next action.`, cta: isKo ? "일정 보기" : "View schedule" }
