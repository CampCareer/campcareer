...
export function TodayDashboard({
  ...
  return <section id="today" aria-labelledby="today-dashboard-title" className="scroll-mt-6 space-y-10 pb-12">
    <header className="px-1 py-3">
      <div className="flex flex-col gap-7 lg:flex-row lg:items-start lg:justify-between">
        <div className="max-w-2xl">
          <p className="text-xs font-semibold tracking-[.12em] text-blue-400">MY PLAN · TODAY</p>
          <input id="today-dashboard-title" value={planTitle} onChange={(event) => setPlanTitle(event.target.value.slice(0, 160))} onBlur={() => void savePlanField("plan_title", planTitle)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); event.currentTarget.blur() } if (event.key === "Escape") { setPlanTitle(goalProfile.plan_title); event.currentTarget.blur() } }} placeholder={isKo ? "나의 호주 경로" : "My Australia pathway"} aria-label={isKo ? "플랜 제목 수정" : "Edit plan title"} aria-busy={savingField === "plan_title"} className="-mx-1 mt-3 w-[calc(100%+0.5rem)] cursor-text rounded px-1 text-3xl font-semibold tracking-tight text-white outline-none transition hover:bg-white/5 focus:bg-white/8 sm:text-4xl" />
          <textarea value={strategy} onChange={(event) => setStrategy(event.target.value.slice(0, 500))} onBlur={() => void savePlanField("strategy", strategy)} onKeyDown={(event) => { if (event.key === "Escape") { setStrategy(goalProfile.strategy); event.currentTarget.blur() } if (event.key === "Enter" && (event.metaKey || event.ctrlKey)) { event.preventDefault(); event.currentTarget.blur() } }} placeholder={isKo ? "큰 결정보다 다음 한 걸음에 집중해 계획을 현실로 만들어 보세요." : "Focus on the next useful decision, then turn it into a real plan."} aria-label={isKo ? "전략 수정" : "Edit plan strategy"} aria-busy={savingField === "strategy"} rows={2} className="-mx-1 mt-3 w-[calc(100%+0.5rem)] resize-none cursor-text rounded px-1 text-sm leading-6 text-slate-400 outline-none transition hover:bg-white/5 focus:bg-white/8 sm:text-base" />
        </div>
        <div className="min-w-[12rem] border-l border-white/10 pl-5">
          <p className="text-xs font-semibold uppercase tracking-[.14em] text-slate-500">{isKo ? "준비도" : "Readiness"}</p>
          <p className="mt-2 text-3xl font-semibold tracking-tight text-white">{readyCount}<span className="ml-1 text-base font-medium text-slate-500">/ 9</span></p>
          <div className="mt-3 flex gap-1" aria-label={isKo ? `핵심 준비 9개 중 ${readyCount}개 완료` : `${readyCount} of 9 key preparations complete`}>{readiness.map((complete, index) => <span key={index} className={cn("h-1.5 flex-1 rounded-full", complete ? "bg-blue-500" : "bg-white/10")} />)}</div>
      ...
      <div className="mt-7 flex flex-wrap gap-x-5 gap-y-2 border-t border-white/10 pt-5 text-sm text-slate-400"><span className="inline-flex items-center gap-2"><BriefcaseBusiness className="size-4 text-blue-400" />{goalProfile.target_occupation_title || (isKo ? "직업 목표를 정해보세요" : "Choose a career direction")}</span><span className="inline-flex items-center gap-2"><GraduationCap className="size-4 text-blue-400" />{goalProfile.target_study_concept_label || (isKo ? "전공 방향을 정해보세요" : "Choose a study direction")}</span><span className="inline-flex items-center gap-2"><CalendarClock className="size-4 text-blue-400" />{goalProfile.target_intake_month ? `${isKo ? "목표 입학" : "Target intake"} · ${formatMonth(goalProfile.target_intake_month, locale)}` : (isKo ? "입학 시기 미정" : "Intake to confirm")}</span></div>
    </header>

    <section className="border-t border-white/10 pt-8" aria-labelledby="plan-health-title">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div><p className="text-xs font-semibold uppercase tracking-[.14em] text-blue-400">{isKo ? "계획 건강도" : "PLAN HEALTH"}</p><h2 id="plan-health-title" className="mt-1 text-xl font-semibold tracking-tight text-white">{healthStatusCopy(planHealth.status, isKo)}</h2><p className="mt-1 text-sm leading-6 text-slate-400">{isKo ? "입력한 일정·자금·영어·후보 데이터를 바탕으로 다음 우선순위를 먼저 보여드립니다." : "Your saved timeline, funding, English and pathway details shape the priority shown here."}</p></div>
        <div className={cn("flex shrink-0 items-center gap-3", planHealth.status === "on-track" ? "text-emerald-400" : planHealth.status === "attention" ? "text-amber-400" : "text-rose-400")}><span className="text-3xl font-semibold tracking-tight">{planHealth.score}</span><span className="border-l border-current/20 pl-3 text-xs font-semibold uppercase tracking-[.12em]">{healthStatusCopy(planHealth.status, isKo)}</span></div>
      </div>
    ...
    <div className="grid gap-8 border-t border-white/10 pt-8 xl:grid-cols-[minmax(0,1.35fr)_minmax(18rem,.65fr)] xl:divide-x xl:divide-white/10">
      <section className="xl:pr-8" aria-labelledby="next-best-move-title">
        <div className="flex items-start gap-4"><span className="grid size-11 shrink-0 place-items-center rounded-2xl bg-blue-600 text-white shadow-[0_10px_20px_rgba(59,130,246,.3)]"><Compass className="size-5" /></span><div className="min-w-0"><p className="text-xs font-semibold uppercase tracking-[.14em] text-blue-400">Next best move · 10 min</p><h2 id="next-best-move-title" className="mt-1 text-xl font-semibold tracking-tight text-white">{nextMove.title}</h2><p className="mt-2 max-w-xl text-sm leading-6 text-slate-400">{nextMove.description}</p></div></div>
        <Link href={nextMove.href} className="mt-5 inline-flex min-h-11 items-center gap-2 rounded-xl bg-blue-600 px-4 text-sm font-semibold text-white transition hover:bg-blue-500">{nextMove.cta}<ArrowRight className="size-4" /></Link>
      </section>

      <section id="my-pathway" className="scroll-mt-6 border-t border-white/10 pt-8 xl:border-t-0 xl:pl-8 xl:pt-0" aria-labelledby="route-title">
        <div className="flex items-center gap-2"><Target className="size-4 text-blue-400" /><h2 id="route-title" className="text-sm font-semibold text-white">{isKo ? "현재 경로" : "Current route"}</h2></div>
        <p className="mt-2 text-sm leading-6 text-slate-400">{goalProfile.target_occupation_title || goalProfile.target_study_concept_label || (isKo ? "목표를 설정하면 경로가 여기에 표시됩니다." : "Set a goal to see your route here.")}</p>
        <div className="mt-4 space-y-1">{goalOptions.length ? goalOptions.slice(0, 3).map((option) => <div key={option.id} className="flex min-w-0 items-center gap-3 border-b border-white/5 py-2.5 last:border-b-0"><span className="grid size-6 shrink-0 place-items-center rounded-lg bg-blue-600/20 text-xs font-bold text-blue-300">{option.position}</span><div className="min-w-0"><p className="truncate text-sm font-semibold text-slate-200">{option.title}</p><p className="truncate text-xs text-slate-500">{option.field_name || option.provider_name || (isKo ? "저장한 후보" : "Saved option")}</p></div></div>) : <Link href="/au/study" className="flex items-center justify-between border-b border-dashed border-white/10 py-3 text-sm font-medium text-slate-400 transition hover:border-blue-500/30 hover:text-blue-400"><span>{isKo ? "첫 후보를 저장해 보세요" : "Save your first option"}</span><ArrowRight className="size-4" /></Link>}</div>
      </section>
    </div>

    <section className="grid gap-5 border-t border-white/10 pt-8 sm:grid-cols-3 sm:divide-x sm:divide-white/10 sm:gap-0" aria-label={isKo ? "핵심 준비 현황" : "Key preparation status"}>
    ...
    <section className="border-t border-white/10 pt-8" aria-labelledby="journey-title">
      <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[.14em] text-slate-500">Australia decision journey</p><h2 id="journey-title" className="mt-1 text-xl font-semibold tracking-tight text-white">{isKo ? "한 번에 전부가 아니라, 다음 단계만 선명하게" : "Make the next decision, not every decision at once."}</h2></div><p className="text-sm text-slate-500">{isKo ? `${readyCount}/9 준비 완료` : `${readyCount}/9 essentials ready`}</p></div>
...
function HealthSignalCard({ signal }: { signal: PlanHealthSignal }) {
  ...
  return <Link href={signal.href} className={cn("group border-l-2 py-1 pl-4 transition", isPositive ? "border-emerald-500/60" : isCritical ? "border-rose-500/60" : "border-amber-500/60")}><div className="flex items-start gap-3"><span className={cn("mt-0.5 grid size-8 shrink-0 place-items-center rounded-xl", isPositive ? "bg-emerald-500/15 text-emerald-400" : isCritical ? "bg-rose-500/15 text-rose-400" : "bg-amber-500/15 text-amber-400")}>{isPositive ? <CircleCheck className="size-4" /> : <Circle className="size-4 fill-current" />}</span><div className="min-w-0 flex-1"><p className="text-sm font-semibold text-slate-200">{signal.title}</p><p className="mt-1 text-xs leading-5 text-slate-400">{signal.description}</p><span className="mt-3 inline-flex items-center gap-1 text-xs font-semibold text-slate-300 transition group-hover:gap-1.5">{signal.cta}<ArrowRight className="size-3.5" /></span></div></div></Link>
}
...
function RoiReportCard({ readiness, isKo, onOpenReport }: { readiness: RoiReportReadiness; isKo: boolean; onOpenReport?: () => void }) {
  ...
  return <section className="border-t border-white/10 pt-8" aria-labelledby="roi-report-title">
    <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
      <div className="max-w-2xl"><p className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[.14em] text-violet-400"><FileCheck2 className="size-4" />MY AUSTRALIA ROI DECISION REPORT</p><h2 id="roi-report-title" className="mt-2 text-xl font-semibold tracking-tight text-white">{readiness.ready ? readyTitle : preparingTitle}</h2><p className="mt-2 text-sm leading-6 text-slate-400">{readiness.ready ? (isKo ? "My Plan의 후보, 자금, 영어 목표와 직업 방향을 리포트 초안으로 가져옵니다. 동의를 검토한 뒤에만 저장됩니다." : "Your shortlist, funding, English goals and career direction will be brought into a report draft. Nothing is saved until you review consent.") : (isKo ? `후보·자금·영어·직업의 네 가지 기준을 채우면 개인화 ROI 분석을 바로 준비할 수 있어요.${missing ? ` 다음은 ${labels[missing.id]}입니다.` : ""}` : "Complete the four decision inputs to begin a personalised ROI analysis.")}</p></div>
      {readiness.ready && onOpenReport ? <button type="button" onClick={onOpenReport} className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-xl bg-violet-600 px-4 text-sm font-semibold text-white transition hover:bg-violet-500"><FileCheck2 className="size-4" />{isKo ? "리포트 초안 준비" : "Prepare report draft"}<ArrowRight className="size-4" /></button> : <Link href={readiness.ready ? "/myplan/report" : readiness.nextHref} className={cn("inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-xl px-4 text-sm font-semibold transition", readiness.ready ? "bg-violet-600 text-white hover:bg-violet-500" : "bg-white/10 text-white hover:bg-white/15")}><FileCheck2 className="size-4" />{readiness.ready ? (isKo ? "리포트 초안 준비" : "Prepare report draft") : (isKo ? "다음 조건 채우기" : "Complete next input")}<ArrowRight className="size-4" /></Link>}
    </div>
    <div className="mt-5 grid gap-x-5 gap-y-3 sm:grid-cols-2 lg:grid-cols-4">{readiness.checks.map((check) => <Link key={check.id} href={check.href} className={cn("flex min-h-8 items-center gap-2 text-xs font-semibold transition", check.complete ? "text-emerald-400" : "text-slate-500 hover:text-blue-400")}><span className={cn("grid size-5 shrink-0 place-items-center rounded-full", check.complete ? "bg-emerald-500 text-white" : "bg-white/10 text-slate-500")}>{check.complete ? <Check className="size-3" /> : <Circle className="size-3" />}</span>{labels[check.id]}</Link>)}</div>
  </section>
}
...
function MetricCard({ icon: Icon, tone, label, value, detail, href }: { icon: LucideIcon; tone: "blue" | "emerald" | "violet"; label: string; value: string; detail: string; href: string }) {
  const tones = { blue: "bg-blue-500/15 text-blue-400", emerald: "bg-emerald-500/15 text-emerald-400", violet: "bg-violet-500/15 text-violet-400" }
  return <Link href={href} className="group px-1 py-2 transition hover:text-blue-400 sm:px-5 sm:first:pl-0"><span className={cn("grid size-9 place-items-center rounded-xl", tones[tone])}><Icon className="size-4" /></span><p className="mt-4 text-xs font-semibold uppercase tracking-[.12em] text-slate-500">{label}</p><p className="mt-1 text-lg font-semibold tracking-tight text-white">{value}</p><p className="mt-2 text-xs leading-5 text-slate-500">{detail}</p></Link>
}

function JourneyStep({ index, label, detail, complete }: { index: number; label: string; detail: string; complete: boolean }) {
  return <div className="relative min-h-24 border-l border-white/10 pl-4 first:border-l-0 first:pl-0"><div className="flex items-center justify-between gap-2"><span className={cn("grid size-6 place-items-center rounded-full text-xs font-bold", complete ? "bg-blue-600 text-white" : "bg-white/10 text-slate-500")}>{complete ? <Check className="size-3.5" /> : index}</span>{complete ? <CircleCheck className="size-4 text-blue-500" /> : <Circle className="size-4 text-white/10" />}</div><p className={cn("mt-4 text-sm font-semibold", complete ? "text-blue-300" : "text-slate-300")}>{label}</p><p className="mt-1 text-xs leading-5 text-slate-500">{detail}</p></div>
}
