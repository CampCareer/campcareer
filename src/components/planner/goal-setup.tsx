...
export function GoalSetup({ occupations, studyConcepts, universities, courses, onComplete }: GoalSetupProps) {
  ...
  return <main className="min-h-screen bg-slate-50 px-5 py-7 sm:px-8 sm:py-10">
    <section className="mx-auto max-w-5xl">
      <div className="flex items-center justify-between gap-4"><Link href="/" className="inline-flex items-center gap-2 text-sm font-semibold text-slate-500 transition hover:text-blue-600"><span className="grid size-9 place-items-center rounded-xl bg-blue-600 text-sm font-bold text-white">C</span>CampCareer</Link><p className="text-xs font-semibold uppercase tracking-[.15em] text-slate-500">My Plan · Australia</p></div>

      <div className="mt-10 grid gap-8 lg:grid-cols-[minmax(0,1fr)_18rem] lg:items-start">
        <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-[0_24px_70px_rgba(0,0,0,.4)] backdrop-blur sm:p-9">
          ...
          <div className="mt-8 flex items-center gap-2" aria-label={isKo ? "목표 설정 단계" : "Goal setup steps"}>{steps.map((label, index) => <div key={label} className="flex min-w-0 items-center gap-2"><span className={cn("grid size-7 shrink-0 place-items-center rounded-full text-xs font-bold", index <= step ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-500")}>{index < step ? <Check className="size-3.5" /> : index + 1}</span><span className={cn("text-xs font-semibold", index === step ? "text-slate-950" : "text-slate-500")}>{label}</span>{index < steps.length - 1 && <span className="h-px w-7 bg-slate-100 sm:w-12" />}</div>)}</div>
...
function GoalStep({ isKo, occupations, studyConcepts, categoryId, occupationCode, conceptSlug, onSelectCategory, onSelectOccupation, onSelectConcept }: { isKo: boolean; occupations: GoalOccupation[]; studyConcepts: GoalStudyConcept[]; categoryId: string; occupationCode: string; conceptSlug: string; onSelectCategory: (category: string) => void; onSelectOccupation: (code: string) => void; onSelectConcept: (slug: string) => void }) {
  ...
  return <div className="mt-8 space-y-8">
    <div>
      <div className="flex items-center gap-2"><GraduationCap className="size-5 text-blue-600" /><h2 className="text-xl font-semibold text-slate-950">{isKo ? "먼저 전공을 골라볼까요?" : "Start with a field"}</h2></div>
    ...
    <div>
      <div className="flex items-center gap-2"><BriefcaseBusiness className="size-5 text-blue-600" /><h2 className="text-xl font-semibold text-slate-950">{isKo ? "목표 직업은 선택 사항이에요" : "Target career is optional"}</h2></div>
...
function StrategyStep({ isKo, occupation, studyConcept, selectedOptions, schoolUndecided, intakeMonth, title, strategy, onIntakeChange, onTitleChange, onStrategyChange }: { isKo: boolean; occupation: GoalOccupation | null; studyConcept: GoalStudyConcept | null; selectedOptions: GoalOption[]; schoolUndecided: boolean; intakeMonth: string; title: string; strategy: string; onIntakeChange: (value: string) => void; onTitleChange: (value: string) => void; onStrategyChange: (value: string) => void }) {
  ...
  return <div className="mt-8"><div><h2 className="text-xl font-semibold text-slate-950">{isKo ? "계획의 방향을 적어주세요" : "Give your plan a direction"}</h2><p className="mt-1 text-sm leading-6 text-slate-500">{isKo ? "이 문장은 계획이 어려워질 때마다 다시 볼 기준이 됩니다." : "This is the line you will return to when the path gets complicated."}</p></div><div className="mt-5 rounded-2xl border border-blue-500/20 bg-blue-500/10 p-4 text-sm leading-6 text-blue-200"><p className="font-semibold text-blue-100">{isKo ? "현재 선택" : "Current direction"}</p><p className="mt-1">{direction || (isKo ? "아직 목표 직업 또는 전공을 선택하지 않았어요." : "A career or field has not been selected yet.")}{selectedOptions.length ? ` · ${selectedOptions.length} ${isKo ? "개 후보" : selectedOptions.length === 1 ? "option" : "options"}` : schoolUndecided ? ` · ${isKo ? "학교 미정" : "school undecided"}` : ""}</p></div><div className="mt-6 grid gap-4 sm:grid-cols-2"><label className="text-sm font-semibold text-slate-600">{isKo ? "목표 입학 시기" : "Target intake"}<span className="mt-1 block text-xs font-normal text-slate-500">{isKo ? "연도와 월을 선택하거나 빠른 시작 시기를 눌러보세요." : "Choose a year and month, or use a quick intake."}</span><span className="mt-3 block rounded-2xl border border-blue-500/20 bg-blue-500/10 p-3"><span className="flex items-center justify-between gap-3"><span className="flex items-center gap-2"><span className="grid size-9 place-items-center rounded-xl bg-slate-100 text-blue-600"><CalendarDays className="size-4" /></span><span className="text-sm font-semibold text-slate-700">{isKo ? `${year || currentYear + 1}년 ${Number(month)}월` : `${monthLabel?.label ?? "January"} ${year || currentYear + 1}`}</span></span><span className="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-semibold text-blue-700">{isKo ? "예정" : "Planned"}</span></span><span className="mt-3 grid grid-cols-2 gap-2"><select aria-label={isKo ? "입학 연도" : "Intake year"} value={year || String(currentYear + 1)} onChange={(event) => updateMonth(event.target.value, month)} className="h-11 rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm font-semibold text-slate-700 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"><option value="" disabled>{isKo ? "연도" : "Year"}</option>{years.map((item) => <option key={item} value={item}>{item}{isKo ? "년" : ""}</option>)}</select><select aria-label={isKo ? "입학 월" : "Intake month"} value={month} onChange={(event) => updateMonth(year, event.target.value)} className="h-11 rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm font-semibold text-slate-700 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"><option value="" disabled>{isKo ? "월" : "Month"}</option>{INTAKE_MONTHS.map((item) => <option key={item.value} value={item.value}>{isKo ? item.labelKo : item.label}</option>)}</select></span><span className="mt-3 flex flex-wrap gap-1.5">{quickMonths.map((quickMonth) => { const quickLabel = INTAKE_MONTHS.find((item) => item.value === quickMonth); return <button key={quickMonth} type="button" onClick={() => updateMonth(year, quickMonth)} className={cn("rounded-full border px-2.5 py-1 text-[11px] font-semibold transition", month === quickMonth ? "border-blue-500 bg-blue-600 text-white" : "border-slate-200 bg-slate-50 text-blue-700 hover:border-blue-500/30 hover:bg-blue-500/10")}>{isKo ? `${quickLabel?.labelKo} 시작` : `${quickLabel?.label} intake`}</button> })}</span></span></label><label className="text-sm font-semibold text-slate-600">{isKo ? "플랜 제목" : "Plan title"}<input value={title} onChange={(event) => onTitleChange(event.target.value.slice(0, 160))} maxLength={160} className="mt-3 h-12 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm font-medium text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20" /></label></div><label className="mt-5 block text-sm font-semibold text-slate-600">{isKo ? "나의 전략" : "My strategy"}<span className="mt-1 block text-xs font-normal text-slate-500">{isKo ? "예: 비용은 통제하고, 취업 가능성이 높은 간호 루트를 우선한다." : "Example: Keep costs controlled and prioritise a nursing route with stronger employment outcomes."}</span><textarea value={strategy} onChange={(event) => onStrategyChange(event.target.value.slice(0, 500))} maxLength={500} rows={4} placeholder={isKo ? "내가 이 경로를 선택하는 기준을 적어보세요." : "Write the rule that will guide your decisions."} className="mt-2 w-full resize-y rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 text-sm leading-6 text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20" /></label></div>
}

function SelectionCard({ active, onClick, icon, title, detail }: { active: boolean; onClick: () => void; icon: ReactNode; title: string; detail: string }) { return <button type="button" onClick={onClick} aria-pressed={active} className={cn("flex min-h-20 items-start gap-3 rounded-2xl border p-4 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 focus-visible:ring-offset-[#1a1a20]", active ? "border-blue-500 bg-blue-500/15 shadow-[0_8px_18px_rgba(59,130,246,.12)]" : "border-slate-200 bg-slate-50 hover:border-blue-500/30 hover:bg-blue-500/10")}><span className={cn("grid size-9 shrink-0 place-items-center rounded-xl", active ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-500")}>{active ? <Check className="size-4" /> : icon}</span><span className="min-w-0"><span className="block truncate text-sm font-semibold text-slate-700">{title}</span><span className="mt-1 block line-clamp-2 text-xs leading-5 text-slate-500">{detail}</span></span></button> }
function EmptySavedState({ isKo, href, label }: { isKo: boolean; href: string; label: string }) { return <div className="mt-4 rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-5 text-sm leading-6 text-slate-500"><p>{isKo ? "아직 저장한 직업이 없습니다. 직업은 나중에 선택해도 괜찮아요." : "No saved careers yet. You can choose a career later as your direction becomes clearer."}</p><Link href={href} className="mt-3 inline-flex items-center gap-1.5 font-semibold text-blue-600 hover:text-blue-700">{label}<ArrowRight className="size-4" /></Link></div> }
