...
export function PlannerToolbar({
  ...
  return (
    <header className="relative z-50 flex h-12 shrink-0 items-center gap-1 border-b border-white/10 bg-[#141418]/80 px-2.5 backdrop-blur-sm">
      ...
      <div role="tablist" aria-label="My Plan pages" className="flex min-w-0 flex-1 items-center gap-1 overflow-x-auto px-1 no-scrollbar">
        {tabs.map((tab) => <div key={tab.id} className="group relative max-w-44 shrink-0">
          <button type="button" role="tab" aria-selected={tab.id === activeTabId} onClick={() => onSelectTab(tab.id)} className={cn("relative block w-full truncate py-1.5 pl-2.5 pr-8 text-left text-sm transition", tab.id === activeTabId ? "font-semibold text-white" : "text-slate-400 hover:text-slate-200")}>
            {plannerTabTitle(tab)}
            {tab.id === activeTabId && <span aria-hidden className="absolute inset-x-2.5 bottom-0 h-0.5 rounded-full bg-blue-500" />}
          </button>
          <button type="button" onClick={() => onCloseTab(tab.id)} className="absolute right-1 top-1/2 grid size-6 -translate-y-1/2 place-items-center rounded-md text-slate-500 opacity-0 transition hover:bg-white/10 hover:text-slate-300 focus:opacity-100 group-hover:opacity-100" title={`Close ${plannerTabTitle(tab)}`} aria-label={`Close ${plannerTabTitle(tab)}`}><X className="size-3.5" /></button>
        </div>)}
        <button type="button" onClick={onNewTab} className="inline-flex size-7 shrink-0 items-center justify-center rounded-md text-slate-500 transition hover:bg-white/10 hover:text-slate-300" title="New page" aria-label="New page"><Plus className="size-4" /></button>
      </div>
