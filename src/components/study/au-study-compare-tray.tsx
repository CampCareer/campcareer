...
function buildStudyUrl(filters: CompareFilters, selected: CompareOption[]) {
  ...
  return `/au/study${query.size ? `?${query}` : ''}`
}
...
export function AuStudyCompareTrayProvider({ initialSelected, filters, children }: { initialSelected: CompareOption[]; filters: CompareFilters; children: ReactNode }) {
  ...
  const updateSelection = useCallback((next: CompareOption[]) => {
    setSelected(next)
    setNotice('')
    router.replace(buildStudyUrl(filters, next), { scroll: false })
  }, [filters, router])
  ...
  const toggle = useCallback((option: CompareOption) => {
    ...
    if (selected.length >= 3) {
      setNotice('You can compare up to three universities at once.')
      return
    }
  ...
  const compareNow = useCallback(() => {
    ...
    router.push(`/au/study/compare?${query}`)
  }, [filters.field, filters.level, router, selected])
  ...
  return <CompareContext.Provider value={{ selected, toggle }}>
    ...
    {selected.length > 0 && <div className="fixed inset-x-3 bottom-3 z-40 mx-auto max-w-5xl rounded-2xl border border-slate-200 bg-white/95 p-3 shadow-[0_20px_60px_rgba(15,23,42,.20)] backdrop-blur sm:bottom-5 sm:p-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-semibold text-slate-950">Compare tray <span className="font-medium text-slate-500">{selected.length}/3</span></p>{selected[0]?.fieldName && <p className="mt-0.5 text-xs text-slate-500">Same subject · {selected[0].fieldName}{selected[0].aqfLevel ? ` · AQF ${selected[0].aqfLevel}` : ''}</p>}</div>{notice && <p aria-live="polite" className="text-xs font-medium text-amber-700">{notice}</p>}</div><div className="mt-2 flex flex-wrap gap-2">{selected.map((item) => <span key={item.id} className="inline-flex max-w-full items-center gap-1.5 rounded-full bg-blue-50 py-1 pl-2.5 pr-1 text-xs font-semibold text-blue-900"><span className="truncate">{item.name}{item.state ? ` · ${item.state}` : ''}</span><button type="button" onClick={() => toggle(item)} aria-label={`Remove ${item.name} from comparison`} className="grid size-5 shrink-0 place-items-center rounded-full text-blue-700 transition hover:bg-blue-100"><X className="size-3.5" /></button></span>)}</div></div>
        <button type="button" onClick={compareNow} disabled={selected.length < 2} className="inline-flex h-11 shrink-0 items-center justify-center gap-2 rounded-xl bg-slate-950 px-4 text-sm font-semibold text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-500"><BarChart3 className="size-4" />{selected.length < 2 ? 'Choose 1 more' : `Compare ${selected.length} schools`}</button>
      </div>
    </div>}
...
export function AuStudyCompareToggle({ option }: { option: CompareOption }) {
  ...
  return <button type="button" onClick={() => context.toggle(option)} className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-xs font-semibold transition ${selected ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-blue-50 text-blue-800 hover:bg-blue-100'}`}><span className="grid size-4 place-items-center rounded-full border border-current/40">{selected ? <Check className="size-3" /> : <Plus className="size-3" />}</span>{selected ? 'Added to compare' : 'Compare'}</button>
}
