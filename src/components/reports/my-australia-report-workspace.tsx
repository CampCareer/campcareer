...
export function MyAustraliaReportWorkspace() {
  ...
  if (loading) return <main className="min-h-screen bg-slate-50 px-5 py-10"><div className="mx-auto max-w-6xl animate-pulse border-t border-slate-200 pt-8"><div className="h-6 w-48 rounded bg-slate-200" /><div className="mt-5 h-36 bg-slate-100" /></div></main>
  if (!user) return null
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_right,#dbeafe,transparent_30%),linear-gradient(180deg,#f8fbff_0%,#ffffff_44%)]">
      <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-10">
        ...
        <section className="mt-6">
          <div className="border-b border-blue-100 bg-[linear-gradient(125deg,#eff6ff_0%,#ffffff_70%)] px-1 py-6 sm:px-2 sm:py-8">
            <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
              ...
              <div className="border-l-2 border-amber-300 px-4 py-3 text-sm text-amber-950 sm:max-w-xs">
            ...
            {importedFromPlan && <div className="mt-5 flex items-start gap-3 border-l-2 border-violet-300 px-4 py-3 text-sm text-violet-950"><Sparkles className="mt-0.5 size-4 shrink-0 text-violet-700" /><span><strong>{copy.planImported}</strong><span className="mt-1 block text-xs leading-5 text-violet-800">{copy.planImportedDescription}</span></span></div>}
          ...
          <div className="grid gap-8 px-1 py-6 sm:px-2 sm:py-8 lg:grid-cols-[minmax(0,1fr)_18rem]">
            ...
            <aside className="space-y-4 lg:sticky lg:top-24 lg:self-start">
              ...
              <section className="border-t border-amber-200 pt-5"><p className="flex items-center gap-2 text-sm font-semibold text-amber-950"><LockKeyhole className="size-4" />{copy.orderHistory}</p><div className="mt-3 space-y-2">{orders.length ? orders.map((order) => <div key={order.id} className="border-b border-amber-200/70 px-1 py-2 text-xs"><p className="font-medium text-slate-800">{REPORT_PRODUCTS.find((product) => product.id === order.product_id)?.[isKo ? "titleKo" : "title"] ?? order.product_id}</p><p className="mt-1 text-amber-800">{orderStatusLabel(order.status, isKo)}</p></div>) : <p className="text-sm leading-5 text-amber-900">{copy.noOrders}</p>}</div></section>
