...
export async function generateMetadata({ searchParams }: { searchParams: Promise<SearchParams> }): Promise<Metadata> {
  ...
  return {
    title: `${result.guide.target.en} in Australia: study and work sources`,
    description: result.guide.summary.en,
    alternates: { canonical: routeGuideHref(result.guide) },
...
export default async function ResultsPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  if (!guide) return <RouteSearchUnavailable locale="en" query={query} goal={goal} />
  const overview = guide.candidateId ? await getAuRouteOverview(guide.candidateId) : null
  return <RouteGuidePage guide={guide} locale="en" initialQuery={query} goal={goal} initialState={state} initialOverview={overview} />
}
