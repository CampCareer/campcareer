...
async function CitiesCompare({ countryCode, params }: { countryCode: string; params: URLSearchParams }) {
  ...
  if (countryCode === "NO") {
    ...
    return (
      <section className="w-full pb-4" aria-label="Norway cities comparison">
        <ComparePageHeader activeType="city" countryCode={countryCode} />
        <NorwayCitiesCompareMatrix left={comparison.left} right={comparison.right} options={comparison.options} />
      </section>
    )
  }
