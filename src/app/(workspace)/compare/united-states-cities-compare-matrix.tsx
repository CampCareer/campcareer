...
function living(profile: UsCityProfile) {
...
function transport(profile: UsCityProfile) {
...
function Header({ city }: { city: UsCityProfile }) {
  return (
    <div className="min-w-0 px-3 py-4 sm:px-4 md:px-5 md:py-5">
      ...
      <Link href={`/cities/us/${city.slug}`} className="mt-2 inline-flex items-center gap-1 text-[11px] font-semibold text-[#2563eb] hover:underline">
...
export function UnitedStatesCitiesCompareMatrix({ left, right, options }: Props) {
  ...
  const lowerLiving =
    leftMid == null || rightMid == null
      ...
      : Math.abs(leftMid - rightMid) < 1
        ...
        : `${leftMid < rightMid ? left.name : right.name} has the lower current midpoint`

  return (
    <div className="w-full">
      <CityCompareSelector options={options} leftSlug={left.slug} rightSlug={right.slug} countryCode="US" />

      <header className="mt-4 rounded-2xl border border-[#d9e3f7] bg-gradient-to-br from-[#f7f9fe] via-white to-[#f7faf5] p-5 sm:p-8">
        <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-[#2563eb]">United States city comparison</p>
        ...
        <p className="mt-3 max-w-3xl text-[13px] leading-6 text-[#6f6d68]">
          Compare approved named-city student living, transport, F-1 work context, canonical institutions and local career sectors. Metro areas and neighbouring municipalities are not silently merged into these city scopes.
        </p>
        <div className="mt-5 flex flex-col gap-2 sm:flex-row">
          <Link href={`/cities/us/${left.slug}`} className="inline-flex items-center justify-center gap-1.5 rounded-lg bg-[#2563eb] px-3.5 py-2.5 text-[11.5px] font-semibold text-white">View {left.name} <ArrowRight className="size-3.5" /></Link>
          <Link href={`/cities/us/${right.slug}`} className="inline-flex items-center justify-center gap-1.5 rounded-lg bg-[#3e7a2e] px-3.5 py-2.5 text-[11.5px] font-semibold text-white">View {right.name} <ArrowRight className="size-3.5" /></Link>
        </div>
      </header>
      ...
      <section className="mt-5 overflow-hidden rounded-2xl border border-[#e7e6e3] bg-white">
        ...
        <Row icon={<Wallet className="size-4" />} label="Student living" note="Indicative monthly USD references from official university budgets; source scenarios differ." left={living(left)} right={living(right)} leftName={left.name} rightName={right.name} />
        <Row icon={<TrainFront className="size-4" />} label="Student transport" note="Source-native periods are preserved. A 7-day cap, semester pass and monthly pass are not treated as equivalent products." left={transport(left)} right={transport(right)} leftName={left.name} rightName={right.name} />
        <Row icon={<Clock3 className="size-4" />} label="F-1 student work" note="20 hours/week is the on-campus academic-session limit. Off-campus employment requires separate authorization." left={left.workRights ? `${left.workRights.hours} h / week on-campus` : "—"} right={right.workRights ? `${right.workRights.hours} h / week on-campus` : "—"} leftName={left.name} rightName={right.name} />
        <Row icon={<Building2 className="size-4" />} label="Canonical institutions" left={left.linkedInstitutionCount.toLocaleString("en-US")} right={right.linkedInstitutionCount.toLocaleString("en-US")} leftName={left.name} rightName={right.name} />
        <Row icon={<MapPin className="size-4" />} label="Canonical campus locations" left={left.linkedCampusCount.toLocaleString("en-US")} right={right.linkedCampusCount.toLocaleString("en-US")} leftName={left.name} rightName={right.name} />
        <Row icon={<Users className="size-4" />} label="City population" note="Named-city population, not metropolitan-area population." left={left.population ? compact(left.population.amount) : "—"} right={right.population ? compact(right.population.amount) : "—"} leftName={left.name} rightName={right.name} />
        <Row icon={<BriefcaseBusiness className="size-4" />} label="Career context" note="Official economic-development focus sectors, not shortage rankings." left={left.employmentSectors.slice(0, 5).join(" · ")} right={right.employmentSectors.slice(0, 5).join(" · ")} leftName={left.name} rightName={right.name} />
      </section>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        <article className="rounded-xl border border-[#e7e6e3] bg-white p-5">
          ...
          <p className="mt-2 text-[11.5px] leading-5 text-[#77746e]">Use this as a directional reference, not a guaranteed personal budget.</p>
        </article>
        <article className="rounded-xl border border-[#eadfca] bg-[#fffaf1] p-5">
          ...
          <p className="mt-2 text-[18px] font-semibold leading-6 text-[#1b1b1b]">Canonical U.S. programme catalogue pending</p>
          <p className="mt-2 text-[11.5px] leading-5 text-[#7a5a31]">CampCareer does not infer programme delivery from institution presence. A missing catalogue is not presented as “0 programmes”.</p>
        </article>
      </div>
    </div>
  )
