  ...
  return (
    <div className="grid grid-cols-2 border-t border-[#ecebe7] md:grid-cols-[220px_minmax(0,1fr)_minmax(0,1fr)]">
      ...
      <div className="min-w-0 border-t border-[#f0efec] px-3 py-3.5 text-[13px] font-semibold leading-5 text-[#1b1b1b] sm:px-4 md:border-l md:border-t-0 md:px-5 md:py-4 md:text-[14px]">
        <span className="mb-1 block text-[9.5px] font-semibold uppercase tracking-[0.08em] text-[#9a978f] md:hidden">
          {leftName}
        </span>
...
function CityHeader({ city }: { city: CaCityProfile }) {
  return (
    <div className="min-w-0 px-3 py-4 sm:px-4 md:px-5 md:py-5">
      <p className="truncate text-[9.5px] font-semibold uppercase tracking-[0.08em] text-[#8f8c85] sm:text-[10.5px] sm:tracking-[0.1em]">
        {city.regionName}
      </p>
      <h2 className="mt-1 flex min-w-0 items-start gap-1.5 text-[18px] font-semibold leading-tight tracking-[-0.03em] text-[#1b1b1b] sm:text-[20px] md:text-[24px]">
        ...
        <span className="min-w-0 break-words">{city.name}</span>
      </h2>
      <Link
        href={`/cities/ca/${city.slug}`}
        className="mt-2 inline-flex items-center gap-1 text-[10.5px] font-semibold text-[#2563eb] hover:underline sm:text-[11px]"
      >
...
export function CanadaCitiesCompareMatrix({
  ...
  const livingDifference =
    leftLivingMidpoint !== null && rightLivingMidpoint !== null
      ? leftLivingMidpoint - rightLivingMidpoint
      : null
  const livingSignal =
    livingDifference === null
  ...
  const programmeDifference = left.linkedProgramCount - right.linkedProgramCount
  const programmeSignal =
    programmeDifference === 0
      ? "Current linked programme counts are equal"
      : `${programmeDifference > 0 ? left.name : right.name} currently has more linked programmes`

  const sharedProgrammeSignal =
    sharedProgramCount === 0
      ? "No shared canonical programme IDs in current coverage"
      : `${sharedProgramCount.toLocaleString("en-CA")} shared canonical ${sharedProgramCount === 1 ? "programme" : "programmes"}`
  ...
  return (
    <div className="w-full">
      ...
      <header className="mt-4 rounded-2xl border border-[#dfe6dc] bg-gradient-to-br from-[#f5f9f3] via-white to-[#eef4ff] p-5 sm:p-8">
        ...
        <h2 className="mt-2 text-[30px] font-semibold leading-tight tracking-[-0.035em] text-[#1b1b1b] sm:text-[40px]">
        ...
        <div className="mt-5 flex flex-col gap-2 sm:flex-row sm:flex-wrap">
          <Link
            href={`/cities/ca/${left.slug}`}
            className="inline-flex w-full items-center justify-center gap-1.5 rounded-lg bg-[#2563eb] px-3.5 py-2.5 text-[11.5px] font-semibold text-white sm:w-auto sm:py-2"
          >
            View {left.name} <ArrowRight className="size-3.5" />
          </Link>
          <Link
            href={`/cities/ca/${right.slug}`}
            className="inline-flex w-full items-center justify-center gap-1.5 rounded-lg bg-[#3e7a2e] px-3.5 py-2.5 text-[11.5px] font-semibold text-white sm:w-auto sm:py-2"
          >
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-3">
        <article className="rounded-xl border border-[#e7e6e3] bg-white p-5">
          ...
          <p className="mt-2 text-[11.5px] leading-5 text-[#77746e]">
            Source scenarios differ by institution, so use this as a directional city signal rather than a guaranteed personal budget.
          </p>
        </article>
        <article className="rounded-xl border border-[#e7e6e3] bg-white p-5">
        ...
        <article className="rounded-xl border border-[#e7e6e3] bg-white p-5">
          ...
          <p className="mt-2 text-[18px] font-semibold leading-6 text-[#1b1b1b]">{sharedProgrammeSignal}</p>
          <p className="mt-2 text-[11.5px] leading-5 text-[#77746e]">
            This counts identical canonical programme IDs linked to campus offerings in both cities; zero does not mean the cities lack similar study fields.
          </p>
        </article>
      </div>
      ...
      <section className="mt-5 rounded-xl border border-[#d9e3f7] bg-[#f7f9fe] p-5 sm:p-6">
        ...
        <p className="mt-2 max-w-4xl text-[12px] leading-5 text-[#5e6f91]">
          Use living, transport, local career context and current study coverage to choose the city environment that fits you. The student work-hours row is a national rule, not a reason to prefer one Canadian city over another. Then verify the actual institution, campus and programme before making a study decision.
        </p>
        <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:flex-wrap">
          <Link href={`/cities/ca/${left.slug}`} className="inline-flex w-full items-center justify-center gap-1.5 rounded-lg border border-[#bfcff0] bg-white px-3.5 py-2.5 text-[11.5px] font-semibold text-[#2563eb] sm:w-auto sm:py-2">
            View {left.name} <ArrowRight className="size-3.5" />
          </Link>
          <Link href={`/cities/ca/${right.slug}`} className="inline-flex w-full items-center justify-center gap-1.5 rounded-lg border border-[#cfd9ca] bg-white px-3.5 py-2.5 text-[11.5px] font-semibold text-[#3e7a2e] sm:w-auto sm:py-2">
            View {right.name} <ArrowRight className="size-3.5" />
          </Link>
        </div>
      </section>
