...
export function UnitedStatesCountryDashboard({ metrics }: { metrics: CountryMetrics }) {
  const explorer = getCountryExplorer("US")
  const countryProfile = getCountryProfile("US")
  const profile = UNITED_STATES_OCCUPATION_COUNTRY_PROFILE
  const visas = VISA_CATALOG.filter((visa) => visa.countryCode === "US")
  ...
  const salaryHint = metrics.salaryRange
    ? `National median-to-mean wage benchmark · ranking value ${formatRankingValue(metrics.salaryRange)}`
    : "Verified national wage benchmark coming soon"
  const livingHint = metrics.livingCostRange
    ? "Independent off-campus planning range · monthly · tuition excluded"
    ...
    <div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <MetricCard
          ...
          hint="Study, practical training and work pathways"
          href="/visas?country=US"
        />
        <MetricCard
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-3">
        <section className="min-w-0 rounded-xl border border-[#e7e6e3] bg-white lg:col-span-2">
          <div className="flex items-center gap-2.5 border-b border-[#f0efec] px-5 py-4">
            ...
            <h2 className="text-[14.5px] font-semibold text-[#1b1b1b]">States &amp; cities</h2>
