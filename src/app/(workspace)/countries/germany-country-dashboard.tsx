...
export function GermanyCountryDashboard({ metrics }: { metrics: CountryMetrics }) {
  const explorer = getCountryExplorer("DE")
  const countryProfile = getCountryProfile("DE")
  const profile = GERMANY_OCCUPATION_COUNTRY_PROFILE
  const visas = VISA_CATALOG.filter((visa) => visa.countryCode === "DE")
  if (!explorer || !countryProfile) return null
  const regions = explorer.regions

  const cityCount = regions.reduce((total, region) => total + region.cities.length, 0)
  const salaryHint = metrics.salaryRange
    ? `Destatis national median-to-mean benchmark · median ${formatRankingValue(metrics.salaryRange)}`
    : "Verified salary range coming soon"
  const livingHint = metrics.livingCostRange ? "Visa funding requirement to DAAD planning benchmark · tuition excluded" : "Verified student planning range coming soon"

  return (
    <div>
      <p className="mb-4 max-w-3xl text-[13px] leading-6 text-[#6f6d68]">{profile.introduction}</p>
      <div className="grid gap-4 sm:grid-cols-3">
        <MetricCard icon={<Stamp className="size-4 text-[#6d4fc4]" />} accent="bg-[#f3f0fa]" label="Visa options" value={String(visas.length)} hint="Study, EU Blue Card and skilled-worker pathways" href="/visas?country=DE" />
