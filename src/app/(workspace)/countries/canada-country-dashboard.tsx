...
export function CanadaCountryDashboard({ metrics }: { metrics: CountryMetrics }) {
  const explorer = getCountryExplorer("CA")
  const countryProfile = getCountryProfile("CA")
  const profile = CANADA_OCCUPATION_COUNTRY_PROFILE
  const visas = VISA_CATALOG.filter((visa) => visa.countryCode === "CA")
  ...
  const salaryHint = metrics.salaryRange
    ? `Middle 50% of NOC occupation medians · midpoint ${formatRankingValue(metrics.salaryRange)}`
    : "Verified national range coming soon"
  const livingHint = metrics.livingCostRange
    ? "Government planning range · monthly · tuition and travel excluded"
    ...
    <div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <MetricCard
          ...
          hint="Study, work and economic pathways"
          href="/visas?country=CA"
        />
        <MetricCard
      ...
      <section className="mt-4 rounded-xl border border-[#e7e6e3] bg-white p-5">
        <div className="flex items-center gap-2 text-[#6d4fc4]">
          ...
          <h2 className="text-[14.5px] font-semibold">Major universities and public colleges</h2>
        </div>
