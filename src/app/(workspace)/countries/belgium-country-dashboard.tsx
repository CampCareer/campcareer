...
export function BelgiumCountryDashboard({ metrics }: { metrics: CountryMetrics }) {
  const explorer = getCountryExplorer("BE")
  const countryProfile = getCountryProfile("BE")
  const profile = BELGIUM_OCCUPATION_COUNTRY_PROFILE
  const visas = VISA_CATALOG.filter((visa) => visa.countryCode === "BE")
  ...
  const salaryHint = metrics.salaryRange
    ? `Statbel full-time gross P10–P90, annualised · median ${formatRankingValue(metrics.salaryRange)}`
    : "Verified salary range coming soon"
  const livingHint = metrics.livingCostRange
    ? "Official regional student-budget guidance · tuition excluded"
    ...
    <div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <MetricCard
          ...
          hint="Study, single-permit and working-holiday pathways"
          href="/visas?country=BE"
        />
        <MetricCard
