...
export function NetherlandsCountryDashboard({ metrics }: { metrics: CountryMetrics }) {
  const explorer = getCountryExplorer("NL")
  const countryProfile = getCountryProfile("NL")
  const profile = NETHERLANDS_OCCUPATION_COUNTRY_PROFILE
  const visas = VISA_CATALOG.filter((visa) => visa.countryCode === "NL")

  if (!explorer || !countryProfile) return null

  const extraRegions = [
    { name: "Groningen", cities: ["Groningen"] },
    { name: "Overijssel", cities: ["Enschede", "Zwolle"] },
    { name: "Limburg", cities: ["Maastricht", "Venlo"] },
  ]
  const regions = [
    ...explorer.regions,
    ...extraRegions.filter((extra) => !explorer.regions.some((region) => region.name === extra.name)),
  ]
  const cityCount = regions.reduce((total, region) => total + region.cities.length, 0)
  const salaryHint = metrics.salaryRange
    ? `Derived full-time averages by employer size · national estimate ${formatRankingValue(metrics.salaryRange)}`
    : "Verified salary range coming soon"
  const livingHint = metrics.livingCostRange
    ? "Study in NL monthly student planning range · tuition excluded"
    ...
    <div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <MetricCard
          ...
          hint="Study, orientation-year and skilled-work pathways"
          href="/visas?country=NL"
        />
        <MetricCard
