...
export function FranceCountryDashboard({ metrics }: { metrics: CountryMetrics }) {
  const explorer = getCountryExplorer("FR")
  const countryProfile = getCountryProfile("FR")
  const profile = FRANCE_OCCUPATION_COUNTRY_PROFILE
  const visas = VISA_CATALOG.filter((visa) => visa.countryCode === "FR")
  ...
  const salaryHint = metrics.salaryRange
    ? `INSEE private-sector net EQTP D1–D9, annualised · median ${formatRankingValue(metrics.salaryRange)}`
    : "Verified salary range coming soon"
  const livingHint = metrics.livingCostRange
    ? "2026 visa resource floor to Paris planning benchmark · tuition excluded"
    ...
    <div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <MetricCard
          ...
          hint="Study, talent-passport and working-holiday pathways"
          href="/visas?country=FR"
        />
        <MetricCard
