  ...
  return (
    <div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <MetricCard icon={<Stamp className="size-4 text-[#6d4fc4]" />} accent="bg-[#f3f0fa]" label="Visa options" value={String(visas.length)} hint="Study, work and skilled pathways" href="/visas?country=AU" />
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        <section className="rounded-xl border border-[#e7e6e3] bg-white p-5">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {profile.academicYear.intakes.map((intake) => <span key={intake} className="rounded-full bg-[#eef4ff] px-3 py-1.5 text-[11px] font-semibold text-[#2563eb]">{intake}</span>)}
          </div>
        </section>
      ...
      <section className="mt-4 rounded-xl border border-[#e7e6e3] bg-white p-5">
        <div className="flex items-center gap-2 text-[#6d4fc4]"><Building2 className="size-4" /><h2 className="text-[14.5px] font-semibold">Major universities and public VET providers</h2></div>
        <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
          {profile.majorInstitutions.map((institution) => (
            <div key={institution.name} className="rounded-lg border border-[#f0efec] bg-[#fafaf8] px-3 py-3">
              ...
              <p className="mt-1 text-[10.5px] text-[#9a978f]">{COUNTRY_INSTITUTION_TYPE_LABELS[institution.type]} · {institution.location}</p>
            </div>
          ))}
        </div>
      </section>
        ...
        <section className="min-w-0 rounded-xl border border-[#e7e6e3] bg-white lg:col-span-2">
          ...
          <div className="grid gap-x-8 gap-y-5 px-5 py-5 sm:grid-cols-2">
            {explorer.regions.map((region) => (
              <div key={region.name}>
                <h3 className="flex items-center gap-1.5 text-[13px] font-semibold text-[#1b1b1b]"><MapPin className="size-3.5 text-[#9c9a94]" />{region.name}</h3>
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {region.cities.map((city) => {
                    const href = CITY_LINKS[city]
                    return href ? (
                      <Link key={city} href={href} className="rounded-md border border-[#bfcff0] bg-[#eef4ff] px-2.5 py-1 text-[12px] font-semibold text-[#2563eb] transition hover:border-[#2563eb] hover:bg-white">{city}</Link>
                    ) : (
                      <span key={city} className="rounded-md border border-[#e7e6e3] bg-[#fafaf8] px-2.5 py-1 text-[12px] font-medium text-[#4d4c48]">{city}</span>
                    )
                  })}
                </div>
              </div>
          ...
          <div className="border-t border-[#f0efec] px-5 py-4">
            <Link href="/cities/au/compare" className="inline-flex items-center gap-1.5 text-[11.5px] font-semibold text-[#3e7a2e] hover:underline">Compare Sydney vs Melbourne →</Link>
          </div>
        </section>
            ...
            <div className="px-5 py-4">
              ...
              <ul className="mt-3 space-y-2.5">
                {countryProfile.workOpportunities.items.map((item) => (
                  <li key={item.title} className="flex items-center gap-2"><span className="size-1.5 shrink-0 rounded-full bg-[#2563eb]" /><span className="text-[12.5px] font-medium text-[#4d4c48]">{item.title}</span></li>
                ))}
              </ul>
            </div>
          )}
