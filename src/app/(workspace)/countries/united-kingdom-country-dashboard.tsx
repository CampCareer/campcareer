...
export function UnitedKingdomCountryDashboard({ metrics }: { metrics: CountryMetrics }) {
  const explorer = getCountryExplorer("UK")
  const countryProfile = getCountryProfile("UK")
  const profile = UNITED_KINGDOM_OCCUPATION_COUNTRY_PROFILE
  const visas = VISA_CATALOG.filter((visa) => visa.countryCode === "UK")
  ...
  const salaryHint = metrics.salaryRange
    ? `Middle 50% of ASHE occupation medians · midpoint ${formatRankingValue(metrics.salaryRange)}`
    : "Verified occupation salary range coming soon"
  const livingHint = metrics.livingCostRange
    ? "UKVI maintenance requirement · London and outside-London range · tuition excluded"
    ...
    <div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <MetricCard
          ...
          hint="Study, graduate and skilled-work pathways"
          href="/visas?country=UK"
        />
        <MetricCard
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-3">
        <section className="min-w-0 rounded-xl border border-[#e7e6e3] bg-white lg:col-span-2">
          <div className="flex items-center gap-2.5 border-b border-[#f0efec] px-5 py-4">
            ...
            <h2 className="text-[14.5px] font-semibold text-[#1b1b1b]">Nations &amp; cities</h2>
