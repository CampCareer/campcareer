...
export function CareerResultPage() {
  ...
  if (!query) return <main className="min-h-[calc(100vh-3.5rem)] bg-campcareer-canvas" />

  return <main className="cc-result-motion min-h-[calc(100vh-3.5rem)] bg-campcareer-canvas px-4 pb-16 pt-5 sm:px-8 sm:pt-8">
    <div className="mx-auto max-w-5xl">
      <Link href={localizePath("/", locale)} className="inline-flex items-center gap-1.5 rounded-md px-2.5 py-2 text-sm font-medium text-campcareer-muted transition-colors hover:bg-white hover:text-brand"><ArrowLeft className="size-4" /> {locale === "ko" ? "다시 검색하기" : "Search again"}</Link>
