...
export function GermanyInstitutionDetailView({ institution }: { institution: InstitutionDetail }) {
  const website = safeWebsiteUrl(institution.websiteUrl)
  const domainSource = safeWebsiteUrl(institution.officialDomainSourceUrl)
  ...
  return (
    <>
      <nav aria-label="Breadcrumb" className="flex flex-wrap items-center gap-1.5 text-[11px] font-medium text-[#8f8c85]">
        ...
        <Link href={countryPath} className="transition hover:text-[#3e7a2e]">Germany</Link><span>/</span>
      ...
      <header className="mt-5 rounded-2xl border border-[#e7e6e3] bg-white p-6 sm:p-7">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
          <div className="flex min-w-0 items-start gap-4">
            ...
            <div className="min-w-0">
              <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-[#3e7a2e]">Germany Tier A university</p>
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2">
                ...
                {institution.officialDomain ? <span className="rounded-full border border-[#e3e2dd] px-3 py-1.5 text-[11px] font-medium text-[#686660]">{institution.officialDomain}</span> : null}
              </div>
            </div>
          </div>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-3">
          ...
          <div className="rounded-xl bg-[#fafaf8] p-4"><Building2 className="size-4 text-[#3e7a2e]" /><p className="mt-2 text-[22px] font-semibold text-[#1b1b1b]">{institution.campusCount.toLocaleString()}</p><p className="mt-0.5 text-[10.5px] font-medium uppercase tracking-[0.06em] text-[#929089]">Verified locations</p></div>
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1.65fr)_minmax(280px,0.8fr)] lg:items-start">
        <main className="min-w-0 space-y-5">
          <section className="rounded-2xl border border-[#e7e6e3] bg-white p-5 sm:p-6">
            ...
            <div className="mt-4 rounded-xl border border-dashed border-[#dcdad4] bg-[#fbfbf9] p-6"><p className="text-[12px] leading-5 text-[#77746e]">CampCareer has not published the Germany program catalogue yet. This does not mean the university offers no programs; institution identity and verified location data are published independently while the program pipeline is prepared.</p></div>
          </section>

          <section className="rounded-2xl border border-[#e7e6e3] bg-white p-5 sm:p-6">
            <div className="flex items-center gap-2"><MapPin className="size-4 text-[#3e7a2e]" /><h2 className="text-[16px] font-semibold text-[#1b1b1b]">Verified location</h2></div>
            <p className="mt-1.5 text-[11.5px] leading-5 text-[#77746e]">The current city is verified against the official DFG Excellence-site cohort. CampCareer does not infer a complete campus inventory, street address or coordinates from this record.</p>
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {institution.campuses.map((location) => <article key={location.id} className="rounded-xl border border-[#e7e6e3] bg-white p-4"><div className="flex items-start gap-3"><span className="grid size-9 shrink-0 place-items-center rounded-lg bg-[#f2f5ef] text-[#3e7a2e]"><MapPin className="size-4" /></span><div><h3 className="text-[13px] font-semibold leading-5 text-[#1b1b1b]">{location.name ?? "Verified institution city"}</h3><p className="mt-1 text-[11.5px] leading-5 text-[#6f6d68]">{location.city ?? location.reportedCity}</p></div></div></article>)}
            </div>
          </section>
        </main>
        ...
        <aside className="space-y-5">
          <section className="rounded-2xl border border-[#e7e6e3] bg-white p-5">
            ...
            <dl className="mt-4 space-y-4">
              <div><dt className="text-[10px] font-semibold uppercase tracking-[0.07em] text-[#aaa7a0]">Country</dt><dd className="mt-1 text-[12.5px] font-medium text-[#4d4c48]">Germany</dd></div>
              <div><dt className="text-[10px] font-semibold uppercase tracking-[0.07em] text-[#aaa7a0]">Verified type</dt><dd className="mt-1 text-[12.5px] font-medium text-[#4d4c48]">University</dd></div>
              {institution.officialDomain ? <div><dt className="text-[10px] font-semibold uppercase tracking-[0.07em] text-[#aaa7a0]">HRK-verified official domain</dt><dd className="mt-1 flex items-center gap-2 text-[12.5px] font-semibold text-[#4d4c48]">{institution.officialDomain}{domainSource ? <a href={domainSource} target="_blank" rel="noreferrer" aria-label="Open official university domain" className="text-[#3e7a2e]"><ExternalLink className="size-3.5" /></a> : null}</dd></div> : null}
            </dl>
          </section>
          <section className="rounded-2xl border border-[#e7e6e3] bg-[#fbfbf9] p-5"><h2 className="text-[12.5px] font-semibold text-[#4d4c48]">About this profile</h2><p className="mt-2 text-[10.5px] leading-5 text-[#8b8881]">Tier A is based on the 2026 DFG decision for Excellence sites continuing from 2027. HRK verifies the institutions as state or state-recognised; CampCareer does not present the domain as a German regulatory ID and leaves ownership unpublished until separately verified.</p></section>
        </aside>
      </div>
    </>
  )
