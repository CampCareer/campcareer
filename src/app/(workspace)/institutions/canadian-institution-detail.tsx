...
function LocationList({
  ...
  locations: InstitutionCampusLocation[]
  total: number
}) {
    ...
    <>
      <div className="grid gap-3 sm:grid-cols-2">
        {programs.map((program) => (
          <ProgramPreviewContent key={program.id} program={program} />
        ))}
      </div>
...
export function CanadianInstitutionDetailView({
  institution,
}: {
      ...
      <header className="mt-5 rounded-2xl border border-[#e7e6e3] bg-white p-6 sm:p-7">
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-3">
          <div className="rounded-xl bg-[#fafaf8] p-4">
            ...
            <p className="mt-2 text-[22px] font-semibold tracking-[-0.02em] text-[#1b1b1b]">{institution.programCount.toLocaleString()}</p>
        ...
        <main className="min-w-0 space-y-5">
          <section className="rounded-2xl border border-[#e7e6e3] bg-white p-5 sm:p-6">
            ...
            <p className="mt-1.5 text-[11.5px] leading-5 text-[#77746e]">
              Active canonical programs connected to this institution. Canadian program detail routes are not invented before that public surface is ready.
            </p>
            <div className="mt-4"><ProgramList programs={institution.programs} total={institution.programCount} /></div>
          </section>
          ...
          <section className="rounded-2xl border border-[#e7e6e3] bg-white p-5 sm:p-6">
            ...
            <p className="mt-1.5 text-[11.5px] leading-5 text-[#77746e]">The largest study areas and qualification types among active canonical programs at this institution.</p>
          ...
          <section className="rounded-2xl border border-[#e7e6e3] bg-white p-5">
            ...
            <dl className="mt-4 space-y-4">
              <div>
                ...
                <dd className="mt-1 text-[12.5px] font-medium text-[#4d4c48]">Canada</dd>
              </div>
              ...
              {institution.cityNames.length ? (
                <div>
                  <dt className="text-[10px] font-semibold uppercase tracking-[0.07em] text-[#aaa7a0]">Location areas</dt>
              ...
              {institution.dliNumber ? (
                <div>
                  ...
                  <dd className="mt-1 flex items-center gap-2 text-[12.5px] font-semibold text-[#4d4c48]">
                    {institution.dliNumber}
                    {dliSource ? (
                      <a href={dliSource} target="_blank" rel="noreferrer" aria-label="Open IRCC DLI source" className="text-[#3e7a2e]">
                        <ExternalLink className="size-3.5" />
                      </a>
                    ) : null}
                  </dd>
