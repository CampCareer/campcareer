...
import type { NoProgramSearchResult } from "@/lib/programs/no-programs.server"
import { noProgramDetailPath } from "@/lib/programs/no-program-seo"
...
function admissionLabel(status: string, verified: string) {
  ...
  return "English-taught pathway verified · current window unknown"
}
...
function Pagination({ filters, result }: { filters: ProgramSearchFilters; result: NoProgramSearchResult }) {
  if (result.pageCount <= 1) return null
  return <nav aria-label="Norway program pages" className="mt-6 flex items-center justify-between rounded-xl border border-[#e7e6e3] bg-white px-4 py-3">
...
export function NoProgramsExplorer({ filters, result }: { filters: ProgramSearchFilters; result: NoProgramSearchResult }) {
  return <section className="mt-7 min-w-0">
    <ProgramsSortControl filters={filters} total={result.total} />
    <div className="mt-3 space-y-3">
      {result.programs.map((program) => <Link key={program.id} href={noProgramDetailPath(program.slug)} className="block rounded-xl border border-[#e7e6e3] bg-white p-5 transition hover:border-[#b9cdb2] hover:shadow-sm">
        <div className="flex flex-wrap items-center gap-2 text-[10.5px] font-semibold">
          ...
          <span className="inline-flex items-center gap-1 rounded-full bg-[#eef4ff] px-2.5 py-1 text-[#2563eb]"><ShieldCheck className="size-3" />HK-dir source verified</span>
    ...
    {result.programs.length === 0 && <div className="mt-4 rounded-xl border border-dashed border-[#dcdad4] bg-[#fbfbf9] p-10 text-center text-[13px] text-[#77746e]">No Norway programs match these filters.</div>}
    <Pagination filters={filters} result={result} />
    <p className="mt-4 text-[10.5px] leading-5 text-[#aaa7a0]">Study in Norway/HK-dir confirms the English-taught programme catalogue. Programme existence, international eligibility and the current application window are tracked separately; institutional accreditation is not presented as a programme-level NOKUT claim.</p>
  </section>
}
