...
export function ProgramCard({ program }: { program: AuProgramListItem }) {
  ...
  return (
    <article className="group relative h-full cursor-pointer rounded-xl border border-[#e6e5e1] bg-white p-4 transition hover:border-[#bfcdb9] hover:shadow-[0_12px_30px_rgba(40,70,30,0.07)] focus-within:border-[#3e7a2e] focus-within:ring-4 focus-within:ring-[#3e7a2e]/15 sm:p-5">
      <Link href={detailHref} aria-label={`View ${program.title} at ${program.institutionName}`} className="absolute inset-0 z-10 rounded-xl outline-none" />
      ...
      <div className="pointer-events-none flex gap-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="min-w-0">
              <div className="flex flex-wrap gap-1.5">
                ...
                {verified && <span className="inline-flex items-center gap-1 rounded-md bg-[#edf5ea] px-2 py-1 text-[10px] font-semibold uppercase tracking-wide text-[#3e7a2e]"><BadgeCheck className="size-3" />Official page verified</span>}
                {locationVerified && <span className="inline-flex items-center gap-1 rounded-md bg-[#eef4ff] px-2 py-1 text-[10px] font-semibold uppercase tracking-wide text-[#2563eb]"><MapPin className="size-3" />CRICOS location verified</span>}
              </div>
              ...
              <p className="mt-1.5 flex items-center gap-1.5 text-[12.5px] font-medium text-[#5e5c57]"><Building2 className="size-3.5 shrink-0 text-[#9b9891]" /><span className="truncate">{program.institutionName}</span></p>
            </div>

            <div className="shrink-0 text-right">
              <p className="text-[17px] font-semibold tracking-[-0.02em] text-[#1b1b1b]">{tuition ?? "Fee unavailable"}</p>
          ...
          <div className="mt-3 flex flex-wrap gap-x-4 gap-y-2 text-[11.5px] text-[#76736d]">
            ...
            {program.aqfLevel != null && <span className="inline-flex items-center gap-1.5"><GraduationCap className="size-3.5 text-[#a3a19b]" />AQF level {program.aqfLevel}</span>}
            {program.cricosCode && <span className="inline-flex items-center gap-1.5"><ShieldCheck className="size-3.5 text-[#a3a19b]" />Provider {program.cricosCode}</span>}
          </div>
          ...
          <div className="mt-4 border-t border-[#efeeea] pt-3">
            <p className="text-[10.5px] font-medium text-[#aaa7a0]">{locationVerified ? "Official CRICOS delivery locations synced" : "Active CRICOS record · location review pending"}</p>
          </div>
        </div>
      </div>
    </article>
