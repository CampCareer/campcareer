...
import { PROGRAM_SORTS, type ProgramSearchFilters } from "@/lib/programs/program-search"
import { useProgramNavigation } from "./programs-navigation"

export function ProgramsSortControl({ filters, total }: { filters: ProgramSearchFilters; total: number }) {
    ...
    <div className="flex flex-wrap items-center justify-between gap-3">
      ...
      <label className="flex items-center gap-2 text-[11.5px] font-medium text-[#8f8c85]">
        ...
        <select value={filters.sort} onChange={(event: ChangeEvent<HTMLSelectElement>) => replace({ sort: event.target.value })} className="h-9 rounded-lg border border-[#deddd8] bg-white px-3 text-[12px] font-medium text-[#4d4c48] outline-none focus:border-[#3e7a2e]">
          {PROGRAM_SORTS.map((sort) => <option key={sort.value} value={sort.value}>{locale === "ko" ? sort.labelKo : sort.label}</option>)}
        </select>
      </label>
    </div>
  )
