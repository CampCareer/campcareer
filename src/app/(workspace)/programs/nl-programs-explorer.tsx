...
function levelLabel(level: string | null, degreeType: string | null) {
  const value = degreeType ?? level
  if (!value) return "Programme"
  return value.toLowerCase().replaceAll("_", " ").replace(/\b\w/g, (letter) => letter.toUpperCase())
}

function Pagination({ filters, result }: { filters: ProgramSearchFilters; result: NlProgramSearchResult }) {
  if (result.pageCount <= 1) return null
  return (
    <nav aria-label="Netherlands programme pages" className="mt-6 flex items-center justify-between rounded-xl border border-[#e7e6e3] bg-white px-4 py-3">
      {result.page > 1 ? (
...
export function NlProgramsExplorer({ filters, result }: { filters: ProgramSearchFilters; result: NlProgramSearchResult }) {
  return (
    <section className="mt-7 min-w-0">
      ...
      <div className="mt-3 space-y-3">
        {result.programs.map((program) => (
          <Link key={program.id} href={nlProgramDetailPath(program.slug)} className="block rounded-xl border border-[#e7e6e3] bg-white p-5 transition hover:border-[#b9cdb2] hover:shadow-sm">
            <div className="flex flex-wrap items-center gap-2 text-[10.5px] font-semibold">
              <span className="rounded-full bg-[#f3f4f1] px-2.5 py-1 text-[#686660]">{levelLabel(program.canonicalLevel, program.degreeType)}</span>
            ...
            <div className="mt-3 flex flex-wrap gap-x-5 gap-y-2 text-[11.5px] text-[#77746e]">
              ...
              {program.ects && <span>{program.ects} ECTS</span>}
      ...
      {result.programs.length === 0 && <div className="mt-4 rounded-xl border border-dashed border-[#dcdad4] bg-[#fbfbf9] p-10 text-center text-[13px] text-[#77746e]">No Netherlands programmes match these filters.</div>}
      <Pagination filters={filters} result={result} />
      <p className="mt-4 text-[10.5px] leading-5 text-[#aaa7a0]">Programme identity, Dutch recognition evidence, international-student eligibility, recognised sponsor evidence and the current application window are tracked separately. Campus and city are not shown unless programme-level evidence supports them.</p>
    </section>
  )
}
