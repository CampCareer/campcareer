  ...
  return (
    <div ref={rootRef} className="relative">
      <button
        ...
        aria-expanded={open}
        className="inline-flex h-9 items-center gap-2 rounded-full border border-[#e0dfdb] bg-white pl-2 pr-3 text-[13px] font-medium text-[#1b1b1b] transition hover:border-[#3e7a2e]/50"
      >
      ...
      {open && (
        <div
          role="listbox"
          aria-label={locale === "ko" ? "프로그램 국가 선택" : "Select a program country"}
        ...
        >
          <p className="px-2.5 pb-1 pt-2 text-[10.5px] font-semibold uppercase tracking-[0.08em] text-[#a3a19b]">
            {locale === "ko" ? "호주·캐나다 공개" : "Australia & Canada live · more countries next"}
          </p>
          <ul className="max-h-72 overflow-y-auto">
            {LAUNCH_COUNTRIES.map((country) => {
              const isSelected = country.code === selected.code
              const isProgramsLive = country.code === "AU" || country.code === "CA"
              return (
                <li key={country.code}>
                  <button
                    type="button"
                    ...
                    className={cn(
                      ...
                      isSelected ? "bg-[#edf5ea] text-[#3e7a2e]" : "text-[#4d4c48] hover:bg-[#fafaf8]",
                    )}
                  >
                    <img src={country.image} alt="" width={40} height={28} className="size-4 shrink-0 rounded-full object-cover" />
                    <span className="truncate">{country.name}</span>
                    <span className="ml-auto flex items-center gap-2">
                      {!isProgramsLive && <span className="text-[10px] font-semibold uppercase text-[#b0ada6]">Soon</span>}
              ...
              placeholder={
                locale === "ko"
                  ? filters.country === "CA"
                    ...
                    : "과정명으로 검색하세요. 예: Nursing, Data Science"
                  : filters.country === "CA"
        ...
        <div className="mt-5 flex gap-2 overflow-x-auto pb-1">
          {PROGRAM_LEVELS.map((level) => (
            <button
              key={level.value}
              type="button"
              onClick={() => replace({ level: level.value })}
              className={cn(
                "shrink-0 rounded-lg border px-3.5 py-2 text-[12.5px] font-medium transition",
                filters.level === level.value
                  ? "border-[#3e7a2e] bg-[#3e7a2e] text-white"
                  : "border-[#dfded9] bg-white text-[#686660] hover:border-[#3e7a2e]/45 hover:text-[#3e7a2e]",
              )}
            >
