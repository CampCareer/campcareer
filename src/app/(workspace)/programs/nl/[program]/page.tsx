...
import { getNlProgramBySlug } from "@/lib/programs/nl-programs.server"
import { isIndexableNlProgramSlug, nlProgramDetailPath } from "@/lib/programs/nl-program-seo"
...
function admissionCopy(status: string) {
  if (status === "open") return { label: "Applications open", body: "A current programme or admission source supports an open application window. Recheck the official page before applying because deadlines can change.", className: "border-[#cfe0c9] bg-[#f4f9f2] text-[#37672c]" }
  ...
  return { label: "Current application window not confirmed", body: "The programme, recognition and international-study context are source-backed, but CampCareer has not inferred that applications are open today.", className: "border-[#d9e3f7] bg-[#f7f9ff] text-[#315f9f]" }
}

async function loadProgram(segment: string) {
  const slug = safeSlug(segment)
  return slug ? getNlProgramBySlug(slug) : null
}
...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  const canonical = nlProgramDetailPath(program.slug)
  const indexable = program.indexable && isIndexableNlProgramSlug(program.slug)
  return {
    title: `${program.title} · ${program.institutionName}`,
    description: `${program.title} at ${program.institutionName}. Dutch recognition, international-student eligibility, recognised sponsor evidence and application timing are tracked separately.`,
    alternates: { canonical: `${SITE_URL}${canonical}` },
    robots: { index: indexable, follow: true },
  }
}

export default async function NlProgramDetailPage({ params }: Params) {
  const { program: segment } = await params
  const program = await loadProgram(segment)
  if (!program) notFound()
  const canonicalPath = nlProgramDetailPath(program.slug)
  if (segment !== program.slug) permanentRedirect(canonicalPath)

  const admission = admissionCopy(program.admissionState)
  const institutionPath = institutionDetailPath("NL", program.institutionSlug)
  const officialUrl = safeUrl(program.officialProgramUrl)
  const recognitionUrl = safeUrl(program.recognitionSourceUrl)
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <Link href="/programs?country=NL" className="inline-flex items-center gap-1.5 text-[12px] font-semibold text-[#6f6d68] hover:text-[#3e7a2e]"><ArrowLeft className="size-3.5" /> Back to Netherlands programmes</Link>
      <div className="mt-5 grid gap-6 lg:grid-cols-[minmax(0,1fr)_300px] lg:items-start">
        <main className="min-w-0">
          <header className="rounded-2xl border border-[#dfe6dc] bg-gradient-to-br from-[#f4f8f2] via-white to-[#eef4ff] p-6 sm:p-8">
            <div className="flex flex-wrap gap-2 text-[10.5px] font-semibold">
              <span className="rounded-full bg-white px-3 py-1">{humanize(program.degreeType ?? program.canonicalLevel)}</span>
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            <Metric icon={<GraduationCap className="size-4" />} label="Degree" value={humanize(program.degreeType ?? program.canonicalLevel)} />
            ...
            <Metric icon={<CalendarClock className="size-4" />} label="ECTS" value={program.ects ? `${program.ects} ECTS` : "Not published"} />
          </div>

          <section className="mt-5 rounded-xl border border-[#e7e6e3] bg-white p-5 sm:p-6">
            <h2 className="text-[14.5px] font-semibold">Dutch recognition and international-study evidence</h2>
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <Evidence label="Accredited programme evidence" value={program.accreditedProgrammeEvidence === true ? "Confirmed" : "Not resolved"} />
              ...
              <Evidence label="Recognised sponsor evidence" value={program.studentSponsorEligible === true ? "Confirmed" : program.studentSponsorEligible === false ? "Not confirmed" : "Not resolved"} />
              <Evidence label="Framework" value={[program.nlqfLevel ? `NLQF ${program.nlqfLevel}` : null, program.eqfLevel ? `EQF ${program.eqfLevel}` : null].filter(Boolean).join(" · ") || program.nativeLevelCode || "Not published"} />
            </div>
            <p className="mt-4 text-[10.5px] leading-5 text-[#8f8c85]">Recognised sponsor status, programme recognition and current admission availability are separate facts. CampCareer does not infer an open application window from institutional sponsor status.</p>
          </section>
          ...
          {program.occupationRelations.length > 0 && (
            <section className="mt-5 rounded-xl border border-[#e7e6e3] bg-white p-5 sm:p-6">
              ...
              <p className="mt-3 text-[10.5px] leading-5 text-[#8f8c85]">These are reviewed education relationships. They do not imply professional registration, licensing or guaranteed employment eligibility in the Netherlands.</p>
            </section>
          )}
        </main>
        ...
        <aside className="rounded-2xl border border-[#e7e6e3] bg-white p-5 lg:sticky lg:top-20">
          ...
          <div className="mt-4 space-y-2">
            ...
            {recognitionUrl && recognitionUrl !== officialUrl && <SourceLink href={recognitionUrl}>Recognition source</SourceLink>}
            {internationalUrl && internationalUrl !== officialUrl && <SourceLink href={internationalUrl}>International-student source</SourceLink>}
            {sponsorUrl && sponsorUrl !== internationalUrl && sponsorUrl !== officialUrl && <SourceLink href={sponsorUrl}>Recognised sponsor source</SourceLink>}
