...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `${BASE_URL}/programs?country=${filters.country}` },
    robots: { index: isAustraliaBase, follow: true },
  }
}
...
function EmptyResults({ filters }: { filters: ProgramSearchFilters }) {
  return (
    <div className="mt-4 flex min-h-72 flex-col items-center justify-center rounded-xl border border-dashed border-[#dcdad4] bg-[#fbfbf9] p-8 text-center">
      ...
      <Link
        href={buildProgramsUrl(filters, { q: "", level: "all", field: "all", city: "all", state: "all", duration: "all", fee: "all", source: "all", sort: "recommended", page: 1 })}
...
function CountryComingSoon({ countryCode }: { countryCode: string }) {
  ...
  return (
    <div className="flex min-h-[420px] flex-col items-center justify-center rounded-2xl border border-dashed border-[#dcdad4] bg-[#fbfbf9] p-10 text-center">
      ...
      <p className="mt-3 max-w-lg text-[13px] leading-6 text-[#6f6d68]">Australia is the first complete program catalogue. Other countries will open after their institutions, course identifiers, fees and official source links pass the same review.</p>
      <Link href="/programs?country=AU" className="mt-5 rounded-lg bg-[#3e7a2e] px-4 py-2.5 text-[12.5px] font-semibold text-white transition hover:bg-[#326625]">Browse Australia</Link>
    </div>
  )
}
          ...
          <section className="min-w-0">
            {errorMessage ? (
              <div className="flex min-h-72 flex-col items-center justify-center rounded-xl border border-[#f0d8d2] bg-[#fff9f7] p-8 text-center">
                ...
                <h2 className="mt-3 text-[16px] font-semibold text-[#1b1b1b]">Australian program data is temporarily unavailable</h2>
              ...
              <>
                ...
                {result.programs.length === 0 ? <EmptyResults filters={filters} /> : <div className="mt-3 space-y-3">{result.programs.map((program) => <ProgramCard key={program.id} program={program} />)}</div>}
