...
export async function generateMetadata({
  ...
  return {
    ...
    description,
    alternates: { canonical: `${SITE_URL}${programsCanonicalPath(filters.country)}` },
    robots: { index: isPublishedBase, follow: true },
  }
}
...
function Pagination({
  ...
  return (
    <nav aria-label="Program result pages" className="mt-6 flex items-center justify-between rounded-xl border border-[#e7e6e3] bg-white px-4 py-3">
      {page > 1 ? (
        <Link href={buildProgramsUrl(filters, { page: page - 1 })} className="rounded-lg border border-[#deddd8] px-3 py-2 text-[12px] font-semibold text-[#4d4c48] transition hover:border-[#3e7a2e]/50 hover:text-[#3e7a2e]">Previous</Link>
      ) : <span />}
      <p className="text-[11.5px] font-medium text-[#8f8c85]">Page {page} of {pageCount}</p>
      {page < pageCount ? (
        <Link href={buildProgramsUrl(filters, { page: page + 1 })} className="rounded-lg bg-[#3e7a2e] px-3.5 py-2 text-[12px] font-semibold text-white transition hover:bg-[#326625]">Next</Link>
      ) : <span />}
    </nav>
  )
}
...
function EmptyResults({ filters }: { filters: ProgramSearchFilters }) {
  return (
    <div className="mt-4 flex min-h-72 flex-col items-center justify-center rounded-xl border border-dashed border-[#dcdad4] bg-[#fbfbf9] p-8 text-center">
      <span className="grid size-12 place-items-center rounded-2xl bg-[#edf5ea] text-[#3e7a2e]"><GraduationCap className="size-5" /></span>
      <h2 className="mt-4 text-[17px] font-semibold text-[#1b1b1b]">No programs match these filters</h2>
      <p className="mt-2 max-w-md text-[12.5px] leading-5 text-[#77746e]">Try a broader city, study level, field, state, duration or tuition range.</p>
      <Link href={buildProgramsUrl(filters, { q: "", level: "all", field: "all", city: "all", state: "all", duration: "all", fee: "all", source: "all", sort: "recommended", page: 1 })} className="mt-4 rounded-lg border border-[#cfd9ca] bg-white px-4 py-2 text-[12px] font-semibold text-[#3e7a2e] transition hover:bg-[#edf5ea]">Clear filters</Link>
    </div>
  )
}
...
function CountryComingSoon({ countryCode }: { countryCode: string }) {
  ...
  return (
    <div className="flex min-h-[420px] flex-col items-center justify-center rounded-2xl border border-dashed border-[#dcdad4] bg-[#fbfbf9] p-10 text-center">
      <span className="grid size-14 place-items-center rounded-2xl bg-[#eef4ff] text-[#2563eb]"><MapPinned className="size-6" /></span>
      ...
      <p className="mt-3 max-w-lg text-[13px] leading-6 text-[#6f6d68]">Country catalogues open after program identities, official sources and current international-admission evidence pass review.</p>
...
export default async function ProgramsPage({
  ...
  if (filters.country === "AU") {
    try { auResult = await searchAuPrograms(filters) }
    catch (error) {
  ...
  } else if (filters.country === "AE") {
    try { aeResult = await searchAePrograms(filters) }
    catch (error) {
