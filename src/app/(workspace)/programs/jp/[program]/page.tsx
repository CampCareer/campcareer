...
import { getJpProgramBySlug } from "@/lib/programs/jp-programs.server"
import { isIndexableJpProgramSlug, jpProgramDetailPath } from "@/lib/programs/jp-program-seo"
import { institutionDetailPath } from "@/lib/institutions/institution-search"

const BASE_URL = "https://www.campcareer.com"
const JP_PUBLIC_INSTITUTION_SLUGS = new Set([
...
function admissionCopy(status: string, verification: string) {
  if (verification === "unverified" || verification === "stale" || verification === "rejected") return {
    label: "Current international admission not verified",
    body: "The programme record exists, but CampCareer has not verified a current international application pathway for this programme.",
    className: "border-[#e5e1d7] bg-[#faf9f5] text-[#6f6d68]",
  }
  if (status === "open") return {
    label: "International applications open",
    body: "A current programme source supports an open international application window. Recheck the provider page before submitting because deadlines can change.",
    className: "border-[#cfe0c9] bg-[#f4f9f2] text-[#37672c]",
  }
  if (status === "closed") return {
    label: "Verified application cycle closed",
    body: "The programme remains in the source-verified catalogue, but the verified intake is closed. For final-intake programmes, no future intake is implied.",
  ...
  return {
    label: "International pathway verified · current window unknown",
    body: "A current international-student programme or provider source exists, but CampCareer has not inferred that applications are open today.",
...
async function loadProgram(segment: string) {
  ...
  return slug ? getJpProgramBySlug(slug) : null
}
...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  const canonical = jpProgramDetailPath(program.slug)
  return {
    title: `${program.title} · ${program.institutionName}`,
    description: `${program.title} at ${program.institutionName}. International programme source and current application status are tracked separately.`,
    alternates: { canonical: `${BASE_URL}${canonical}` },
    robots: { index: isIndexableJpProgramSlug(program.slug) && program.internationalAdmissionStatus !== "closed", follow: true },
  }
}

export default async function JpProgramDetailPage({ params }: Params) {
  const { program: segment } = await params
  const program = await loadProgram(segment)
  if (!program) notFound()
  const canonicalPath = jpProgramDetailPath(program.slug)
  ...
  const institutionPath = program.institutionSlug && JP_PUBLIC_INSTITUTION_SLUGS.has(program.institutionSlug)
    ? institutionDetailPath("JP", program.institutionSlug)
    : null
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <Link href="/programs?country=JP" className="inline-flex items-center gap-1.5 text-[12px] font-semibold text-[#6f6d68] hover:text-[#3e7a2e]"><ArrowLeft className="size-3.5" /> Back to Japan programs</Link>
      <div className="mt-5 grid gap-6 lg:grid-cols-[minmax(0,1fr)_300px] lg:items-start">
        <main className="min-w-0">
          <header className="rounded-2xl border border-[#dfe6dc] bg-gradient-to-br from-[#f4f8f2] via-white to-[#eef4ff] p-6 sm:p-8">
            ...
            <div className="mt-3 flex flex-wrap gap-x-5 gap-y-2 text-[12px] text-[#77746e]">
              {program.city && <span className="inline-flex items-center gap-1.5"><MapPin className="size-4" />{program.city}</span>}
              {program.languageContext && <span className="inline-flex items-center gap-1.5"><Languages className="size-4" />{program.languageContext}</span>}
            </div>
          </header>
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            <Metric icon={<GraduationCap className="size-4" />} label="Study level" value={program.degreeLevel} />
            <Metric icon={<Clock3 className="size-4" />} label="Duration" value={durationLabel(program.durationMonths)} />
            <Metric icon={<Languages className="size-4" />} label="Language context" value={program.languageRequirementContext ?? program.languageContext ?? "Confirm with provider"} />
            <Metric icon={<CalendarClock className="size-4" />} label="Admission status" value={program.internationalAdmissionStatus.replaceAll("_", " ")} />
          </div>

          <section className="mt-5 rounded-xl border border-[#e7e6e3] bg-white p-5 sm:p-6">
            <h2 className="text-[14.5px] font-semibold">Source semantics</h2>
            <p className="mt-3 text-[12px] leading-6 text-[#65625c]">{program.sourceVerificationLabel ?? "Official programme/provider and international-student source verified."}</p>
            <p className="mt-2 text-[11px] leading-5 text-[#8f8c85]">JASSO / Study in Japan is used as an international-student discovery and course source where applicable. CampCareer does not label that listing as programme-level accreditation.</p>
          </section>

          {program.occupationIds.length > 0 && <section className="mt-5 rounded-xl border border-[#e7e6e3] bg-white p-5 sm:p-6"><h2 className="text-[14.5px] font-semibold">CampCareer career links</h2><div className="mt-3 flex flex-wrap gap-2">{program.occupationIds.map((id) => <span key={id} className="rounded-full bg-[#f3f4f1] px-2.5 py-1 text-[10.5px] font-medium text-[#686660]">{id.replace(/-/g, " ")}</span>)}</div><p className="mt-3 text-[10.5px] leading-5 text-[#8f8c85]">These are manually reviewed education relationships, not claims of Japanese professional licensing or guaranteed employment eligibility.</p></section>}
        </main>
        ...
        <aside className="rounded-2xl border border-[#e7e6e3] bg-white p-5 lg:sticky lg:top-20">
          ...
          <div className="mt-5 space-y-2">
            ...
            {internationalUrl && internationalUrl !== officialUrl && <SourceLink href={internationalUrl}>International programme source</SourceLink>}
          ...
          <p className="mt-4 text-[10.5px] leading-5 text-[#aaa7a0]">Admission windows and language requirements can change. Unknown values stay unknown until a current source is verified.</p>
        </aside>
      </div>
    </div>
  )
