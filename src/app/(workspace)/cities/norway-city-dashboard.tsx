...
import type { NoCityProfile } from "@/lib/cities/no-city-profile.server"

function money(value: number, currency = "NOK") {
  return new Intl.NumberFormat("en-NO", { style: "currency", currency, maximumFractionDigits: value % 1 ? 2 : 0 }).format(value)
}

function compact(value: number) {
  return new Intl.NumberFormat("en-NO", { notation: "compact", maximumFractionDigits: 2 }).format(value)
}
...
export function NorwayCityDashboard({ profile }: { profile: NoCityProfile }) {
  ...
  return <div>
    <section className="bg-gradient-to-br from-[#7a1224] via-[#b11f3a] to-[#e9d8dc] text-white">
      <div className="mx-auto w-full max-w-6xl px-4 pb-20 pt-14 sm:px-8 sm:pt-20 lg:px-10">
        <nav className="flex flex-wrap items-center gap-2 text-[11px] font-medium text-white/75" aria-label="Breadcrumb">
          ...
          <Link href="/countries/no" className="hover:text-white">Norway</Link><span>/</span><span>{profile.name}</span>
        </nav>
        ...
        <p className="mt-2 text-[14px] font-medium text-white/90">{profile.regionName} · {profile.scopeLabel} · Statistics Norway {profile.municipalityCode}</p>
      </div>
    </section>
    ...
    <main className="mx-auto w-full max-w-6xl px-4 pb-12 sm:px-8 lg:px-10">
      <section className="-mt-8 rounded-2xl border border-[#e7e6e3] bg-white p-5 shadow-xl shadow-black/10 sm:p-6">
        <p className="text-[12px] font-semibold text-[#9d1832]">Student decision snapshot</p>
        <h2 className="mt-1 text-[20px] font-semibold tracking-[-0.02em] text-[#1b1b1b]">Verified municipality evidence for {profile.name}</h2>
        <p className="mt-1.5 max-w-3xl text-[12px] leading-5 text-[#77746e]">Population, route identity and study-location membership use the exact Statistics Norway municipality boundary. Broader regions and multi-campus institution footprints are not silently merged into the City profile.</p>
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <MetricCard icon={<Users className="size-4 text-[#9d1832]" />} label="Population" value={profile.population ? compact(profile.population.amount) : "—"} note={profile.population ? `${profile.population.geography} · ${profile.population.dataAsOf}` : "Verified population unavailable"} />
        <MetricCard icon={<Wallet className="size-4 text-[#a56b00]" />} label="Student living" value={living} note={profile.livingCost?.citySpecific ? "City-specific monthly reference" : "National 2026–27 planning/funds reference · not a measured city cost ranking"} />
        ...
        <MetricCard icon={<Clock3 className="size-4 text-[#3e7a2e]" />} label="Study-permit work" value={profile.workRights ? `${profile.workRights.hoursNormalPeriod} h / ${period(profile.workRights.period)}` : "—"} note="National UDI permit context · full-time during holidays · not a city differentiator" />
      </div>
      ...
      <section className="mt-5 rounded-xl border border-[#eadfca] bg-[#fffaf1] p-5 sm:p-6">
        <div className="flex items-start gap-3"><Info className="mt-0.5 size-4 shrink-0 text-[#a86514]" /><div>
          ...
          <p className="mt-1 text-[11.5px] leading-5 text-[#7a5a31]">The current provider foundation covers the 11 NOKUT university-category institutions, not Norway&apos;s complete approved HEI universe. Institution identity remains an authority-backed name identity rather than a durable national identifier.</p>
        </div></div>
      </section>
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1.4fr)_minmax(300px,0.8fr)]">
        <div className="space-y-5">
          <section className="rounded-xl border border-[#e7e6e3] bg-white p-5 sm:p-6">
            <div className="flex flex-wrap items-center gap-2 text-[#9d1832]"><GraduationCap className="size-4" /><h2 className="text-[15px] font-semibold">Verified university study locations</h2><span className="ml-auto text-[10.5px] font-semibold">{profile.linkedInstitutionCount} institutions</span></div>
            ...
            <div className="mt-4 grid gap-2.5 sm:grid-cols-2">{profile.institutions.map((institution) => <article key={institution.id} className="rounded-lg border border-[#eeece8] bg-[#fafaf8] p-3.5">
              <div className="flex gap-3"><Building2 className="mt-1 size-4 shrink-0 text-[#9d1832]" /><div className="min-w-0">
                ...
                {institution.websiteUrl ? <a href={institution.websiteUrl} target="_blank" rel="noreferrer" className="mt-2 inline-block text-[10.5px] font-semibold text-[#9d1832] hover:underline">Official site <ExternalLink className="inline size-3" /></a> : null}
              </div></div>
            </article>)}</div>
          </section>
          ...
          <section className="rounded-xl border border-[#e7e6e3] bg-white p-5 sm:p-6">
            ...
            <p className="mt-2 text-[11.5px] leading-5 text-[#77746e]">Up to 8 alphabetical examples from {profile.linkedProgramCount} exact source-city matched programmes. Counts remain verified-partial because the current institution foundation does not cover every approved Norwegian HEI category.</p>
        ...
        <div className="space-y-5">
          ...
          <section className="rounded-xl border border-[#d9e3f7] bg-[#f7f9fe] p-5"><h2 className="text-[14.5px] font-semibold">Student work context</h2><p className="mt-2 text-[11.5px] leading-5 text-[#5e6f91]">{profile.workRights?.note ?? "Verified national study-permit work context unavailable."}</p></section>
