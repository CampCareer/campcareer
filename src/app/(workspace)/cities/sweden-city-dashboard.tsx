...
function money(value: number, currency = "SEK") {
  return new Intl.NumberFormat("en-SE", { style: "currency", currency, maximumFractionDigits: value % 1 ? 2 : 0 }).format(value)
}
...
export function SwedenCityDashboard({ profile }: { profile: SeCityProfile }) {
  const living = profile.livingCost ? (profile.livingCost.low === profile.livingCost.high ? `~${money(profile.livingCost.low)}` : `${money(profile.livingCost.low)}–${money(profile.livingCost.high)}`) : "—"
  const transport = profile.transport ? `${money(profile.transport.amount)} / ${period(profile.transport.period)}` : "—"

  return <div>
    <section className="bg-gradient-to-br from-[#173b64] via-[#245a8d] to-[#e3b341] text-white">
      <div className="mx-auto w-full max-w-6xl px-4 pb-20 pt-14 sm:px-8 sm:pt-20 lg:px-10">
        <nav className="flex flex-wrap items-center gap-2 text-[11px] font-medium text-white/75" aria-label="Breadcrumb">
          ...
          <Link href="/countries/se" className="hover:text-white">Sweden</Link><span>/</span><span>{profile.name}</span>
        </nav>
        <p className="mt-6 text-[11px] font-semibold uppercase tracking-[0.14em] text-white/75">Cities</p>
        <h1 className="mt-2 text-[38px] font-semibold leading-tight tracking-[-0.03em] sm:text-[48px]">{profile.name}</h1>
        <p className="mt-2 text-[14px] font-medium text-white/90">{profile.regionName} · {profile.scopeLabel} · SCB {profile.municipalityCode}</p>
      </div>
    </section>
    ...
    <main className="mx-auto w-full max-w-6xl px-4 pb-12 sm:px-8 lg:px-10">
      <section className="-mt-8 rounded-2xl border border-[#e7e6e3] bg-white p-5 shadow-xl shadow-black/10 sm:p-6">
        <p className="text-[12px] font-semibold text-[#245a8d]">Student decision snapshot</p>
        <h2 className="mt-1 text-[20px] font-semibold tracking-[-0.02em] text-[#1b1b1b]">Verified municipality evidence for {profile.name}</h2>
        <p className="mt-1.5 max-w-3xl text-[12px] leading-5 text-[#77746e]">Population and university-location membership use the exact SCB municipality boundary. Metro and county geography are not silently substituted.</p>
        <div className="mt-4 flex flex-wrap gap-2 text-[10.5px] font-semibold text-[#5d6470]">
          ...
          <span className="rounded-full bg-[#f4f6f9] px-2.5 py-1">{profile.linkedCampusCount} verified city locations</span>
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <MetricCard icon={<Users className="size-4 text-[#245a8d]" />} label="Population" value={profile.population ? compact(profile.population.amount) : "—"} note={profile.population ? `${profile.population.geography} · ${profile.population.dataAsOf}` : "Verified population unavailable"} />
        <MetricCard icon={<Wallet className="size-4 text-[#b38318]" />} label="Student living" value={living} note={profile.livingCost?.citySpecific ? "City-specific monthly reference" : "Official national monthly student-budget baseline · not city-specific"} />
        <MetricCard icon={<TrainFront className="size-4 text-[#6d4fc4]" />} label="Public transport" value={transport} note={profile.transport?.note ?? "Source-native operator reference"} />
        <MetricCard icon={<Clock3 className="size-4 text-[#3e7a2e]" />} label="Student permit work" value={profile.workRights ? `${profile.workRights.hoursNormalPeriod} h / ${profile.workRights.period}` : "—"} note={profile.workRights?.effectiveFrom ? `Relevant first/second-cycle permit rule from ${profile.workRights.effectiveFrom}; transition rules can apply.` : "Verified work-rule context unavailable"} />
      </div>
      ...
      <section className="mt-5 rounded-xl border border-[#eadfca] bg-[#fffaf1] p-5 sm:p-6">
        <div className="flex items-start gap-3"><Info className="mt-0.5 size-4 shrink-0 text-[#a86514]" /><div>
          ...
          <p className="mt-1 text-[11.5px] leading-5 text-[#7a5a31]">The current provider layer is a selected ten-university core. Full UKÄ-recognised higher-education provider coverage remains pending.</p>
        </div></div>
      </section>
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1.4fr)_minmax(300px,0.8fr)]">
        <div className="space-y-5">
          <section className="rounded-xl border border-[#e7e6e3] bg-white p-5 sm:p-6">
            <div className="flex flex-wrap items-center gap-2 text-[#245a8d]"><GraduationCap className="size-4" /><h2 className="text-[15px] font-semibold">Verified university city locations</h2><span className="ml-auto text-[10.5px] font-semibold">{profile.linkedInstitutionCount} institutions</span></div>
            <p className="mt-2 text-[11.5px] leading-5 text-[#77746e]">Each institution is UKÄ-backed. A city location is not claimed to be a complete campus inventory.</p>
            <div className="mt-4 grid gap-2.5 sm:grid-cols-2">{profile.institutions.map((institution) => <article key={institution.id} className="rounded-lg border border-[#eeece8] bg-[#fafaf8] p-3.5">
              <div className="flex gap-3"><Building2 className="mt-1 size-4 shrink-0 text-[#245a8d]" /><div className="min-w-0">
                <p className="text-[12.5px] font-semibold leading-5">{institution.name}</p><p className="mt-0.5 text-[10.5px] text-[#8f8c85]">UKÄ identity: {institution.authorityIdentifier}</p>
                {institution.locations.map((location) => <p key={location.id} className="mt-1.5 flex gap-1 text-[10.5px] text-[#77746e]"><MapPin className="mt-0.5 size-3 shrink-0" />{location.name}</p>)}
                <div className="mt-2 flex gap-3">{institution.websiteUrl ? <a href={institution.websiteUrl} target="_blank" rel="noreferrer" className="text-[10.5px] font-semibold text-[#245a8d] hover:underline">Official site <ExternalLink className="inline size-3" /></a> : null}<a href={institution.authoritySourceUrl} target="_blank" rel="noreferrer" className="text-[10.5px] font-semibold text-[#245a8d] hover:underline">UKÄ source <ExternalLink className="inline size-3" /></a></div>
              </div></div>
            </article>)}</div>
          </section>
          ...
          <section className="rounded-xl border border-[#e7e6e3] bg-white p-5 sm:p-6">
            ...
            <p className="mt-2 text-[11.5px] leading-5 text-[#77746e]">Up to 8 alphabetical examples from {profile.linkedProgramCount} programmes whose official source city matches this verified university municipality location.</p>
            <div className="mt-4 space-y-2">{profile.programmeSample.map((programme) => <a key={programme.id} href={programme.officialUrl} target="_blank" rel="noreferrer" className="block rounded-lg border border-[#eeece8] bg-[#fafaf8] px-3.5 py-3 hover:border-[#d7d3cc]">
              <p className="text-[12px] font-semibold leading-5">{programme.title}</p><p className="mt-0.5 text-[10.5px] text-[#77746e]">{programme.institutionName} · {programme.locationName}{programme.type ? ` · ${programme.type}` : ""}</p>
            </a>)}</div>
          </section>
        </div>
