...
import { PUBLISHED_CH_CITY_SLUGS, isPublishedChCitySlug } from "@/lib/cities/city-routes"
import { buildCityCompareCanonicalHref } from "@/lib/compare-routes"

export const dynamic = "force-dynamic"

const CITY_NAMES: Record<(typeof PUBLISHED_CH_CITY_SLUGS)[number], string> = {
  zurich: "Zurich",
  lausanne: "Lausanne",
  basel: "Basel",
  lugano: "Lugano",
...
export function generateStaticParams() {
  return PUBLISHED_CH_CITY_SLUGS.map((city) => ({ city }))
}

export async function generateMetadata({ params }: { params: Promise<{ city: string }> }): Promise<Metadata> {
  const { city } = await params
  const normalized = city.trim().toLowerCase()
  if (!isPublishedChCitySlug(normalized)) return { robots: { index: false, follow: false } }
...
export default async function SwitzerlandCityPage({ params }: { params: Promise<{ city: string }> }) {
  const { city } = await params
  const normalized = city.trim().toLowerCase()
  if (!isPublishedChCitySlug(normalized)) notFound()
