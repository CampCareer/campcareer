...
export const metadata: Metadata = {
  title: "Sydney vs Melbourne for International Students",
  description:
    "Compare Sydney and Melbourne student living costs, verified CRICOS programs, registered study locations, transport references and career context.",
  alternates: { canonical: "/cities/au/compare" },
  ...
  return (
    <div className="px-4 py-5 md:px-5">
      ...
      <div className="mt-1 flex items-center justify-between gap-3">
        ...
        <Link href={`/cities/au/${city.slug}`} className="text-[11px] font-semibold text-[#2563eb] hover:underline">City profile →</Link>
      </div>
    </div>
  )
}

export default async function SydneyMelbourneComparePage() {
  const comparison = await getSydneyMelbourneComparison()
  if (!comparison) notFound()

  const { left: sydney, right: melbourne, sharedProgramCount } = comparison
  const lowerLiving = sydney.livingCost && melbourne.livingCost
    ? (sydney.livingCost.low + sydney.livingCost.high) / 2 < (melbourne.livingCost.low + melbourne.livingCost.high) / 2
      ? "Sydney"
      : "Melbourne"
    : null
  const morePrograms = sydney.verifiedProgramCount >= melbourne.verifiedProgramCount ? "Sydney" : "Melbourne"
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 pb-14 pt-8 sm:px-8 lg:px-10">
      <nav className="flex flex-wrap items-center gap-2 text-[11px] font-medium text-[#9a978f]" aria-label="Breadcrumb">
        <Link href="/countries/au" className="hover:text-[#2563eb]">Australia</Link><span>/</span><Link href="/cities/au/sydney" className="hover:text-[#2563eb]">Cities</Link><span>/</span><span>Compare</span>
      </nav>
      ...
      <header className="mt-6 rounded-2xl border border-[#dfe6dc] bg-gradient-to-br from-[#f5f9f3] via-white to-[#eef4ff] p-6 sm:p-8">
        ...
        <h1 className="mt-2 text-[32px] font-semibold tracking-[-0.035em] text-[#1b1b1b] sm:text-[40px]">Sydney vs Melbourne</h1>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          <Link href="/programs?country=AU&city=sydney" className="rounded-lg bg-[#2563eb] px-3.5 py-2 text-[11.5px] font-semibold text-white">Sydney programs</Link>
          <Link href="/programs?country=AU&city=melbourne" className="rounded-lg bg-[#3e7a2e] px-3.5 py-2 text-[11.5px] font-semibold text-white">Melbourne programs</Link>
        </div>
      </header>
      ...
      <section className="mt-5 overflow-hidden rounded-2xl border border-[#e7e6e3] bg-white">
        <div className="grid md:grid-cols-[220px_minmax(0,1fr)_minmax(0,1fr)]">
          ...
          <div className="border-l border-[#ecebe7]"><CityHeader city={sydney} /></div>
          <div className="border-l border-[#ecebe7]"><CityHeader city={melbourne} /></div>
        </div>
        ...
        <ComparisonRow icon={<Users className="size-4" />} label="Metro population" note="ABS Greater Capital City Statistical Area" left={cityValue(sydney, "population")} right={cityValue(melbourne, "population")} />
        <ComparisonRow icon={<BriefcaseBusiness className="size-4" />} label="Career context" note="Official study-destination guidance, not shortage rankings" left={sydney.employmentSectors.slice(0, 4).join(" · ")} right={melbourne.employmentSectors.slice(0, 5).join(" · ")} />
      </section>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-3">
        <article className="rounded-xl border border-[#e7e6e3] bg-white p-5">
          ...
          <p className="mt-2 text-[18px] font-semibold text-[#1b1b1b]">{lowerLiving ? `${lowerLiving} has the lower current midpoint` : "Compare the published ranges"}</p>
          <p className="mt-2 text-[11.5px] leading-5 text-[#77746e]">This is an indicative source comparison, not a guaranteed personal budget. Melbourne&apos;s monthly range is calculated from Monash&apos;s annual range.</p>
        </article>
        <article className="rounded-xl border border-[#e7e6e3] bg-white p-5">
          ...
          <p className="mt-2 text-[11.5px] leading-5 text-[#77746e]">Sydney: {sydney.verifiedProgramCount.toLocaleString("en-AU")} · Melbourne: {melbourne.verifiedProgramCount.toLocaleString("en-AU")} active CRICOS programs.</p>
        </article>
        <article className="rounded-xl border border-[#e7e6e3] bg-white p-5">
          ...
          <p className="mt-2 text-[11.5px] leading-5 text-[#77746e]">Active programs currently registered at delivery locations in both Greater Sydney and Greater Melbourne.</p>
        </article>
      </div>
      ...
      <section className="mt-5 rounded-xl border border-[#d9e3f7] bg-[#f7f9fe] p-5 sm:p-6">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <Link href="/programs?country=AU&city=sydney" className="inline-flex items-center gap-1.5 rounded-lg border border-[#bfcff0] bg-white px-3.5 py-2 text-[11.5px] font-semibold text-[#2563eb]">Browse Sydney <ArrowRight className="size-3.5" /></Link>
          <Link href="/programs?country=AU&city=melbourne" className="inline-flex items-center gap-1.5 rounded-lg border border-[#cfd9ca] bg-white px-3.5 py-2 text-[11.5px] font-semibold text-[#3e7a2e]">Browse Melbourne <ArrowRight className="size-3.5" /></Link>
        </div>
      </section>
    </div>
  )
