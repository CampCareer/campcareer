...
export function generateStaticParams() { return SUPPORTED_FI_CITY_SLUGS.map((city) => ({ city })) }
  ...
  return <>
    ...
    <div className="mx-auto -mt-6 w-full max-w-6xl px-4 pb-12 sm:px-8 lg:px-10">
      <div className="rounded-xl border border-[#dbe7e3] bg-[#f4f8f6] p-4 sm:flex sm:items-center sm:justify-between sm:gap-4">
        <div><p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-[#275b50]">City Compare</p><p className="mt-1 text-[12px] leading-5 text-[#64748b]">Compare {profile.name} with another verified Finland Tier A municipality using the same evidence contract.</p></div>
