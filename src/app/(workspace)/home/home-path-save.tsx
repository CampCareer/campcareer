...
export function HomePathSaveProvider({ values, children }: { values: SavedPathwayInput; children: ReactNode }) {
  ...
  const pathwayKey = `${values.country}:${values.field}:${values.status}`
  ...
  const save = useCallback(async () => {
    ...
    setState("error")
    setMessage(result.state === "invalid" ? "This path is no longer valid. Update your search and try again." : "We couldn’t save this path. Please try again.")
  }, [router, values])
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (result.state === "unauthenticated") {
        setState("idle")
        setMessage("Sign in to save this path.")
        return
      }
      setState("error")
      setMessage(result.state === "invalid" ? "This path is no longer valid. Update your search and try again." : "We couldn’t save this path. Please try again.")
...
export function HomePathSaveButton({ compact = false, prominent = false }: { compact?: boolean; prominent?: boolean }) {
  ...
  const label = saved ? "Saved" : compact ? "Save path" : "Save this path"
