...
export function HomeResults({ query }: { query: FormValues }) {
  ...
  const summary = [
    getOptionLabel(COUNTRY_OPTIONS, query.country),
    getOptionLabel(FIELD_OPTIONS, query.field),
    getOptionLabel(STATUS_OPTIONS, query.status),
  ].filter(Boolean).join(" · ")

  if (schoolFixture) {
    return <HomePathSaveProvider values={query}><HomeSchoolResults fixture={schoolFixture} /></HomePathSaveProvider>
  }

  if (fieldExplorerFixture) {
    return <HomePathSaveProvider values={query}><HomeFieldExplorerResults fixture={fieldExplorerFixture} /></HomePathSaveProvider>
  }

  if (applicationFixture) {
    return <HomePathSaveProvider values={query}><HomeApplicationResults fixture={applicationFixture} /></HomePathSaveProvider>
  }

  if (visaFixture) {
    return <HomePathSaveProvider values={query}><HomeVisaResults fixture={visaFixture} /></HomePathSaveProvider>
  }

  return (
    <section className="mx-auto max-w-5xl px-1 pt-4" aria-label="Pathway search results">
      <div className="border-b border-[#e7e6e3] pb-7">
        <p className="text-sm font-semibold text-[#4a4842]">Your pathway</p>
        <p className="mt-1 text-sm leading-6 text-[#6f6d68]">{summary}</p>
      </div>
      ...
      <HomePathSaveProvider values={query}><HomePathSaveFooter /></HomePathSaveProvider>
...
function RecommendedPathways({ pathways }: { pathways: readonly PathwayFixture[] }) {
  return (
    <section className="border-t border-[#e7e6e3] py-9" aria-labelledby="recommended-pathways-heading">
      ...
      {pathways.length ? (
        ...
        <p role="status" className="mt-4 rounded-xl border border-dashed border-[#dfddd8] px-4 py-3 text-sm text-[#6f6d68]">
          Pathway recommendations for this selection are being prepared.
        </p>
      )}
    </section>
  )
...
function KeyRequirements({ items }: { items: readonly string[] }) {
  return (
    <section className="border-t border-[#e7e6e3] py-9" aria-labelledby="requirements-heading">
