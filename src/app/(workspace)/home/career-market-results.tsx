  ...
  return <section className={cn("mx-auto max-w-5xl", presentation === "page" ? "mt-5" : "mt-12")} aria-live="polite">
    <div>
      <header className="px-1 py-2 sm:px-2">
        <p className="text-xs font-bold tracking-[0.12em] text-blue-700">{t("취업시장 우선 국가 후보", "MARKET-FIRST COUNTRY SHORTLIST")}</p>
      ...
      <div className="mt-7 grid gap-3 md:grid-cols-2">
        {insight.recommendations.map((choice, index) => <Link key={choice.countryCode} href={`${localizePath("/career", locale)}?country=${choice.countryCode}&occupation=${query.occupation}&personalised=1`} className="cc-result-reveal group rounded-2xl border border-[#e2e8f0] bg-white p-5 transition hover:-translate-y-0.5 hover:border-blue-300 hover:shadow-[0_18px_36px_-26px_rgba(30,64,175,.36)]">
          <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold text-slate-400">0{index + 1} · {locale === "ko" ? insight.career.labelKo : englishSafeText(locale, choice.officialTitle, insight.career.label)}</p><h3 className="mt-1 text-lg font-semibold tracking-[-0.03em] text-slate-950">{countryDisplayName(locale, choice.countryCode, choice.countryName)}</h3></div>{choice.opportunityScore != null && <span className="rounded-full bg-[#f4f6fb] px-3 py-1.5 text-xs font-semibold text-[#334155]">{t("취업시장 점수", "Job market score")} {choice.opportunityScore}/100</span>}</div>
...
function GenericCountryCareerInsight({ insight, query, locale, authenticated, personalisation, personalised, presentation }: CountryCareerInsightProps) {
  ...
  const countryName = insight.country ? countryDisplayName(locale, insight.country.code, insight.country.name) : ""

  return <section className={cn("mx-auto max-w-5xl", presentation === "page" ? "mt-5" : "mt-12")} aria-live="polite">
    <div>
      <header className="flex flex-col gap-5 px-1 py-2 sm:flex-row sm:items-start sm:justify-between sm:px-2">
        <div><p className="text-xs font-bold tracking-[0.12em] text-blue-700">{t("무료 커리어 시장 브리프", "FREE CAREER MARKET BRIEF")}</p><h2 className="mt-3 text-2xl font-semibold tracking-[-0.05em] text-slate-950 sm:text-3xl">{locale === "ko" ? `${countryName}에서 ${insight.career.labelKo}로 일하기` : `Working as ${insight.career.label} in ${countryName}`}</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">{locale === "ko" ? (insight.demand?.note ?? insight.career.overview?.ko ?? "현지 직업 시장과 실제 진입 조건을 함께 확인하세요.") : (insight.career.overview?.en ?? "Review the local job market and real entry conditions together.")}</p></div>
      ...
      <div className="mt-8 grid gap-4 lg:grid-cols-2">
        <InsightCard icon={<ShieldCheck className="size-5" />} eyebrow={t("비자 · 면허 · 자격 인정", "VISA · LICENCE · RECOGNITION")} title={t("비자·면허·자격 인정의 막힘 요소", "Visa, licence and recognition blockers")}>
        ...
        <InsightCard icon={<TrendingUp className="size-5" />} eyebrow={t("현실적인 진입 경로", "REALISTIC ENTRY ROUTE")} title={t("현실적인 진입 경로", "A realistic entry route")}>
        ...
        <InsightCard icon={<GraduationCap className="size-5" />} eyebrow={t("교육 · 학업 경로", "EDUCATION · STUDY ROUTE")} title={t("관련 교육·유학 경로", "Related education and study routes")}>
        ...
        <InsightCard icon={<Building2 className="size-5" />} eyebrow={t("고용주 · 실제 채용", "EMPLOYERS · LIVE ROLES")} title={t("관련 직업·고용주 정보", "Related roles and employers")}>
    ...
    <div>
      <header className="px-1 py-2 sm:px-2">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <p className="text-xs font-bold tracking-[0.12em] text-blue-700">{t("호주 · 간호사", "AUSTRALIA · REGISTERED NURSE")}</p>
      ...
      <div className="space-y-10 py-8 sm:py-10">
        ...
        <section aria-labelledby="nursing-route-heading" className="cc-result-reveal">
          <div><p className="text-xs font-bold tracking-[0.1em] text-blue-700">{t("학업 → 등록 → 취업", "STUDY → REGISTRATION → WORK")}</p><h3 id="nursing-route-heading" className="mt-2 text-xl font-semibold tracking-[-0.045em] text-slate-950">{t("학업은 첫 간호사 직장으로 이어지는 세 단계 중 하나예요.", "Study is one of three stages that lead to a first nursing role.")}</h3></div>
        ...
        {regionalSignals.length > 0 && <section aria-labelledby="nursing-region-heading" className="cc-result-reveal">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold tracking-[0.1em] text-blue-700">{t("수요 신호가 있는 지역", "WHERE THE SIGNALS ARE")}</p><h3 id="nursing-region-heading" className="mt-2 text-lg font-semibold tracking-[-0.035em] text-slate-950">{t("처음 일자리를 찾을 때 볼 지역 신호", "Regional signals for your first role")}</h3></div><p className="text-xs text-slate-500">{t("최근 3개월 평균 채용 신호", "Latest three-month average hiring signal")}</p></div>
...
function ScoreExplanation({ locale, children }: { locale: Locale; children: ReactNode }) {
  return <details className="cc-score-explanation relative inline-flex">
    <summary aria-label={locale === "ko" ? "취업시장 점수 설명" : "About this job market score"} className="inline-flex cursor-pointer list-none text-slate-400 transition hover:text-blue-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600/30 [&::-webkit-details-marker]:hidden"><CircleAlert className="size-3" /></summary>
