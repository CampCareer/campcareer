...
export function Batch3MethodologyPage({ profile }: { profile: Batch3CountryContent }) {
  return (
    <main className="mx-auto max-w-4xl px-5 py-12 sm:px-6 sm:py-16">
      <JsonLd
        data={breadcrumbLd([
          { name: "Methodology", path: "/methodology" },
          { name: profile.countryName, path: `/methodology/${profile.methodologySlug}` },
        ])}
      />
      ...
      <div className="mt-10 space-y-5">
        {profile.methodologySources.map((item) => (
          <section key={item.title} className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
            ...
            <a href={item.url} target="_blank" rel="noopener noreferrer" className="mt-4 inline-flex items-center gap-1.5 text-sm font-semibold text-blue-600 hover:underline">
              {item.source}
              <ExternalLink className="size-3.5" aria-hidden="true" />
            </a>
            {item.secondaryUrl ? (
              <a href={item.secondaryUrl} target="_blank" rel="noopener noreferrer" className="ml-4 inline-flex items-center gap-1.5 text-sm font-semibold text-blue-600 hover:underline">
      ...
      <p className="mt-8 text-xs leading-5 text-slate-500">
        Last reviewed {profile.lastReviewed}. A national range describes its source population or stated planning scenario; it does not predict an individual&apos;s salary, expenses, admission, employment or residence-permit outcome.
      </p>
    </main>
  )
}
