...
export default function FranceRegionPage({ params }: { params: { region: string } }) {
  ...
  return <main className="mx-auto max-w-5xl px-4 py-12 sm:px-6"><nav className="text-sm text-slate-500"><Link href="/fr" className="hover:text-slate-900">France</Link><span className="px-2">/</span>{region.nameFr}</nav><h1 className="mt-5 text-4xl font-semibold text-slate-950">{region.nameFr} 직업·주거·대학 지도</h1><p className="mt-4 text-slate-600">France Travail BMO 2026 · INSEE private-sector salaries · MESR public institutions</p><section className="mt-8 grid gap-4 md:grid-cols-3"><Box title="임대광고 지표" value={region.rent.advertisedRentEurM2 != null ? `€${region.rent.advertisedRentEurM2.toFixed(1)}/m² / month` : "적격 도시 표본 부족"} note={region.rent.advertisedRentEurM2 != null ? `${region.rent.cityCoverage}/${region.rent.sourceCityCount} 핵심 도시 인구가중 추정` : "관측 평균 월세가 아님"} /><Box title="고용 수요" value={`${demand.length}개 BMO 직업군`} note="지역 채용계획 상위" /><Box title="공공기관" value={`${universities.length}개 핀`} note="MESR 기관 카탈로그" /></section><section className="mt-10 grid gap-8 md:grid-cols-2"><List title="채용 수요 상위" rows={demand.map((row) => ({ name: row.nameKo ?? row.localName, value: `${row.projects.toLocaleString()} projects` }))} /><List title="고소득 직업군" rows={salaries.map((salary) => ({ name: `PCS ${salary.pcsCode}`, value: `€${Math.round(salary.monthlyNetEur).toLocaleString()} net/month` }))} /></section><Link href={`/map?country=fr&state=${region.code}`} className="mt-10 inline-flex text-sm font-semibold text-blue-700 hover:underline">인터랙티브 지도에서 보기</Link></main>
}
