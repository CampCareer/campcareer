...
function getUniversity(slug: string) {
  return DK_UNIVERSITIES.find((university) => university.slug === slug) ?? null
}

export function generateStaticParams() {
  return DK_UNIVERSITIES.map((university) => ({ slug: university.slug }))
}
...
export async function generateMetadata(props: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...pageMetadata({
    title: `${university.nameEn} | Denmark university map | CampCareer`,
    description: `${university.nameEn} (${university.nameKo}), a higher-education institution in ${university.cityName}, Denmark.`,
    path: `/map/dk/university/${university.slug}`,
    }),
    robots: { index: isCountrySearchIndexable("DK"), follow: true },
  }
}

export default async function DenmarkUniversityPage(props: { params: Promise<{ slug: string }> }) {
  const params = await props.params
  const university = getUniversity(params.slug)
  if (!university) notFound()
  return (
    <main className="mx-auto max-w-3xl px-4 py-12 sm:px-6">
      <Link href="/dk" className="text-sm font-semibold text-blue-700 hover:underline">Denmark profile</Link>
