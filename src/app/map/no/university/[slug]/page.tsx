...
function getUniversity(slug: string) {
  return NO_UNIVERSITIES.find((university) => university.slug === slug) ?? null
}

export function generateStaticParams() {
  return NO_UNIVERSITIES.map((university) => ({ slug: university.slug }))
}
...
export async function generateMetadata(props: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...pageMetadata({
    title: `${university.nameEn} | Norway university map | CampCareer`,
    description: `${university.nameEn} (${university.nameKo}), a higher-education institution in ${university.cityName}, Norway.`,
    path: `/map/no/university/${university.slug}`,
    }),
    robots: { index: isCountrySearchIndexable("NO"), follow: true },
  }
}

export default async function NorwayUniversityPage(props: { params: Promise<{ slug: string }> }) {
  const params = await props.params
  const university = getUniversity(params.slug)
  if (!university) notFound()
  return (
    <main className="mx-auto max-w-3xl px-4 py-12 sm:px-6">
      <Link href="/no" className="text-sm font-semibold text-blue-700 hover:underline">Norway profile</Link>
