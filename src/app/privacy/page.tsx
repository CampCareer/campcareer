...
function KoreanPrivacyPage() {
  return (
    <div className="mx-auto max-w-3xl px-6 py-12">
      ...
      <div className="space-y-8 text-sm leading-relaxed text-slate-600">
        <section><h2 className="font-display mb-2 text-lg font-semibold text-slate-800">1. 운영 주체와 문의</h2><p>CampCareer는 유학·커리어 의사결정 서비스를 운영합니다. 이 방침은 어떤 정보를 왜 수집하는지와 이용자의 선택권을 설명합니다. 개인정보 관련 문의나 요청은 <a href="mailto:contact@campcareer.com" className="text-blue-600 hover:underline">contact@campcareer.com</a>으로 보내주세요. 유료 파트너 프로그램을 새 국가에 출시하기 전에는 적용되는 법인과 등록 주소를 이 페이지에 공개합니다.</p></section>
        ...
        <section><h2 className="font-display mb-2 text-lg font-semibold text-slate-800">6. 이용자의 권리</h2><p>거주 지역에 따라 열람, 정정, 내보내기, 삭제를 요청할 수 있습니다. 계정은 <Link href="/ko/settings" className="text-blue-600 hover:underline">계정 설정</Link>에서 삭제할 수 있으며, 그 밖의 요청은 등록 이메일 주소에서 contact@campcareer.com으로 보내주세요. 확인된 요청은 원칙적으로 30일 이내 처리합니다.</p></section>
        <section><h2 className="font-display mb-2 text-lg font-semibold text-slate-800">7. 쿠키와 측정 선택</h2><p>인증과 화면 상태에는 필수 쿠키를 사용합니다. 제품 측정·유입 분석은 “측정 허용”을 선택한 뒤에만 시작합니다. Google Analytics나 제3자 광고 쿠키는 사용하지 않습니다. 제휴 링크를 열면 해당 외부 서비스의 개인정보 처리방침이 적용됩니다.</p></section>
