...
export default function PlannerPage() {
  ...
  return <main className={cn("min-h-screen transition-colors duration-300", plannerThemeClasses[plannerTheme])}>
    <header className="border-b border-slate-200 bg-white/90 backdrop-blur">
      <div className="mx-auto flex max-w-7xl flex-col items-start gap-3 px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:gap-4 sm:px-6">
        <div><h1 className="text-2xl font-semibold tracking-tight text-slate-950 sm:text-3xl">Planner</h1><p className="mt-1 text-sm text-slate-500">{name}&apos;s private space · {formatLongDate(new Date())}</p></div>
        <div className="flex w-full items-center justify-end gap-2 sm:w-auto"><a href="#today" className="hidden rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-sm font-semibold text-slate-700 hover:border-blue-200 hover:text-blue-700 sm:inline-flex">New note</a><div className="relative"><button type="button" onClick={() => setPaletteOpen((open) => !open)} aria-expanded={paletteOpen} className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 hover:border-blue-200 hover:text-blue-700"><Palette className="h-4 w-4" /><span className="hidden sm:inline">Style</span></button>{paletteOpen && <div className="absolute right-0 z-30 mt-2 w-44 rounded-2xl border border-slate-200 bg-white p-2 shadow-xl">{themeOptions.map((theme) => <button type="button" key={theme.id} onClick={() => setTheme(theme.id)} className={cn("flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-sm hover:bg-slate-50", plannerTheme === theme.id && "bg-slate-100 font-semibold text-slate-950")}><span className={cn("h-4 w-4 rounded-full ring-1 ring-slate-200", theme.swatch)} />{theme.label}</button>)}</div>}</div><button type="button" onClick={() => setCustomising((value) => !value)} className={cn("inline-flex items-center gap-1.5 rounded-xl border px-3 py-2 text-sm font-semibold", customising ? "border-blue-600 bg-blue-600 text-white" : "border-slate-200 bg-white text-slate-700 hover:border-blue-200 hover:text-blue-700")}><GripVertical className="h-4 w-4" /><span className="hidden sm:inline">{customising ? "Done" : "Arrange"}</span></button>{customising && <button type="button" onClick={resetPlannerLayout} className="inline-flex rounded-xl border border-slate-200 bg-white p-2 text-slate-500 hover:text-slate-800" title="Reset layout"><RotateCcw className="h-4 w-4" /></button>}<Link href="/profile" aria-label="Open profile" className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 hover:border-blue-200 hover:text-blue-700">{avatarUrl ? <img src={avatarUrl} alt="" className="h-5 w-5 rounded-full object-cover" /> : <UserRound className="h-4 w-4" />}<span className="hidden sm:inline">Profile</span></Link></div>
      </div>
    </header>

    <DndContext sensors={sensors} onDragEnd={handleDragEnd}>
      <SortableContext items={widgetOrder} strategy={rectSortingStrategy}>
        <section className="mx-auto grid max-w-7xl grid-cols-1 gap-5 px-5 py-7 sm:px-6 sm:py-9 lg:grid-cols-12">
      <div className="contents">
        <PlannerWidget id="today" order={widgetOrder.indexOf("today")} size={widgetSizes.today} customising={customising} onResize={cycleWidgetSize}><section id="today" className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
        ...
        </section></PlannerWidget>

        <PlannerWidget id="dates" order={widgetOrder.indexOf("dates")} size={widgetSizes.dates} customising={customising} onResize={cycleWidgetSize}><section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
