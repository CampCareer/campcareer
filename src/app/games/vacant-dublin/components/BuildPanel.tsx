...
function BuildingPreview({ cols, floors, method, tenure }: { cols: number; floors: number; method: Method; tenure: Tenure }) {
  ...
  return (
    <div className="relative rounded-xl mb-3 p-3 flex flex-col items-center gap-px min-h-[106px] justify-end overflow-hidden"
        ...
        {rows.map(r => (
          <div key={r} className="flex gap-px">
            {Array.from({ length: cols2 }).map((_, u) => (
              <div key={u} className="w-[11px] h-[8px] rounded-sm"
      ...
      {/* road */}
      <div className="absolute bottom-0 left-0 right-0 h-[7px] bg-[#78909C]"
...
function Chip({ active, onClick, children, variant = "default" }: { active: boolean; onClick: () => void; children: React.ReactNode; variant?: "default" | "green" | "blue" }) {
  const activeClass = variant === "green" ? "bg-[#43A047] border-[#43A047] text-white" : variant === "blue" ? "bg-[#1E88E5] border-[#1E88E5] text-white" : "bg-[#FDD835] border-[#FDD835] text-[#0d1642]"
  return (
    <button onClick={onClick}
      className={`px-2 py-1 rounded-2xl border-2 text-[0.45rem] font-bold cursor-pointer transition-all whitespace-nowrap ${active ? activeClass : "bg-white/5 border-white/15 text-white/48 hover:border-[#FDD835] hover:text-white"}`}>
      {children}
    </button>
  )
}

export function BuildPanel({ isOpen, site, method, tenure, floors, difficulty, calcResult: r, pendingScore: ps, onMethod, onTenure, onFloors, onBuild, onClose }: Props) {
  if (!site) return null
  ...
  return (
    <div className={`absolute top-[84px] right-0 bottom-0 w-[318px] z-[200] flex flex-col transition-transform duration-300 ease-[cubic-bezier(.34,1.56,.64,1)]`}
      style={{
        background: "rgba(13,22,66,0.96)", borderLeft: "2px solid rgba(255,255,255,0.08)",
        backdropFilter: "blur(14px)", transform: isOpen ? "translateX(0)" : "translateX(318px)"
      }}>
      ...
      <div className="px-4 py-3 border-b border-white/8 bg-white/[0.03]">
        <div className="text-[0.38rem] text-white/28 mb-0.5">{site.id} · {site.years}년 방치</div>
        <div className="font-pixel text-[0.47rem] text-white leading-relaxed mb-0.5">{site.name}</div>
        <div className="text-[0.41rem] text-white/30">{site.owner} · {site.area}ha</div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 space-y-3 [&::-webkit-scrollbar]:w-[3px] [&::-webkit-scrollbar-thumb]:bg-[#FDD835]">
        ...
        {/* construction method */}
        <div>
          <div className="font-pixel text-[0.36rem] text-[#FDD835] mb-2 tracking-[0.04em]">🏗 건설 방식</div>
          <div className="flex gap-1.5 flex-wrap">
            ...
            <Chip active={method === "modular"}  onClick={() => onMethod("modular")}  variant="green">
              📦 Modular <span className="text-[0.38rem] text-[#A5D6A7]">−30%</span>
            </Chip>
          </div>
        </div>
        ...
        {/* tenure */}
        <div>
          <div className="font-pixel text-[0.36rem] text-[#FDD835] mb-2 tracking-[0.04em]">🏘 임대 유형</div>
          <div className="flex gap-1.5 flex-wrap">
        ...
        <div className="bg-white/[0.04] rounded-xl p-3 border border-white/8">
          <div className="flex justify-between mb-2">
            <span className="text-[0.45rem] text-white/44">층수 (Floors)</span>
            <span className="font-pixel text-[0.58rem] text-[#FDD835]">{floors}F</span>
          </div>
          <input type="range" min={3} max={20} value={floors}
          ...
          <div className="flex justify-between text-[0.35rem] text-white/20 mt-1"><span>3F</span><span>20F</span></div>
        </div>
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {[
              ...
              <div key={l} className="bg-white/[0.04] rounded-lg p-3 border border-white/7">
                <div className="font-pixel text-[0.56rem] leading-tight" style={{ color: c }}>{v}</div>
                <div className="text-[0.37rem] text-white/30 mt-0.5 uppercase">{l}</div>
              </div>
            ))}
          </div>
        )}
        ...
        {/* ROI box */}
        {r && (
          <div className="rounded-xl p-3 text-center" style={{ background: "linear-gradient(135deg,rgba(255,184,0,.1),rgba(229,57,53,.1))", border: "2px solid rgba(255,184,0,.28)" }}>
            <div className="font-pixel text-[1.22rem] leading-none mb-1" style={{ color: roiC }}>{r.roi.toFixed(0)}%</div>
            <div className="text-[0.38rem] text-white/36">10년 ROI (10-Year Return)</div>
        ...
        <div className="rounded-xl px-3 py-2" style={{ background: "rgba(67,160,71,.08)", border: "1px solid rgba(67,160,71,.22)" }}>
          <div className="text-[0.37rem] text-white/35 uppercase tracking-[0.05em] mb-2">🏕️ 사회적 임팩트 예측</div>
          <div className="flex justify-between text-[0.5rem] text-white/70 mb-1">
            <span>노숙자 감소</span>
            <span className="font-bold text-[#A5D6A7]">{hlEst}명</span>
          </div>
          <div className="flex justify-between text-[0.5rem] text-white/70">
            <span>월세 인하 기여</span>
        ...
        {/* difficulty notice */}
        {difficulty !== "easy" && (
          <div className="rounded-xl p-2.5 text-[0.41rem] text-white/50 leading-relaxed"
        ...
        {/* build button */}
        <button onClick={onBuild} disabled={!ps}
          className="w-full py-3 rounded-xl font-pixel text-[0.47rem] text-[#0d1642] cursor-pointer tracking-[0.04em] transition-all hover:-translate-y-0.5 disabled:opacity-40 disabled:cursor-not-allowed disabled:transform-none"
