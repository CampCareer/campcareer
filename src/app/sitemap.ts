...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const institutionPages: MetadataRoute.Sitemap = INDEXABLE_UK_INSTITUTION_PATHS.map((path) => ({
    url: `${SITE_URL}${path}`,
    lastModified,
    priority: 0.72,
  ...
  return Array.from(
    new Map([...staticPages, ...institutionPages, ...routePages].map((entry) => [entry.url, entry])).values(),
  )
}
