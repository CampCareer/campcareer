...
export function UkJobsClient({ occupations }: { occupations: OccRow[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (sort === "salary") {
      return [...base].sort((a, b) => (b.median_salary_gbp ?? 0) - (a.median_salary_gbp ?? 0))
    ...
    <>
      ...
      <div className="rounded-xl border border-slate-200 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.map((occ) => (
              <tr key={occ.soc_code} className="border-t border-slate-100 hover:bg-slate-50 transition-colors">
                <td className="px-4 py-3">
                  <Link
                    href={`/roi-explorer/uk/occupation/${occ.soc_code}`}
                    className="font-medium text-foreground hover:text-brand transition-colors"
                  >
                    {occ.occupation_en}
                  </Link>
                  <span className="ml-2 text-xs text-slate-400">SOC {occ.soc_code}</span>
                </td>
