...
export async function generateMetadata({ params }: { params: Promise<{ origin: string; destination: string }> }) {
  ...
  if (!guide) return {}
  return pageMetadata({
    title: `${guide.target.en} in ${guide.destination.name.en}: study and work sources`,
    description: guide.summary.en,
    path: routeGuideHref(guide),
  })
}

export default async function GlobalRouteGuideRoute({ params }: { params: Promise<{ origin: string; destination: string }> }) {
  const route = await params
  const guide = getRouteGuideForDestination(route.origin, route.destination)
  if (!guide) notFound()
